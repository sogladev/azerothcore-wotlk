...
static_assert((sizeof(_luaEvalPrefix) - 1 + sizeof(_luaEvalMidfix) - 1 + sizeof(_luaEvalPostfix) - 1 +
                  WARDEN_MAX_LUA_CHECK_LENGTH) == 255);
...
void WardenWin::RequestChecks()
{
    ...
    for (uint16 const checkId : _CurrentChecks)
    {
        ...
        switch (check->Type)
        {
            case LUA_EVAL_CHECK:
            {
                buff << uint8(sizeof(_luaEvalPrefix) - 1 + check->Str.size() + sizeof(_luaEvalMidfix) - 1 +
                              check->IdStr.size() + sizeof(_luaEvalPostfix) - 1);
    ...
    for (uint16 const checkId : _CurrentChecks)
    {
        ...
        switch (type)
        {
            case MEM_CHECK:
            {
                ...
                {
                    LOG_DEBUG(
                        "warden", "RESULT MEM_CHECK fail CheckId {} account Id {}", checkId, _session->GetAccountId());
                    checkFailed = checkId;
                ...
                buff.rpos(buff.rpos() + rd->Length);
                LOG_DEBUG(
                    "warden", "RESULT MEM_CHECK passed CheckId {} account Id {}", checkId, _session->GetAccountId());
                break;
            }
            case PAGE_CHECK_A:
            case PAGE_CHECK_B:
            ...
            case LUA_EVAL_CHECK:
            {
                ...
                LOG_DEBUG("warden",
                    "LUA_EVAL_CHECK CheckId {} account Id {} got in-warden dummy response",
                    checkId,
            ...
            case MPQ_CHECK:
            {
                ...
                {
                    LOG_DEBUG(
                        "warden", "RESULT MPQ_CHECK fail, CheckId {} account Id {}", checkId, _session->GetAccountId());
                    checkFailed = checkId;
                ...
                buff.rpos(buff.rpos() + Acore::Crypto::Constants::SHA1_DIGEST_LENGTH_BYTES); // 20 bytes SHA1
                LOG_DEBUG(
                    "warden", "RESULT MPQ_CHECK passed, CheckId {} account Id {}", checkId, _session->GetAccountId());
                break;
            }
        }
    }
