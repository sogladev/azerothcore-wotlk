...
template <> bool ActivePoolData::IsActiveObject<Creature>(uint32 db_guid) const
{
    return mSpawnedCreatures.find(db_guid) != mSpawnedCreatures.end();
}

// Method that tell if a gameobject is spawned currently
template <> bool ActivePoolData::IsActiveObject<GameObject>(uint32 db_guid) const
{
    return mSpawnedGameobjects.find(db_guid) != mSpawnedGameobjects.end();
}

// Method that tell if a pool is spawned currently
template <> bool ActivePoolData::IsActiveObject<Pool>(uint32 sub_pool_id) const
{
    return mSpawnedPools.find(sub_pool_id) != mSpawnedPools.end();
}

// Method that tell if a quest can be started
template <> bool ActivePoolData::IsActiveObject<Quest>(uint32 quest_id) const
{
    return mActiveQuests.find(quest_id) != mActiveQuests.end();
}

template <> void ActivePoolData::ActivateObject<Creature>(uint32 db_guid, uint32 pool_id)
{
    mSpawnedCreatures.insert(db_guid);
    ++mSpawnedPools[pool_id];
}

template <> void ActivePoolData::ActivateObject<GameObject>(uint32 db_guid, uint32 pool_id)
{
    mSpawnedGameobjects.insert(db_guid);
    ++mSpawnedPools[pool_id];
}

template <> void ActivePoolData::ActivateObject<Pool>(uint32 sub_pool_id, uint32 pool_id)
{
    mSpawnedPools[sub_pool_id] = 0;
    ++mSpawnedPools[pool_id];
}

template <> void ActivePoolData::ActivateObject<Quest>(uint32 quest_id, uint32 pool_id)
{
    mActiveQuests.insert(quest_id);
    ++mSpawnedPools[pool_id];
}

template <> void ActivePoolData::RemoveObject<Creature>(uint32 db_guid, uint32 pool_id)
{
...
template <> void ActivePoolData::RemoveObject<GameObject>(uint32 db_guid, uint32 pool_id)
{
...
template <> void ActivePoolData::RemoveObject<Pool>(uint32 sub_pool_id, uint32 pool_id)
{
...
template <> void ActivePoolData::RemoveObject<Quest>(uint32 quest_id, uint32 pool_id)
{
...
template <class T> void PoolGroup<T>::AddEntry(PoolObject& poolitem, uint32 maxentries)
{
...
template <class T> bool PoolGroup<T>::CheckPool() const
{
...
template <class T> void PoolGroup<T>::DespawnObject(ActivePoolData& spawns, ObjectGuid::LowType guid)
{
...
template <> void PoolGroup<Creature>::Despawn1Object(ObjectGuid::LowType guid)
{
...
template <> void PoolGroup<GameObject>::Despawn1Object(ObjectGuid::LowType guid)
{
...
// Same on one pool
template <> void PoolGroup<Pool>::Despawn1Object(uint32 child_pool_id)
{
    sPoolMgr->DespawnPool(child_pool_id);
}

// Same on one quest
template <> void PoolGroup<Quest>::Despawn1Object(uint32 quest_id)
{
    // Creatures
...
template <> void PoolGroup<Pool>::RemoveOneRelation(uint32 child_pool_id)
{
...
template <class T> void PoolGroup<T>::SpawnObject(ActivePoolData& spawns, uint32 limit, uint32 triggerFrom)
{
...
template <> void PoolGroup<Creature>::Spawn1Object(PoolObject* obj)
{
...
template <> void PoolGroup<GameObject>::Spawn1Object(PoolObject* obj)
{
...
// Same for 1 pool
template <> void PoolGroup<Pool>::Spawn1Object(PoolObject* obj)
{
    sPoolMgr->SpawnPool(obj->guid);
}

// Same for 1 quest
template <> void PoolGroup<Quest>::Spawn1Object(PoolObject* obj)
{
    // Creatures
    QuestRelations* questMap = sObjectMgr->GetCreatureQuestRelationMap();
...
template <> void PoolGroup<Quest>::SpawnObject(ActivePoolData& spawns, uint32 limit, uint32 triggerFrom)
{
...
template <> void PoolGroup<Creature>::ReSpawn1Object(PoolObject* obj)
{
...
template <> void PoolGroup<GameObject>::ReSpawn1Object(PoolObject* obj)
{
    Despawn1Object(obj->guid);
    Spawn1Object(obj);
}
...
template <> void PoolGroup<Pool>::ReSpawn1Object(PoolObject* /*obj*/) { }

// Nothing to do for a quest
template <> void PoolGroup<Quest>::ReSpawn1Object(PoolObject* /*obj*/) { }
    ...
    LOG_INFO("server.loading", "Loading Creatures Pooling Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    LOG_ERROR("sql.sql",
                        "`pool_creature` has a non existing creature spawn (GUID: {}) defined for pool id ({}), skipped.",
                        guid,
                        pool_id);
                    continue;
                }
                ...
                {
                    LOG_ERROR("sql.sql",
                        "`pool_creature` has an invalid chance ({}) for creature guid ({}) in pool id ({}), skipped.",
                        chance,
                        guid,
                        pool_id);
                    continue;
            ...
            LOG_INFO(
                "server.loading", ">> Loaded {} Creatures In Pools in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    }
    ...
    LOG_INFO("server.loading", "Loading Gameobjects Pooling Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    LOG_ERROR("sql.sql",
                        "`pool_gameobject` has a non existing gameobject spawn (GUID: {}) defined for pool id ({}), skipped.",
                        guid,
                        pool_id);
                    continue;
                }
                ...
                {
                    LOG_ERROR("sql.sql",
                        "`pool_gameobject` has a not lootable gameobject spawn (GUID: {}, type: {}) defined for pool id ({}), skipped.",
                        guid,
                        goinfo->type,
                        pool_id);
                    continue;
                ...
                {
                    LOG_ERROR("sql.sql",
                        "`pool_gameobject` has an invalid chance ({}) for gameobject guid ({}) in pool id ({}), skipped.",
                        chance,
                        guid,
                        pool_id);
                    continue;
            ...
            LOG_INFO(
                "server.loading", ">> Loaded {} Gameobjects In Pools in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    }
        ...
        else
        {
            ...
            do
            {
...

                {
                    ...
                    {
                        LOG_ERROR("sql.sql",
                            "`pool_pool` mother_pool id ({}) is not in `pool_template`, skipped.",
                            mother_pool_id);
                        continue;
                    }
                }
                {
                    ...
                    {
                        LOG_ERROR("sql.sql",
                            "`pool_pool` included pool_id ({}) is not in `pool_template`, skipped.",
                            child_pool_id);
                        continue;
                    }
                }
                ...
                {
                    LOG_ERROR("sql.sql",
                        "`pool_pool` pool_id ({}) includes itself, dead-lock detected, skipped.",
                        child_pool_id);
                    continue;
                }
                if (chance < 0 || chance > 100)
                {
                    LOG_ERROR("sql.sql",
                        "`pool_pool` has an invalid chance ({}) for pool id ({}) in mother pool id ({}), skipped.",
                        chance,
                        child_pool_id,
                        mother_pool_id);
                    continue;
            ...
            for (auto const& it : mPoolTemplate)
            {
                ...
                for (SearchMap::iterator poolItr = mPoolSearchMap.find(it.first); poolItr != mPoolSearchMap.end();
                     poolItr = mPoolSearchMap.find(poolItr->second))
                {
                    ...
                    if (checkedPools.find(poolItr->second) != checkedPools.end())
                    {
                        ...
                        ss << "The pool(s) ";
                        for (std::set<uint32>::const_iterator itr = checkedPools.begin(); itr != checkedPools.end();
                             ++itr)
                            ss << *itr << ' ';
            ...
            LOG_INFO(
                "server.loading", ">> Loaded {} Pools In Mother Pools in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
            LOG_INFO("server.loading", " ");
        }
    }
    ...
    LOG_INFO("server.loading", "Loading Quest Pooling Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    LOG_ERROR("sql.sql",
                        "`pool_quest` has a non existing quest template (Entry: {}) defined for pool id ({}), skipped.",
                        entry,
                        pool_id);
                    continue;
                }
                ...
                {
                    LOG_ERROR("sql.sql",
                        "`pool_quest` has an quest ({}) which is not daily or weekly in pool id ({}), use ExclusiveGroup instead, skipped.",
                        entry,
                        pool_id);
                    continue;
                }
                ...
                if (poolTypeMap[pool_id] != currType)
                {
                    LOG_ERROR("sql.sql",
                        ...
                        pool_id,
                        poolTypeMap[pool_id] == QUEST_DAILY ? "QUEST_DAILY" : "QUEST_WEEKLY");
                    continue;
                }
                ...
                {
                    LOG_ERROR("sql.sql",
                        "`pool_quest` lists entry ({}) as member of pool ({}) but is not started anywhere, skipped.",
                        entry,
                        pool_id);
                    continue;
                }
            ...
            do
            {
                ...
                if (!CheckPool(pool_entry))
                {
                    if (pool_pool_id)
                        ...
                        LOG_ERROR("sql.sql",
                            "Pool Id {} has no equal chance pooled entites defined and explicit chance sum is not 100. This broken pool is a child pool of Id {} and cannot be safely removed.",
                            pool_entry,
                            fields[2].Get<uint32>());
                    else
                        LOG_ERROR("sql.sql",
                            "Pool Id {} has no equal chance pooled entites defined and explicit chance sum is not 100. The pool will not be spawned.",
                            pool_entry);
                    continue;
                }
        ...
        if (deletedPools.find(itr->second) != deletedPools.end())
            if (IsSpawnedObject<Quest>(itr->first))
            {
                CharacterDatabasePreparedStatement* stmt =
                    CharacterDatabase.GetPreparedStatement(CHAR_INS_QUEST_POOL_SAVE);
...
template <> void PoolMgr::SpawnPool<Creature>(uint32 pool_id, uint32 db_guid)
{
...
template <> void PoolMgr::SpawnPool<GameObject>(uint32 pool_id, uint32 db_guid)
{
...
template <> void PoolMgr::SpawnPool<Pool>(uint32 pool_id, uint32 sub_pool_id)
{
...
template <> void PoolMgr::SpawnPool<Quest>(uint32 pool_id, uint32 quest_id)
{
...
template <typename T> void PoolMgr::UpdatePool(uint32 pool_id, uint32 db_guid_or_pool_id)
{
