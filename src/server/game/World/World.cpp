...
void World::AddSession_(WorldSession* s)
{
    ...
    if (pLimit > 0 && Sessions >= pLimit && AccountMgr::IsPlayerAccount(s->GetSecurity()) && !s->CanSkipQueue() &&
...
void World::LoadConfigSettings(bool reload)
{
    ...
    {
        LOG_ERROR(
            "server.loading", "Rate.Rage.Loss ({}) must be > 0. Using 1 instead.", _rate_values[RATE_POWER_RAGE_LOSS]);
    ...
    {
        LOG_ERROR(
            "server.loading", "Rate.RepairCost ({}) must be >=0. Using 0.0 instead.", _rate_values[RATE_REPAIRCOST]);
    ...
    _rate_values[RATE_REPUTATION_RECRUIT_A_FRIEND_BONUS] =
        sConfigMgr->GetOption<float>("Rate.Reputation.RecruitAFriendBonus", 0.1f);
    ...
    _rate_values[RATE_CREATURE_ELITE_RAREELITE_DAMAGE] =
        sConfigMgr->GetOption<float>("Rate.Creature.Elite.RAREELITE.Damage", 1.0f);
    _rate_values[RATE_CREATURE_ELITE_WORLDBOSS_DAMAGE] =
        sConfigMgr->GetOption<float>("Rate.Creature.Elite.WORLDBOSS.Damage", 1.0f);
    ...
    _rate_values[RATE_CREATURE_ELITE_ELITE_SPELLDAMAGE] =
        sConfigMgr->GetOption<float>("Rate.Creature.Elite.Elite.SpellDamage", 1.0f);
    _rate_values[RATE_CREATURE_ELITE_RAREELITE_SPELLDAMAGE] =
        sConfigMgr->GetOption<float>("Rate.Creature.Elite.RAREELITE.SpellDamage", 1.0f);
    _rate_values[RATE_CREATURE_ELITE_WORLDBOSS_SPELLDAMAGE] =
        sConfigMgr->GetOption<float>("Rate.Creature.Elite.WORLDBOSS.SpellDamage", 1.0f);
    _rate_values[RATE_CREATURE_ELITE_RARE_SPELLDAMAGE] =
        sConfigMgr->GetOption<float>("Rate.Creature.Elite.RARE.SpellDamage", 1.0f);
    ...
    _rate_values[RATE_MISS_CHANCE_MULTIPLIER_TARGET_CREATURE] =
        sConfigMgr->GetOption<float>("Rate.MissChanceMultiplier.TargetCreature", 11.0f);
    _rate_values[RATE_MISS_CHANCE_MULTIPLIER_TARGET_PLAYER] =
        sConfigMgr->GetOption<float>("Rate.MissChanceMultiplier.TargetPlayer", 7.0f);
    _bool_configs[CONFIG_MISS_CHANCE_MULTIPLIER_ONLY_FOR_PLAYERS] =
        sConfigMgr->GetOption<bool>("Rate.MissChanceMultiplier.OnlyAffectsPlayer", false);
    ...
    for (uint8 i = 0; i < MAX_MOVE_TYPE; ++i)
        playerBaseMoveSpeed[i] = baseMoveSpeed[i] * _rate_values[RATE_MOVESPEED];
    ...
    {
        LOG_ERROR("server.loading",
            "Compression level ({}) must be in range 1..9. Using default compression level (1).",
    ...
    _int_configs[CONFIG_INTERVAL_SAVE] =
        sConfigMgr->GetOption<int32>("PlayerSaveInterval", 15 * MINUTE * IN_MILLISECONDS);
    ...
    {
        LOG_ERROR("server.loading",
            "PlayerSave.Stats.MinLevel ({}) must be in range 0..80. Using default, do not save character stats (0).",
    ...
    {
        LOG_ERROR("server.loading",
            "MapUpdateInterval ({}) must be greater {}. Use this minimal value.",
    ...
    _int_configs[CONFIG_INTERVAL_CHANGEWEATHER] =
        sConfigMgr->GetOption<int32>("ChangeWeatherInterval", 10 * MINUTE * IN_MILLISECONDS);
    ...
    if (reload)
    {
        ...
        if (val != _int_configs[CONFIG_PORT_WORLD])
            LOG_ERROR("server.loading",
                "WorldServerPort option can't be changed at worldserver.conf reload, using current value ({}).",
    ...
    if (reload)
    {
        ...
        if (val != _int_configs[CONFIG_GAME_TYPE])
            LOG_ERROR("server.loading",
                "GameType option can't be changed at worldserver.conf reload, using current value ({}).",
    ...
    if (reload)
    {
        ...
        if (val != _int_configs[CONFIG_REALM_ZONE])
            LOG_ERROR("server.loading",
                "RealmZone option can't be changed at worldserver.conf reload, using current value ({}).",
    ...
    {
        LOG_ERROR("server.loading",
            "CharactersPerAccount ({}) can't be less than CharactersPerRealm ({}).",
    ...
    _int_configs[CONFIG_CHARACTER_CREATING_MIN_LEVEL_FOR_HEROIC_CHARACTER] =
        sConfigMgr->GetOption<int32>("CharacterCreating.MinLevelForHeroicCharacter", 55);
    ...
    if (reload)
    {
        ...
        if (val != _int_configs[CONFIG_MAX_PLAYER_LEVEL])
            LOG_ERROR("server.loading",
                "MaxPlayerLevel option can't be changed at config reload, using current value ({}).",
    ...
    if (_int_configs[CONFIG_START_PLAYER_LEVEL] < 1 ||
        _int_configs[CONFIG_START_PLAYER_LEVEL] > _int_configs[CONFIG_MAX_PLAYER_LEVEL])
    {
        LOG_ERROR("server.loading",
            "StartPlayerLevel ({}) must be in range 1..MaxPlayerLevel({}). Set to 1.",
    ...
    {
        LOG_ERROR("server.loading",
            "StartHeroicPlayerLevel ({}) must be in range 1..MaxPlayerLevel({}). Set to 55.",
    ...
    if (int32(_int_configs[CONFIG_START_HEROIC_PLAYER_MONEY]) < 0 ||
        int32(_int_configs[CONFIG_START_HEROIC_PLAYER_MONEY]) > MAX_MONEY_AMOUNT)
    {
        LOG_ERROR("server.loading",
    ...
    if (int32(_int_configs[CONFIG_START_HONOR_POINTS]) < 0 ||
        int32(_int_configs[CONFIG_START_HONOR_POINTS]) > int32(_int_configs[CONFIG_MAX_HONOR_POINTS]))
    {
        LOG_ERROR("server.loading",
    ...
    if (int32(_int_configs[CONFIG_START_ARENA_POINTS]) < 0 ||
        int32(_int_configs[CONFIG_START_ARENA_POINTS]) > int32(_int_configs[CONFIG_MAX_ARENA_POINTS]))
    {
        LOG_ERROR("server.loading",
    ...
    _int_configs[CONFIG_INSTANCE_RESET_TIME_RELATIVE_TIMESTAMP] =
        sConfigMgr->GetOption<int32>("Instance.ResetTimeRelativeTimestamp", 1135814400);
    _int_configs[CONFIG_INSTANCE_UNLOAD_DELAY] =
        sConfigMgr->GetOption<int32>("Instance.UnloadDelay", 30 * MINUTE * IN_MILLISECONDS);
    ...
    _int_configs[CONFIG_LOGDB_CLEARTIME] =
        sConfigMgr->GetOption<int32>("LogDB.Opt.ClearTime", 1209600); // 14 days default
    ...
    _bool_configs[CONFIG_DONT_CACHE_RANDOM_MOVEMENT_PATHS] =
        sConfigMgr->GetOption<bool>("DontCacheRandomMovementPaths", false);
    ...
    {
        LOG_ERROR("server.loading",
            "MaxOverspeedPings ({}) must be in range 2..infinity (or 0 to disable check). Set to 2.",
    ...
    if (reload)
    {
        ...
        if (val != _int_configs[CONFIG_EXPANSION])
            LOG_ERROR("server.loading",
                "Expansion option can't be changed at worldserver.conf reload, using current value ({}).",
    ...
    _float_configs[CONFIG_CREATURE_FAMILY_FLEE_ASSISTANCE_RADIUS] =
        sConfigMgr->GetOption<float>("CreatureFamilyFleeAssistanceRadius", 30.0f);
    ...
    {
        LOG_ERROR(
            "server.loading", "Guild.ResetHour ({}) can't be load. Set to 6.", _int_configs[CONFIG_GUILD_RESET_HOUR]);
    ...
    _bool_configs[CONFIG_DECLINED_NAMES_USED] = (_int_configs[CONFIG_REALM_ZONE] == REALM_ZONE_RUSSIAN)
                                                    ? true
    ...
    _int_configs[CONFIG_BATTLEGROUND_OVERRIDE_LOWLEVELS_MINPLAYERS] =
        sConfigMgr->GetOption<uint32>("Battleground.Override.LowLevels.MinPlayers", 0);
    _bool_configs[CONFIG_BATTLEGROUND_DISABLE_QUEST_SHARE_IN_BG] =
        sConfigMgr->GetOption<bool>("Battleground.DisableQuestShareInBG", false);
    _bool_configs[CONFIG_BATTLEGROUND_DISABLE_READY_CHECK_IN_BG] =
        sConfigMgr->GetOption<bool>("Battleground.DisableReadyCheckInBG", false);
    ...
    _bool_configs[CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_ENABLE] =
        sConfigMgr->GetOption<bool>("Battleground.QueueAnnouncer.Enable", false);
    _int_configs[CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_LIMIT_MIN_LEVEL] =
        sConfigMgr->GetOption<uint32>("Battleground.QueueAnnouncer.Limit.MinLevel", 0);
    _int_configs[CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_LIMIT_MIN_PLAYERS] =
        sConfigMgr->GetOption<uint32>("Battleground.QueueAnnouncer.Limit.MinPlayers", 3);
    _int_configs[CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_SPAM_DELAY] =
        sConfigMgr->GetOption<uint32>("Battleground.QueueAnnouncer.SpamProtection.Delay", 30);
    _bool_configs[CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_PLAYERONLY] =
        sConfigMgr->GetOption<bool>("Battleground.QueueAnnouncer.PlayerOnly", false);
    ...
    _int_configs[CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_TIMER] =
        sConfigMgr->GetOption<uint32>("Battleground.QueueAnnouncer.Timer", 30000);
    _bool_configs[CONFIG_BATTLEGROUND_STORE_STATISTICS_ENABLE] =
        sConfigMgr->GetOption<bool>("Battleground.StoreStatistics.Enable", false);
    _bool_configs[CONFIG_BATTLEGROUND_TRACK_DESERTERS] =
        sConfigMgr->GetOption<bool>("Battleground.TrackDeserters.Enable", false);
    _int_configs[CONFIG_BATTLEGROUND_PREMATURE_FINISH_TIMER] =
        sConfigMgr->GetOption<int32>("Battleground.PrematureFinishTimer", 5 * MINUTE * IN_MILLISECONDS);
    ...
    _int_configs[CONFIG_BATTLEGROUND_PREMADE_GROUP_WAIT_FOR_MATCH] =
        sConfigMgr->GetOption<int32>("Battleground.PremadeGroupWaitForMatch", 30 * MINUTE * IN_MILLISECONDS);
    ...
    {
        LOG_ERROR("server.loading",
            "Battleground.RestorationBuffRespawn ({}) must be > 0. Using 20 instead.",
            _int_configs[CONFIG_BATTLEGROUND_RESTORATION_BUFF_RESPAWN]);
    ...
    {
        LOG_ERROR("server.loading",
            "Battleground.BerserkingBuffRespawn ({}) must be > 0. Using 120 instead.",
            _int_configs[CONFIG_BATTLEGROUND_BERSERKING_BUFF_RESPAWN]);
    ...
    {
        LOG_ERROR("server.loading",
            "Battleground.SpeedBuffRespawn ({}) must be > 0. Using 150 instead.",
            _int_configs[CONFIG_BATTLEGROUND_SPEED_BUFF_RESPAWN]);
    ...
    _int_configs[CONFIG_ARENA_RATING_DISCARD_TIMER] =
        sConfigMgr->GetOption<uint32>("Arena.RatingDiscardTimer", 10 * MINUTE * IN_MILLISECONDS);
    _int_configs[CONFIG_ARENA_PREV_OPPONENTS_DISCARD_TIMER] =
        sConfigMgr->GetOption<uint32>("Arena.PreviousOpponentsDiscardTimer", 2 * MINUTE * IN_MILLISECONDS);
    _bool_configs[CONFIG_ARENA_AUTO_DISTRIBUTE_POINTS] =
        sConfigMgr->GetOption<bool>("Arena.AutoDistributePoints", false);
    _int_configs[CONFIG_ARENA_AUTO_DISTRIBUTE_INTERVAL_DAYS] = sConfigMgr->GetOption<uint32>(
        "Arena.AutoDistributeInterval", 7); // pussywizard: spoiled by implementing constant day and hour, always 7 now
    ...
    _int_configs[CONFIG_ARENA_START_PERSONAL_RATING] =
        sConfigMgr->GetOption<uint32>("Arena.ArenaStartPersonalRating", 0);
    ...
    _float_configs[CONFIG_ARENA_MATCHMAKER_RATING_MODIFIER] =
        sConfigMgr->GetOption<float>("Arena.ArenaMatchmakerRatingModifier", 24.0f);
    ...
    _int_configs[CONFIG_CREATURE_STOP_FOR_PLAYER] =
        sConfigMgr->GetOption<uint32>("Creature.MovingStopTimeForPlayer", 3 * MINUTE * IN_MILLISECONDS);
    ...
    _int_configs[CONFIG_GUILD_EVENT_LOG_COUNT] =
        sConfigMgr->GetOption<int32>("Guild.EventLogRecordsCount", GUILD_EVENTLOG_MAX_RECORDS);
    ...
    _int_configs[CONFIG_GUILD_BANK_EVENT_LOG_COUNT] =
        sConfigMgr->GetOption<int32>("Guild.BankEventLogRecordsCount", GUILD_BANKLOG_MAX_RECORDS);
    ...
    _maxVisibleDistanceOnContinents =
        sConfigMgr->GetOption<float>("Visibility.Distance.Continents", DEFAULT_VISIBILITY_DISTANCE);
    ...
    _maxVisibleDistanceInInstances =
        sConfigMgr->GetOption<float>("Visibility.Distance.Instances", DEFAULT_VISIBILITY_INSTANCE);
    ...
    _maxVisibleDistanceInBGArenas =
        sConfigMgr->GetOption<float>("Visibility.Distance.BGArenas", DEFAULT_VISIBILITY_BGARENAS);
    ...
    _bool_configs[CONFIG_VMAP_BLIZZLIKE_LOS_OPEN_WORLD] =
        sConfigMgr->GetOption<bool>("vmap.BlizzlikeLOSInOpenWorld", true);
    ...
    LOG_INFO("server.loading",
        "WORLD: VMap support included. LineOfSight:{}, getHeight:{}, indoorCheck:{} PetLOS:{}",
        enableLOS,
        enableHeight,
        enableIndoor,
        enablePetLOS);
    ...
    _bool_configs[CONFIG_ENABLE_CONTINENT_TRANSPORT_PRELOADING] =
        sConfigMgr->GetOption<bool>("IsPreloadedContinentTransport.Enabled", false);
    ...
    _bool_configs[CONFIG_CALCULATE_GAMEOBJECT_ZONE_AREA_DATA] =
        sConfigMgr->GetOption<bool>("Calculate.Gameoject.Zone.Area.Data", false);
    ...
    _bool_configs[CONFIG_SET_ALL_CREATURES_WITH_WAYPOINT_MOVEMENT_ACTIVE] =
        sConfigMgr->GetOption<bool>("SetAllCreaturesWithWaypointMovementActive", false);

    // packet spoof punishment
    _int_configs[CONFIG_PACKET_SPOOF_POLICY] =
        sConfigMgr->GetOption<int32>("PacketSpoof.Policy", (uint32)WorldSession::DosProtection::POLICY_KICK);
    ...
    _bool_configs[CONFIG_DUNGEON_ACCESS_REQUIREMENTS_PORTAL_CHECK_ILVL] =
        sConfigMgr->GetOption<bool>("DungeonAccessRequirements.PortalAvgIlevelCheck", false);
    _bool_configs[CONFIG_DUNGEON_ACCESS_REQUIREMENTS_LFG_DBC_LEVEL_OVERRIDE] =
        sConfigMgr->GetOption<bool>("DungeonAccessRequirements.LFGLevelDBCOverride", false);
    _int_configs[CONFIG_DUNGEON_ACCESS_REQUIREMENTS_OPTIONAL_STRING_ID] =
        sConfigMgr->GetOption<int32>("DungeonAccessRequirements.OptionalStringID", 0);
    ...
    _bool_configs[CONFIG_ALLOW_LOGGING_IP_ADDRESSES_IN_DATABASE] =
        sConfigMgr->GetOption<bool>("AllowLoggingIPAddressesInDatabase", true, true);
    ...
    _int_configs[CONFIG_LFG_KICK_PREVENTION_TIMER] =
        sConfigMgr->GetOption<int32>("LFG.KickPreventionTimer", 15 * MINUTE * IN_MILLISECONDS) * IN_MILLISECONDS;
...
void World::SetInitialWorldSettings()
{
    ...
    if (!sConfigMgr->isDryRun())
    {
        ...
        if (!MapMgr::ExistMapAndVMap(0, -6240.32f, 331.033f) || !MapMgr::ExistMapAndVMap(0, -8949.95f, -132.493f) ||
            !MapMgr::ExistMapAndVMap(1, -618.518f, -4251.67f) || !MapMgr::ExistMapAndVMap(0, 1676.35f, 1677.45f) ||
            !MapMgr::ExistMapAndVMap(1, 10311.3f, 832.463f) || !MapMgr::ExistMapAndVMap(1, -2917.58f, -257.98f) ||
            (_int_configs[CONFIG_EXPANSION] && (!MapMgr::ExistMapAndVMap(530, 10349.6f, -6357.29f) ||
    ...
    LoginDatabase.Execute("UPDATE realmlist SET icon = {}, timezone = {} WHERE id = '{}'",
        server_type,
        realm_zone,
    ...
    sObjectMgr->SetDBCLocaleIndex(
        GetDefaultDbcLocale()); // Get once for all the locale index of DBC language (console/broadcasts)
    ...
    sObjectMgr
        ->LoadQuestGreetingsLocales(); // must be loaded after creature_template, gameobject_template tables, quest_greeting
    ...
    LOG_INFO(
        "server.loading", "Loading World States..."); // must be loaded before battleground, outdoor PvP and conditions
    LoadWorldStates();
    ...
    LOG_INFO("server.loading",
        "Initializing SpellInfo Precomputed Data..."); // must be called after loading items, professions, spells and pretty much anything
    LOG_INFO("server.loading", " ");
    ...
    _timers[WUPDATE_PINGDB].SetInterval(
        getIntConfig(CONFIG_DB_PING_INTERVAL) * MINUTE * IN_MILLISECONDS); // Mysql ping time in minutes
    ...
    LoginDatabase.Execute(
        "DELETE FROM ip_banned WHERE unbandate <= UNIX_TIMESTAMP() AND unbandate<>bandate"); // One-time query
    ...
    METRIC_EVENT("events",
        "World initialized",
        "World Initialized In " + std::to_string(startupDuration / 60000) + " Minutes " +
            std::to_string((startupDuration % 60000) / 1000) + " Seconds");
...
void World::SendGlobalMessage(WorldPacket const* packet, WorldSession* self, TeamId teamId)
{
    ...
    for (itr = _sessions.begin(); itr != _sessions.end(); ++itr)
    {
        if (itr->second && itr->second->GetPlayer() && itr->second->GetPlayer()->IsInWorld() && itr->second != self &&
...
void World::SendZoneText(uint32 zone, char const* text, WorldSession* self, TeamId teamId)
{
    WorldPacket data;
...
void World::ShutdownServ(uint32 time, uint32 options, uint8 exitcode, std::string const& reason)
{
    ...
    if (time >= 5)
    {
        playersSaveScheduler.Schedule(Seconds(time - 5),
...
void World::ShutdownMsg(bool show, Player* player, std::string const& reason)
{
    ...
    if (show || (_shutdownTimer < 5 * MINUTE && (_shutdownTimer % 15) == 0) ||    // < 5 min; every 15 sec
        ...
        (_shutdownTimer > 12 * HOUR && (_shutdownTimer % (12 * HOUR)) == 0))      // > 12 h ; every 12 h
    {
        ...
        ServerMessageType msgid =
            (_shutdownMask & SHUTDOWN_MASK_RESTART) ? SERVER_MSG_RESTART_TIME : SERVER_MSG_SHUTDOWN_TIME;
...
void World::ShutdownCancel()
{
    ...
    ServerMessageType msgid =
        (_shutdownMask & SHUTDOWN_MASK_RESTART) ? SERVER_MSG_RESTART_CANCELLED : SERVER_MSG_SHUTDOWN_CANCELLED;
...
void World::SendServerMessage(
    ServerMessageType messageID, std::string stringParam /*= ""*/, Player* player /*= nullptr*/)
{
...
{
    {
        METRIC_DETAILED_NO_THRESHOLD_TIMER(
            "world_update_time", METRIC_TAG("type", "Add sessions"), METRIC_TAG("parent_type", "Update sessions"));
...
void World::UpdateRealmCharCount(uint32 accountId)
{
    ...
    _queryProcessor.AddCallback(CharacterDatabase.AsyncQuery(stmt).WithPreparedCallback(
        std::bind(&World::_UpdateRealmCharCount, this, std::placeholders::_1)));
}
...
void World::InitCalendarOldEventsDeletionTime()
{
    ...
    Seconds nextDeletionTime =
        currentDeletionTime > 0s
            ? currentDeletionTime
            : Seconds(Acore::Time::GetNextTimeWithDayAndHour(-1, getIntConfig(CONFIG_CALENDAR_DELETE_OLD_EVENTS_HOUR)));
...
void World::LoadWorldStates()
{
    ...
    LOG_INFO(
        "server.loading", ">> Loaded {} World States in {} ms", _worldstates.size(), GetMSTimeDiffToNow(oldMSTime));
    LOG_INFO("server.loading", " ");
}
...
CliCommandHolder::CliCommandHolder(
    void* callbackArg, char const* command, Print zprint, CommandFinished commandFinished) :
