...
std::string WorldState::GetScourgeInvasionPrintout()
{
    ...
    auto formatRemaining = [&](SIRemaining val, char const* name) { return "  " + std::string(name) + ": " + std::to_string(m_siData.m_remaining[val]) + "\n"; };
    ...
    output += "Zone Necropolis Counts Remaining:\n";
    ...
    output += "Zone Timers (next event if applicable, 0 if not set or passed):\n";
    TimePoint now = TimePoint();
    auto formatTimer = [&](SITimers timerId, char const* name)
    {
    ...
    output += formatTimer(SI_TIMER_WINTERSPRING, "Winterspring Invasion");
    output += formatTimer(SI_TIMER_AZSHARA, "Azshara Invasion");
    output += formatTimer(SI_TIMER_BLASTED_LANDS, "Blasted Lands Invasion");
    output += formatTimer(SI_TIMER_BURNING_STEPPES, "Burning Steppes Invasion");
    output += formatTimer(SI_TIMER_TANARIS, "Tanaris Invasion");
    output += formatTimer(SI_TIMER_EASTERN_PLAGUELANDS, "Eastern Plaguelands Invasion");
    output += formatTimer(SI_TIMER_STORMWIND, "Stormwind City Attack");
    output += formatTimer(SI_TIMER_UNDERCITY, "Undercity Attack");

    output += "Active Invasion GUIDs:\n";
    for (auto const& pair : m_siData.m_invasionPoints)
        if (pair.second.mouthGuid)
            output += "  Zone " + std::to_string(pair.first) + " (Mouth): " + pair.second.mouthGuid.ToString() + "\n";
    for (auto const& pair : m_siData.m_attackPoints)
        if (pair.second.pallidGuid)
            output += "  Zone " + std::to_string(pair.first) + " (Pallid): " + pair.second.pallidGuid.ToString() + "\n";
    if (m_siData.m_pendingInvasions.empty() && m_siData.m_pendingPallids.empty())
    {
        bool hasActiveGuid = false;
        for (auto const& pair : m_siData.m_invasionPoints)
            if (pair.second.mouthGuid)
                hasActiveGuid = true;
        for (auto const& pair : m_siData.m_attackPoints)
    ...
    if (!m_siData.m_pendingInvasions.empty())
    {
        output += "  Pending Invasions (Zone IDs): ";
        for (uint32 zoneId : m_siData.m_pendingInvasions)
            output += std::to_string(zoneId) + " ";
        output += "\n";
    }
    if (!m_siData.m_pendingPallids.empty())
    {
        output += "  Pending Pallids (Zone IDs): ";
        for (uint32 zoneId : m_siData.m_pendingPallids)
            output += std::to_string(zoneId) + " ";
        output += "\n";
    }
    if (m_siData.m_pendingInvasions.empty() && m_siData.m_pendingPallids.empty())
...
ScourgeInvasionData::ScourgeInvasionData() : m_state(STATE_0_DISABLED), m_battlesWon(0), m_broadcastTimer(10000), m_lastAttackZone(0), m_remaining{}
{
    InvasionZone winterspring;
    {
        winterspring.map = 1;
        winterspring.zoneId = AREA_WINTERSPRING;
    ...
    {
        blasted_lands.map = 0;
        blasted_lands.zoneId = AREA_BLASTED_LANDS;
    ...
    InvasionZone eastern_plaguelands;
    {
        ...
        eastern_plaguelands.zoneId = AREA_EASTERN_PLAGUELANDS;
    ...
    InvasionZone burning_steppes;
    {
        ...
        burning_steppes.zoneId = AREA_BURNING_STEPPES;
    ...
    m_invasionPoints.emplace(AREA_WINTERSPRING, winterspring);
    m_invasionPoints.emplace(AREA_TANARIS, tanaris);
    m_invasionPoints.emplace(AREA_AZSHARA, azshara);
    m_invasionPoints.emplace(AREA_BLASTED_LANDS, blasted_lands);
    m_invasionPoints.emplace(AREA_EASTERN_PLAGUELANDS, eastern_plaguelands);
    m_invasionPoints.emplace(AREA_BURNING_STEPPES, burning_steppes);

    CityAttack undercity;
    {
        undercity.map = 0;
        undercity.zoneId = AREA_UNDERCITY;
    ...
    {
        stormwind.map = 0;
        stormwind.zoneId = AREA_STORMWIND_CITY;
    ...
    m_attackPoints.emplace(AREA_UNDERCITY, undercity);
    m_attackPoints.emplace(AREA_STORMWIND_CITY, stormwind);
}
...
bool WorldState::SummonMouth(Map* map, ScourgeInvasionData::InvasionZone& zone, Position position, bool newInvasion)
{
   ...
   if (Creature* existingMouth = map->GetCreature(zone.mouthGuid))
    ...
    if (Creature* mouth = map->SummonCreature(NPC_MOUTH_OF_KELTHUZAD, position))
    {
        mouth->GetAI()->DoAction(EVENT_MOUTH_OF_KELTHUZAD_ZONE_START);
...
void WorldState::HandleActiveZone(uint32 attackTimeVar, uint32 zoneId, uint32 remainingVar, TimePoint now)
{
    ...
    if (zone.mouthGuid)
    {
        ...
        else if (timePoint < now && remaining == 0)
        {
                ...
                LOG_DEBUG("gamevent", "[Scourge Invasion Event] The Scourge has been defeated in {}, next attack starting in {} minutes.", zoneId, timeToNextAttack);
                LOG_DEBUG("gamevent", "[Scourge Invasion Event] {} victories", sWorldState->GetBattlesWon());

                if (mouth)
                    mouth->GetAI()->DoAction(EVENT_MOUTH_OF_KELTHUZAD_ZONE_STOP);
                else
                    LOG_ERROR("gamevent", "ScourgeInvasionEvent::HandleActiveZone ObjectGuid {} not found.", zone.mouthGuid.ToString());
        }
        // });
    }
    else
...
uint32 WorldState::GetActiveZones()
{
    ...
    for (auto const& invasionPoint : m_siData.m_invasionPoints)
    {
        Map* mapPtr = GetMap(invasionPoint.second.map, invasionPoint.second.mouth[0]);
        if (!mapPtr)
        {
            LOG_ERROR("gameevent", "ScourgeInvasionEvent::GetActiveZones no map for zone {}.", invasionPoint.second.map);
            continue;
        }
