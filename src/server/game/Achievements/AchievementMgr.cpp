...
bool AchievementCriteriaData::IsValid(AchievementCriteriaEntry const* criteria)
{
    ...
    {
        LOG_ERROR("sql.sql",
            "Table `achievement_criteria_data` for criteria (Entry: {}) has wrong data type ({}), ignored.",
            criteria->ID,
            dataType);
        return false;
    }
    ...
    switch (criteria->requiredType)
    {
        ...
        default:
            ...
            {
                LOG_ERROR("sql.sql",
                    "Table `achievement_criteria_data` has data for non-supported criteria type (Entry: {} Type: {}), ignored.",
                    criteria->ID,
    ...
    switch (dataType)
    {
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_AURA:
        {
            ...
            if (!spellInfo)
            {
                LOG_ERROR("sql.sql",
                    ...
                    criteria->ID,
                    criteria->requiredType,
                    (dataType == ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AURA ? "ACHIEVEMENT_CRITERIA_DATA_TYPE_S_AURA"
        ...
        default:
            LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` (Entry: {} Type: {}) has data for non-supported data type ({}), ignored.",
                criteria->ID,
                criteria->requiredType,
                dataType);
            return false;
...
bool AchievementCriteriaData::Meets(
    uint32 criteria_id, Player const* source, Unit const* target, uint32 miscvalue1 /*= 0*/) const
{
    switch (dataType)
    {
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_NONE:
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_SCRIPT:
            return sScriptMgr->OnCriteriaCheck(
                ScriptId, const_cast<Player*>(source), const_cast<Unit*>(target), criteria_id);
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_MAP_DIFFICULTY:
        {
            ...
            return sAchievementMgr->IsStatisticCriteria(criteria) ? spawnMode == difficulty.difficulty
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "Achievement system call ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT ({}) for achievement criteria {} for non-dungeon/non-raid map {}",
            ...
            {
                LOG_ERROR("sql.sql",
                    "Achievement system call ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT ({}) for achievement criteria {} for map {} but map does not have a instance script",
                    ACHIEVEMENT_CRITERIA_DATA_TYPE_INSTANCE_SCRIPT,
                    criteria_id,
                    map->GetId());
                return false;
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_BG_TEAMS_SCORES:
        {
            ...
            return source->GetTeamId() == winnerTeam && winnnerScore == teams_scores.winner_score &&
...
{
    for (CompletedAchievementMap::const_iterator iter = _completedAchievements.begin();
...
void AchievementMgr::ResetAchievementCriteria(
    AchievementCriteriaCondition condition, uint32 value, bool evenIfCriteriaComplete)
{
    ...
    for (AchievementCriteriaEntryList::const_iterator i = achievementCriteriaList->begin();
         ...
         ++i)
    {
        ...
        if ((IsCompletedCriteria(achievementCriteria, achievement) && !evenIfCriteriaComplete) ||
...
void AchievementMgr::SaveToDB(CharacterDatabaseTransaction trans)
{
    ...
    {
        for (CompletedAchievementMap::iterator iter = _completedAchievements.begin();
             iter != _completedAchievements.end();
             ++iter)
        {
            ...
            CharacterDatabasePreparedStatement* stmt =
                CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_ACHIEVEMENT_BY_ACHIEVEMENT);
    ...
    if (!_criteriaProgress.empty())
    {
        for (CriteriaProgressMap::iterator iter = _criteriaProgress.begin(); iter != _criteriaProgress.end(); ++iter)
        {
            ...
            CharacterDatabasePreparedStatement* stmt =
                CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_ACHIEVEMENT_PROGRESS_BY_CRITERIA);
            stmt->SetData(0, GetPlayer()->GetGUID().GetCounter());
            stmt->SetData(1, iter->first);
            trans->Append(stmt);
...
void AchievementMgr::LoadFromDB(
    PreparedQueryResult achievementResult, PreparedQueryResult criteriaResult, PreparedQueryResult offlineUpdatesResult)
{
        ...
        do
        {
            ...
            if (!criteria)
            {
                ...
                LOG_ERROR("achievement",
                    "Non-existing achievement criteria {} data removed from table `character_achievement_progress`.",
                    id);

                CharacterDatabasePreparedStatement* stmt =
                    CharacterDatabase.GetPreparedStatement(CHAR_DEL_INVALID_ACHIEV_PROGRESS_CRITERIA);
...
void AchievementMgr::SendAchievementEarned(AchievementEntry const* achievement) const
{
    ...
    else if (GetPlayer()->IsInWorld())
    {
        ...
        Acore::BroadcastTextBuilder _builder(GetPlayer(),
            ...
            achievement->ID);
        Acore::LocalizedPacketDo<Acore::BroadcastTextBuilder> _localizer(_builder);
        Acore::PlayerDistWorker<Acore::LocalizedPacketDo<Acore::BroadcastTextBuilder>> _worker(
            GetPlayer(), sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_SAY), _localizer);
        TypeContainerVisitor<Acore::PlayerDistWorker<Acore::LocalizedPacketDo<Acore::BroadcastTextBuilder>>,
...
void AchievementMgr::SendCriteriaUpdate(AchievementCriteriaEntry const* entry, CriteriaProgress const* progress,
    uint32 timeElapsed, bool timedCompleted) const
{
    ...
    if (sAchievementMgr->IsAverageCriteria(entry))
        data << uint32(
            GameTime::GetGameTime().count() - GetPlayer()->GetCreationTime().count()); // for average achievements
    else
        data << uint32(timeElapsed); // time elapsed in seconds
...
void AchievementMgr::UpdateAchievementCriteria(
    AchievementCriteriaTypes type, uint32 miscValue1 /*= 0*/, uint32 miscValue2 /*= 0*/, Unit* unit /*= nullptr*/)
{
    // disable for gamemasters with GM-mode enabled
    if (_player->IsGameMaster())
        return;
        ...
        switch (type)
        {
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILL_LEVEL:
                ...
                if (miscValue1 && miscValue1 != achievementCriteria->learn_skill_level.skillID)
                    continue;
                if (uint32 maxSkillvalue =
                        GetPlayer()->GetPureMaxSkillValue(achievementCriteria->learn_skill_level.skillID))
                    SetCriteriaProgress(achievementCriteria, maxSkillvalue);
                break;
            case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_ACHIEVEMENT:
                if ((miscValue1 && achievementCriteria->complete_achievement.linkedAchievement == miscValue1) ||
                    (!miscValue1 &&
                        GetPlayer()->HasAchieved(achievementCriteria->complete_achievement.linkedAchievement)))
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_QUESTS_IN_ZONE:
            {
                ...
                for (RewardedQuestSet::const_iterator itr = rewQuests.begin(); itr != rewQuests.end(); ++itr)
                {
                    ...
                    if (quest && quest->GetZoneOrSort() >= 0 &&
                        uint32(quest->GetZoneOrSort()) == achievementCriteria->complete_quests_in_zone.zoneID &&
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_DEATH_IN_DUNGEON:
            {
                ...
                Map const* map = GetPlayer()->IsInWorld()
                                     ? GetPlayer()->GetMap()
                                     : sMapMgr->FindMap(GetPlayer()->GetMapId(), GetPlayer()->GetInstanceId());
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_EXPLORE_AREA:
            {
                WorldMapOverlayEntry const* worldOverlayEntry =
                    sWorldMapOverlayStore.LookupEntry(achievementCriteria->explore_area.areaReference);
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_GAIN_REPUTATION:
            {
                ...
                int32 reputation =
                    GetPlayer()->GetReputationMgr().GetReputation(achievementCriteria->gain_reputation.factionID);
                if (reputation > 0)
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_LEARN_SKILLLINE_SPELLS:
            {
                     ...
                     ++spellIter)
                {
                    ...
                    for (SkillLineAbilityMap::const_iterator skillIter = bounds.first; skillIter != bounds.second;
                         ++skillIter)
                    {
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_EARN_HONORABLE_KILL:
                SetCriteriaProgress(
                    achievementCriteria, GetPlayer()->GetUInt32Value(PLAYER_FIELD_LIFETIME_HONORABLE_KILLS));
                break;
            case ACHIEVEMENT_CRITERIA_TYPE_HK_CLASS:
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_EARN_ACHIEVEMENT_POINTS:
            {
                ...
                {
                    uint32 points = 0;
                    for (CompletedAchievementMap::iterator itr = _completedAchievements.begin();
...
bool AchievementMgr::IsCompletedCriteria(
    AchievementCriteriaEntry const* achievementCriteria, AchievementEntry const* achievement)
{
...
bool AchievementMgr::IsCompletedAchievement(AchievementEntry const* entry)
{
    ...
    AchievementCriteriaEntryList const* cList =
        sAchievementMgr->GetAchievementCriteriaByAchievement(achievmentForTestId);
    if (!cList)
        return false;
    uint32 count = 0;
...
void AchievementMgr::CompletedAchievement(AchievementEntry const* achievement)
{
    ...
    // pussywizard: set all progress counters to 0, so progress will be deleted from db during save
    {
        ...
        if (allRefsCompleted)
            if (AchievementCriteriaEntryList const* cList =
                    sAchievementMgr->GetAchievementCriteriaByAchievement(achiCheckId))
                for (AchievementCriteriaEntryList::const_iterator itr = cList->begin(); itr != cList->end(); ++itr)
    ...
    if (uint32 titleId =
            reward->titleId[achievement->ID == 1793 ? GetPlayer()->getGender() : uint8(GetPlayer()->GetTeamId())])
        if (CharTitlesEntry const* titleEntry = sCharTitlesStore.LookupEntry(titleId))
            GetPlayer()->SetTitle(titleEntry);
...
{
    WorldPacket data(
        SMSG_ALL_ACHIEVEMENT_DATA, _completedAchievements.size() * 8 + 4 + _criteriaProgress.size() * 38 + 4);
...
void AchievementMgr::SendRespondInspectAchievements(Player* player) const
{
    WorldPacket data(SMSG_RESPOND_INSPECT_ACHIEVEMENTS,
        9 + _completedAchievements.size() * 8 + 4 + _criteriaProgress.size() * 38 + 4);
    data << GetPlayer()->GetPackGUID();
    BuildAllDataPacket(&data);
...
void AchievementMgr::ProcessOfflineUpdatesQueue()
{
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_ACHIEVEMENT_OFFLINE_UPDATES);
    stmt->SetData(0, GetPlayer()->GetGUID().GetCounter());
    CharacterDatabase.Execute(stmt);
}
...
void AchievementMgr::ProcessOfflineUpdate(AchievementOfflinePlayerUpdate const& update)
{
    switch (update.updateType)
    {
        case ACHIEVEMENT_OFFLINE_PLAYER_UPDATE_TYPE_COMPLETE_ACHIEVEMENT:
        {
            ...
            ASSERT(achievement,
                "Not found achievement to complete for offline achievements update. Wrong arg1 ({}) value?",
                update.arg1);
        ...
        default:
            ASSERT(false,
                "Unknown offline achievement update type ({}) for player - {}",
                update.updateType,
    ...
    for (uint32 entryId = 0; entryId < sAchievementCriteriaStore.GetNumRows(); ++entryId)
    {
        ...
        if (!GetAchievement(criteria->referredAchievement))
        {
            LOG_DEBUG("server.loading",
                "Achievement {} referenced by criteria {} doesn't exist, criteria not loaded.",
        ...
        if (criteria->additionalRequirements[0].additionalRequirement_type != ACHIEVEMENT_CRITERIA_CONDITION_NONE)
            _achievementCriteriasByCondition[criteria->additionalRequirements[0].additionalRequirement_type]
        ...
        if (criteria->additionalRequirements[1].additionalRequirement_type != ACHIEVEMENT_CRITERIA_CONDITION_NONE &&
            ...
            _achievementCriteriasByCondition[criteria->additionalRequirements[1].additionalRequirement_type]
        ...
        switch (criteria->requiredType)
        {
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_REACH_SKILL_LEVEL:
                ...
                break;
            case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_ACHIEVEMENT:
                _specialList[criteria->requiredType][criteria->complete_achievement.linkedAchievement].push_back(
                    criteria);
                break;
            case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_QUESTS_IN_ZONE:
            ...
            {
                WorldMapOverlayEntry const* worldOverlayEntry =
                    sWorldMapOverlayStore.LookupEntry(criteria->explore_area.areaReference);
                if (!worldOverlayEntry)
                    break;
...
void AchievementGlobalMgr::LoadAchievementCriteriaData()
{
    ...
    QueryResult result =
        WorldDatabase.Query("SELECT criteria_id, type, value1, value2, ScriptName FROM achievement_criteria_data");
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` has data for non-existing criteria (Entry: {}), ignore.",
                criteria_id);
            continue;
        }
        ...
        if (scriptName.length()) // not empty
        {
            if (dataType != ACHIEVEMENT_CRITERIA_DATA_TYPE_SCRIPT)
                LOG_ERROR("sql.sql",
                    "Table `achievement_criteria_data` has ScriptName set for non-scripted data type (Entry: {}, type {}), useless data.",
                    criteria_id,
                    dataType);
            else
    ...
    for (uint32 entryId = 0; entryId < sAchievementCriteriaStore.GetNumRows(); ++entryId)
    {
        ...
        if (!GetCriteriaDataSet(criteria) &&
            !DisableMgr::IsDisabledFor(DISABLE_TYPE_ACHIEVEMENT_CRITERIA, entryId, nullptr))
            LOG_ERROR("sql.sql",
                "Table `achievement_criteria_data` does not have expected data for criteria (Entry: {} Type: {}) for achievement {}.",
                criteria->ID,
                criteria->requiredType,
                criteria->referredAchievement);
    }
...
void AchievementGlobalMgr::LoadCompletedAchievements()
{
    ...
    do
    {
        ...
        if (!achievement)
        {
            ...
            LOG_ERROR("achievement",
                "Non-existing achievement {} data removed from table `character_achievement`.",
                achievementId);

            CharacterDatabasePreparedStatement* stmt =
                CharacterDatabase.GetPreparedStatement(CHAR_DEL_INVALID_ACHIEVMENT);
...
void AchievementGlobalMgr::LoadRewards()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql",
                "Table `achievement_reward` (Entry: {}) does not have any title or item reward data. Ignoring.",
                entry);
            continue;
        }
        ...
        if (achievement->requiredFaction == ACHIEVEMENT_FACTION_ANY && (!reward.titleId[0] ^ !reward.titleId[1]))
            LOG_DEBUG("achievement",
                "Table `achievement_reward` (Entry: {}) has title (A: {} H: {}) set for only one team.",
                entry,
                reward.titleId[0],
                reward.titleId[1]);
        ...
        if (reward.titleId[0])
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "Table `achievement_reward` (Entry: {}) has invalid title id ({}) in `title_A`. Setting it to 0.",
                    entry,
                    reward.titleId[0]);
                reward.titleId[0] = 0;
            }
        ...
        if (reward.titleId[1])
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "Table `achievement_reward` (Entry: {}) has invalid title id ({}) in `title_H`. Setting it to 0.",
                    entry,
                    reward.titleId[1]);
                reward.titleId[1] = 0;
            }
        ...
        if (reward.sender)
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "Table `achievement_reward` (Entry: {}) has invalid creature_template entry {} as Sender. Will not send the mail reward.",
                    entry,
                    reward.sender);
                reward.sender = 0;
            }
        ...
        else
        {
            if (reward.itemId)
                LOG_ERROR("sql.sql",
                    "Table `achievement_reward` (Entry: {}) has itemId reward set but does not have Sender data set. Item will not be sent.",
                    entry);

            if (!reward.subject.empty())
                LOG_ERROR("sql.sql",
                    "Table `achievement_reward` (Entry: {}) has mail Subject but does not have Sender data set.",
                    entry); // Maybe add "Mail will not be sent." ?

            if (!reward.text.empty())
                LOG_ERROR("sql.sql",
                    "Table `achievement_reward` (Entry: {}) has mail text (Body) set but does not have Sender data set.",
                    entry); // Maybe add "Mail will not be sent." ?

            if (reward.mailTemplate)
                LOG_ERROR("sql.sql",
                    "Table `achievement_reward` (Entry: {}) has mailTemplate set does not have Sender data set.",
                    entry); // Maybe add "Mail will not be sent." ?
        }
        ...
        if (reward.mailTemplate)
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "Table `achievement_reward` (Entry: {}) has invalid mailTemplate ({}) (check the DBC).",
                    entry,
                    reward.mailTemplate);
                ...
                LOG_ERROR("sql.sql",
                    "Table `achievement_reward` (Entry: {}) has mailTemplate ({}) and mail Subject/Body. To use the column mailTemplate, Subject and Body must be empty.",
                    entry,
                    reward.mailTemplate);
        }
        ...
        if (reward.itemId)
        {
            if (!sObjectMgr->GetItemTemplate(reward.itemId))
            {
                ...
                LOG_ERROR("sql.sql",
                    "Table `achievement_reward` (Entry: {}) has invalid item_template id {}. Reward mail will not contain any item.",
                    entry,
                    reward.itemId);
                reward.itemId = 0;
            }
...
void AchievementGlobalMgr::LoadRewardLocales()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql",
                "Table `achievement_reward_locale` (Entry: {}) has locale strings for non-existing achievement reward.",
                ID);
            continue;
        }
...
void AchievementGlobalMgr::CompletedAchievementForOfflinePlayer(
    ObjectGuid::LowType playerLowGuid, AchievementEntry const* entry)
{
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_CHAR_ACHIEVEMENT_OFFLINE_UPDATES);
...
void AchievementGlobalMgr::UpdateAchievementCriteriaForOfflinePlayer(
    ObjectGuid::LowType playerLowGuid, AchievementCriteriaTypes type, uint32 miscValue1, uint32 miscValue2)
{
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_INS_CHAR_ACHIEVEMENT_OFFLINE_UPDATES);
    stmt->SetData(0, playerLowGuid);
