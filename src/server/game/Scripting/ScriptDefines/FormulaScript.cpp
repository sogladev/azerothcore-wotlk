...
{
    CALL_ENABLED_HOOKS(
        FormulaScript, FORMULAHOOK_ON_HONOR_CALCULATION, script->OnHonorCalculation(honor, level, multiplier));
}

void ScriptMgr::OnGrayLevelCalculation(uint8& grayLevel, uint8 playerLevel)
{
    CALL_ENABLED_HOOKS(
        FormulaScript, FORMULAHOOK_ON_GRAY_LEVEL_CALCULATION, script->OnGrayLevelCalculation(grayLevel, playerLevel));
}
...
{
    CALL_ENABLED_HOOKS(
        FormulaScript, FORMULAHOOK_ON_GROUP_RATE_CALCULATION, script->OnGroupRateCalculation(rate, count, isRaid));
}

void ScriptMgr::OnAfterArenaRatingCalculation(Battleground* const bg, int32& winnerMatchmakerChange,
    int32& loserMatchmakerChange, int32& winnerChange, int32& loserChange)
{
    CALL_ENABLED_HOOKS(FormulaScript,
        FORMULAHOOK_ON_AFTER_ARENA_RATING_CALCULATION,
        script->OnAfterArenaRatingCalculation(
            bg, winnerMatchmakerChange, loserMatchmakerChange, winnerChange, loserChange));
}
...
FormulaScript::FormulaScript(char const* name, std::vector<uint16> enabledHooks) : ScriptObject(name, FORMULAHOOK_END)
{
