...
void ScriptMgr::OnBeforePlayerDurabilityRepair(
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_GOSSIP_SELECT, script->OnGossipSelect(player, menu_id, sender, action));
}

void ScriptMgr::OnGossipSelectCode(Player* player, uint32 menu_id, uint32 sender, uint32 action, char const* code)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_BATTLEGROUND_DESERTION, script->OnBattlegroundDesertion(player, desertionType));
}
...
void ScriptMgr::OnCalculateTalentsPoints(Player const* player, uint32& talentPointsForLevel)
{
    CALL_ENABLED_HOOKS(PlayerScript,
        ...
        script->OnCalculateTalentsPoints(player, talentPointsForLevel));
}
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_PLAYER_PVP_FLAG_CHANGE, script->OnPlayerPVPFlagChange(player, state));
}
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_CREATURE_KILLED_BY_PET, script->OnCreatureKilledByPet(petOwner, killed));
}

void ScriptMgr::OnPlayerKilledByCreature(Creature* killer, Player* killed)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_PLAYER_KILLED_BY_CREATURE, script->OnPlayerKilledByCreature(killer, killed));
}
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_FREE_TALENT_POINTS_CHANGED, script->OnFreeTalentPointsChanged(player, points));
}
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_AFTER_SPEC_SLOT_CHANGED, script->OnAfterSpecSlotChanged(player, newSlot));
}
...
void ScriptMgr::OnPlayerReputationRankChange(
    Player* player, uint32 factionID, ReputationRank newRank, ReputationRank oldRank, bool increased)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_BEFORE_SEND_CHAT_MESSAGE, script->OnBeforeSendChatMessage(player, type, lang, msg));
}

void ScriptMgr::OnPlayerChat(Player* player, uint32 type, uint32 lang, std::string& msg, Player* receiver)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_CHAT_WITH_RECEIVER, script->OnChat(player, type, lang, msg, receiver));
}
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_BIND_TO_INSTANCE, script->OnBindToInstance(player, difficulty, mapid, permanent));
}
...
bool ScriptMgr::OnBeforePlayerTeleport(
    Player* player, uint32 mapid, float x, float y, float z, float orientation, uint32 options, Unit* target)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_QUEUE_RANDOM_DUNGEON, script->OnQueueRandomDungeon(player, rDungeonId));
}

void ScriptMgr::OnPlayerRemoveFromBattleground(Player* player, Battleground* bg)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_REMOVE_FROM_BATTLEGROUND, script->OnRemoveFromBattleground(player, bg));
}

bool ScriptMgr::OnBeforeAchievementComplete(Player* player, AchievementEntry const* achievement)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_ON_BEFORE_ACHI_COMPLETE, !script->OnBeforeAchiComplete(player, achievement));
}
...
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_ON_BEFORE_CRITERIA_PROGRESS, !script->OnBeforeCriteriaProgress(player, criteria));
}
...
void ScriptMgr::OnAchievementSave(
    CharacterDatabaseTransaction trans, Player* player, uint16 achiId, CompletedAchievementData achiData)
{
    CALL_ENABLED_HOOKS(PlayerScript, PLAYERHOOK_ON_ACHI_SAVE, script->OnAchiSave(trans, player, achiId, achiData));
}

void ScriptMgr::OnCriteriaSave(
    CharacterDatabaseTransaction trans, Player* player, uint16 critId, CriteriaProgress criteriaData)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_CRITERIA_SAVE, script->OnCriteriaSave(trans, player, critId, criteriaData));
}

void ScriptMgr::OnPlayerBeingCharmed(Player* player, Unit* charmer, uint32 oldFactionId, uint32 newFactionId)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_BEING_CHARMED, script->OnBeingCharmed(player, charmer, oldFactionId, newFactionId));
}

void ScriptMgr::OnAfterPlayerSetVisibleItemSlot(Player* player, uint8 slot, Item* item)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_AFTER_SET_VISIBLE_ITEM_SLOT, script->OnAfterSetVisibleItemSlot(player, slot, item));
}
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_BEFORE_FILL_QUEST_LOOT_ITEM, script->OnBeforeFillQuestLootItem(player, item));
}
...
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_CAN_PLACE_AUCTION_BID, !script->CanPlaceAuctionBid(player, auction));
}
...
bool ScriptMgr::CanJoinInBattlegroundQueue(Player* player, ObjectGuid BattlemasterGuid, BattlegroundTypeId BGTypeID,
...
{
    CALL_ENABLED_BOOLEAN_HOOKS_WITH_DEFAULT_FALSE(PlayerScript,
...
void ScriptMgr::OnBeforeGuardianInitStatsForLevel(
    Player* player, Guardian* guardian, CreatureTemplate const* cinfo, PetType& petType)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
void ScriptMgr::OnBeforeLoadPetFromDB(
    Player* player, uint32& petentry, uint32& petnumber, bool& current, bool& forceLoadFromDB)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
void ScriptMgr::OnBeforeBuyItemFromVendor(
    Player* player, ObjectGuid vendorguid, uint32 vendorslot, uint32& item, uint8 count, uint8 bag, uint8 slot)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
void ScriptMgr::OnAfterStoreOrEquipNewItem(Player* player, uint32 vendorslot, Item* item, uint8 count, uint8 bag,
    uint8 slot, ItemTemplate const* pProto, Creature* pVendor, VendorItem const* crItem, bool bStore)
{
    CALL_ENABLED_HOOKS(PlayerScript,
        PLAYERHOOK_ON_AFTER_STORE_OR_EQUIP_NEW_ITEM,
        script->OnAfterStoreOrEquipNewItem(
            player, vendorslot, item, count, bag, slot, pProto, pVendor, crItem, bStore));
}

void ScriptMgr::OnAfterUpdateMaxPower(Player* player, Powers& power, float& value)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_AFTER_UPDATE_MAX_POWER, script->OnAfterUpdateMaxPower(player, power, value));
}

void ScriptMgr::OnAfterUpdateMaxHealth(Player* player, float& value)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_AFTER_UPDATE_MAX_HEALTH, script->OnAfterUpdateMaxHealth(player, value));
}
...
void ScriptMgr::OnAfterUpdateAttackPowerAndDamage(
    Player* player, float& level, float& base_attPower, float& attPowerMod, float& attPowerMultiplier, bool ranged)
{
    CALL_ENABLED_HOOKS(PlayerScript,
        PLAYERHOOK_ON_AFTER_UPDATE_ATTACK_POWER_AND_DAMAGE,
        script->OnAfterUpdateAttackPowerAndDamage(
            player, level, base_attPower, attPowerMod, attPowerMultiplier, ranged));
}
...
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_ON_BEFORE_QUEST_COMPLETE, !script->OnBeforeQuestComplete(player, quest_id));
}
...
void ScriptMgr::OnBeforeStoreOrEquipNewItem(Player* player, uint32 vendorslot, uint32& item, uint8 count, uint8 bag,
    uint8 slot, ItemTemplate const* pProto, Creature* pVendor, VendorItem const* crItem, bool bStore)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
bool ScriptMgr::CanJoinInArenaQueue(Player* player, ObjectGuid BattlemasterGuid, uint8 arenaslot,
    BattlegroundTypeId BGTypeID, uint8 joinAsGroup, uint8 IsRated, GroupJoinBattlegroundResult& err)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
...
bool ScriptMgr::CanSendMail(Player* player, ObjectGuid receiverGuid, ObjectGuid mailbox, std::string& subject,
...
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_CAN_SEND_ERROR_ALREADY_LOOTED, !script->CanSendErrorAlreadyLooted(player));
}
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_PETITION_BUY, script->PetitionBuy(player, creature, charterid, cost, type));
}

void ScriptMgr::PetitionShowList(
    Player* player, Creature* creature, uint32& CharterEntry, uint32& CharterDispayID, uint32& CharterCost)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_GET_MAX_SKILL_VALUE, script->OnGetMaxSkillValue(player, skill, result, IsPure));
}

bool ScriptMgr::OnPlayerHasActivePowerType(Player const* player, Powers power)
{
    CALL_ENABLED_BOOLEAN_HOOKS_WITH_DEFAULT_FALSE(
        PlayerScript, PLAYERHOOK_ON_PLAYER_HAS_ACTIVE_POWER_TYPE, script->OnPlayerHasActivePowerType(player, power));
}

void ScriptMgr::OnUpdateGatheringSkill(
    Player* player, uint32 skillId, uint32 currentLevel, uint32 gray, uint32 green, uint32 yellow, uint32& gain)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
void ScriptMgr::OnUpdateCraftingSkill(
    Player* player, SkillLineAbilityEntry const* skill, uint32 currentLevel, uint32& gain)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_CAN_AREA_EXPLORE_AND_OUTDOOR, !script->CanAreaExploreAndOutdoor(player));
}
...
void ScriptMgr::OnVictimRewardAfter(
    Player* player, Player* victim, uint32& killer_title, uint32& victim_rank, float& honor_f)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
void ScriptMgr::OnCustomScalingStatValueBefore(
...
void ScriptMgr::OnCustomScalingStatValue(Player* player, ItemTemplate const* proto, uint32& statType, int32& val,
    uint8 itemProtoStatNumber, uint32 ScalingStatValue, ScalingStatValuesEntry const* ssv)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
void ScriptMgr::OnApplyItemModsBefore(
    Player* player, uint8 slot, bool apply, uint8 itemProtoStatNumber, uint32 statType, int32& val)
{
    CALL_ENABLED_HOOKS(PlayerScript,
        PLAYERHOOK_ON_APPLY_ITEM_MODS_BEFORE,
        script->OnApplyItemModsBefore(player, slot, apply, itemProtoStatNumber, statType, val));
}

void ScriptMgr::OnApplyEnchantmentItemModsBefore(
    Player* player, Item* item, EnchantmentSlot slot, bool apply, uint32 enchant_spell_id, uint32& enchant_amount)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
void ScriptMgr::OnApplyWeaponDamage(
    Player* player, uint8 slot, ItemTemplate const* proto, float& minDamage, float& maxDamage, uint8 damageIndex)
{
    CALL_ENABLED_HOOKS(PlayerScript,
        PLAYERHOOK_ON_APPLY_WEAPON_DAMAGE,
        script->OnApplyWeaponDamage(player, slot, proto, minDamage, maxDamage, damageIndex));
}

bool ScriptMgr::CanArmorDamageModifier(Player* player)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_CAN_ARMOR_DAMAGE_MODIFIER, !script->CanArmorDamageModifier(player));
}

void ScriptMgr::OnGetFeralApBonus(
    Player* player, int32& feral_bonus, int32 dpsMod, ItemTemplate const* proto, ScalingStatValuesEntry const* ssv)
{
    CALL_ENABLED_HOOKS(PlayerScript,
...
bool ScriptMgr::CanApplyWeaponDependentAuraDamageMod(
    Player* player, Item* item, WeaponAttackType attackType, AuraEffect const* aura, bool apply)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
...
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_CAN_APPLY_EQUIP_SPELLS_ITEM_SET, !script->CanApplyEquipSpellsItemSet(player, eff));
}

bool ScriptMgr::CanCastItemCombatSpell(Player* player, Unit* target, WeaponAttackType attType, uint32 procVictim,
    uint32 procEx, Item* item, ItemTemplate const* proto)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
...
bool ScriptMgr::CanCastItemUseSpell(
    Player* player, Item* item, SpellCastTargets const& targets, uint8 cast_count, uint32 glyphIndex)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_APPLY_AMMO_BONUSES, script->OnApplyAmmoBonuses(player, proto, currentAmmoDPS));
}

bool ScriptMgr::CanEquipItem(Player* player, uint8 slot, uint16& dest, Item* pItem, bool swap, bool not_loading)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_CAN_EQUIP_ITEM, !script->CanEquipItem(player, slot, dest, pItem, swap, not_loading));
}
...
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_CAN_SAVE_EQUIP_NEW_ITEM, !script->CanSaveEquipNewItem(player, item, pos, update));
}

bool ScriptMgr::CanApplyEnchantment(
    Player* player, Item* item, EnchantmentSlot slot, bool apply, bool apply_dur, bool ignore_condition)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
...
bool ScriptMgr::PassedQuestKilledMonsterCredit(
    Player* player, Quest const* qinfo, uint32 entry, uint32 real_entry, ObjectGuid guid)
{
    CALL_ENABLED_BOOLEAN_HOOKS(PlayerScript,
...
bool ScriptMgr::NotAvoidSatisfy(
    Player* player, DungeonProgressionRequirements const* ar, uint32 target_map, bool report)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_NOT_AVOID_SATISFY, !script->NotAvoidSatisfy(player, ar, target_map, report));
}

bool ScriptMgr::NotVisibleGloballyFor(Player* player, Player const* u)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_NOT_VISIBLE_GLOBALLY_FOR, !script->NotVisibleGloballyFor(player, u));
}

void ScriptMgr::OnGetArenaPersonalRating(Player* player, uint8 slot, uint32& result)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_GET_ARENA_PERSONAL_RATING, script->OnGetArenaPersonalRating(player, slot, result));
}
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_GET_MAX_SKILL_VALUE_FOR_LEVEL, script->OnGetMaxSkillValueForLevel(player, result));
}
...
bool ScriptMgr::CanJoinLfg(Player* player, uint8 roles, lfg::LfgDungeonSet& dungeons, std::string const& comment)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_CAN_JOIN_LFG, !script->CanJoinLfg(player, roles, dungeons, comment));
}

bool ScriptMgr::CanEnterMap(Player* player, MapEntry const* entry, InstanceTemplate const* instance,
    MapDifficulty const* mapDiff, bool loginCheck)
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_CAN_ENTER_MAP, !script->CanEnterMap(player, entry, instance, mapDiff, loginCheck));
}
...
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_CAN_SET_TRADE_ITEM, !script->CanSetTradeItem(player, tradedItem, tradeSlot));
}

void ScriptMgr::OnSetServerSideVisibility(Player* player, ServerSideVisibilityType& type, AccountTypes& sec)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ON_SET_SERVER_SIDE_VISIBILITY, script->OnSetServerSideVisibility(player, type, sec));
}
...
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_CAN_PLAYER_USE_CHAT, !script->CanPlayerUseChat(player, type, language, msg));
}
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ANTICHEAT_SET_CAN_FLY_BY_SERVER, script->AnticheatSetCanFlybyServer(player, apply));
}

void ScriptMgr::AnticheatSetUnderACKmount(Player* player)
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ANTICHEAT_SET_UNDER_ACK_MOUNT, script->AnticheatSetUnderACKmount(player));
}
...
{
    CALL_ENABLED_HOOKS(
        PlayerScript, PLAYERHOOK_ANTICHEAT_SET_JUMPING_BY_OPCODE, script->AnticheatSetJumpingbyOpcode(player, jump));
}
...
{
    CALL_ENABLED_BOOLEAN_HOOKS(
        PlayerScript, PLAYERHOOK_ANTICHEAT_HANDLE_DOUBLE_JUMP, !script->AnticheatHandleDoubleJump(player, mover));
}
...
PlayerScript::PlayerScript(char const* name, std::vector<uint16> enabledHooks) : ScriptObject(name, PLAYERHOOK_END)
{
