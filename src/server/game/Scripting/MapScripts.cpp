...
void Map::ScriptsStart(ScriptMapMap const& scripts, uint32 id, Object* source, Object* target)
{
    ...
    for (ScriptMap::const_iterator iter = s2->begin(); iter != s2->end(); ++iter)
    {
        ...
        m_scriptSchedule.insert(
            ScriptScheduleMap::value_type(time_t(GameTime::GetGameTime().count() + iter->first), sa));
        if (iter->first == 0)
            immedScript = true;
    ...
    else
    {
        ...
        if (!player)
            LOG_ERROR("maps.script",
                "{} neither source nor target object is player (source: TypeId: {}, Entry: {}, GUID: {}; target: TypeId: {}, Entry: {}, GUID: {}), skipping.",
...
inline Creature* Map::_GetScriptCreatureSourceOrTarget(
    Object* source, Object* target, ScriptInfo const* scriptInfo, bool bReverse) const
{
    ...
    else
    {
        ...
        if (!creature)
            LOG_ERROR("maps.script",
                "{} neither source nor target are creatures (source: TypeId: {}, Entry: {}, GUID: {}; target: TypeId: {}, Entry: {}, GUID: {}), skipping.",
                scriptInfo->GetDebugInfo(),
                source ? source->GetTypeId() : 0,
                source ? source->GetEntry() : 0,
                source ? source->GetGUID().ToString() : "",
...
inline Unit* Map::_GetScriptUnit(Object* obj, bool isSource, ScriptInfo const* scriptInfo) const
{
    ...
    if (!obj)
        LOG_ERROR(
            "maps.script", "{} {} object is nullptr.", scriptInfo->GetDebugInfo(), isSource ? "source" : "target");
    else if (!obj->IsUnit())
        LOG_ERROR("maps.script",
            "{} {} object is not unit (TypeId: {}, Entry: {}, GUID: {}), skipping.",
            scriptInfo->GetDebugInfo(),
...
inline void Map::_ScriptProcessDoor(Object* source, Object* target, ScriptInfo const* scriptInfo) const
{
    ...
    else
    {
        ...
        if (!wSource)
            LOG_ERROR("maps.script",
                "{} source object could not be casted to world object ({}), skipping.",
                scriptInfo->GetDebugInfo(),
                source->GetGUID().ToString());
        else
        {
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= GameTime::GetGameTime().count()))
    {
        ...
        switch (step.script->command)
        {
                ...
                if (source)
                {
                    ...
                    switch (step.script->Talk.ChatType)
                    {
                        ...
                        case CHAT_TYPE_BOSS_EMOTE:
                            sourceUnit->TextEmote(
                                step.script->Talk.TextID, target, step.script->Talk.ChatType == CHAT_TYPE_BOSS_EMOTE);
                            break;
                        ...
                        case CHAT_MSG_RAID_BOSS_WHISPER:
                        {
                            ...
                            if (!receiver)
                                LOG_ERROR("scripts",
                                    "{} attempt to whisper to non-player unit, skipping.",
                                    step.script->GetDebugInfo());
                            else
            ...
            case SCRIPT_COMMAND_FIELD_SET:
                ...
                if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
                {
                    ...
                    if (step.script->FieldSet.FieldID <= OBJECT_FIELD_ENTRY ||
                        ...
                        LOG_ERROR("maps.script",
                            "{} wrong field {} (max count: {}) in object ({}) specified, skipping.",
                            step.script->GetDebugInfo(),
            ...
            case SCRIPT_COMMAND_MOVE_TO:
                ...
                if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
                {
                    ...
                    if (step.script->MoveTo.TravelTime != 0)
                    {
                        float speed =
                            unit->GetDistance(
                                step.script->MoveTo.DestX, step.script->MoveTo.DestY, step.script->MoveTo.DestZ) /
                            ((float)step.script->MoveTo.TravelTime * 0.001f);
                        unit->MonsterMoveWithSpeed(
                            step.script->MoveTo.DestX, step.script->MoveTo.DestY, step.script->MoveTo.DestZ, speed);
                    }
                    else
            ...
            case SCRIPT_COMMAND_QUEST_EXPLORED:
            {
                ...
                if ((!worldObject->IsCreature() || ((Unit*)worldObject)->IsAlive()) &&
                    (step.script->QuestExplored.Distance == 0 ||
                        worldObject->IsWithinDistInMap(player, float(step.script->QuestExplored.Distance))))
            ...
            case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT:
                ...
                {
                    LOG_ERROR(
                        "maps.script", "{} gameobject guid (datalong) is not specified.", step.script->GetDebugInfo());
                    break;
                }
            ...
            case SCRIPT_COMMAND_TEMP_SUMMON_CREATURE:
            {
                ...
                if (WorldObject* pSummoner = _GetScriptWorldObject(source, true, step.script))
                {
                    ...
                    else
                    {
                        ...
                        if (!pSummoner->SummonCreature(entry,
                                x,
                                y,
                                z,
                                o,
            ...
            case SCRIPT_COMMAND_CAST_SPELL:
            {
                ...
                switch (step.script->CastSpell.Flags)
                {
                    ...
                    case SF_CASTSPELL_SEARCH_CREATURE: // source -> creature with entry
                        uSource = source ? source->ToUnit() : nullptr;
                        uTarget = uSource ? GetClosestCreatureWithEntry(uSource,
            ...
            case SCRIPT_COMMAND_CREATE_ITEM:
                ...
                if (Player* pReceiver = _GetScriptPlayerSourceOrTarget(source, target, step.script))
                {
                    ...
                    InventoryResult msg = pReceiver->CanStoreNewItem(
                        NULL_BAG, NULL_SLOT, dest, step.script->CreateItem.ItemEntry, step.script->CreateItem.Amount);
                    if (msg == EQUIP_ERR_OK)
                    {
                ...
                if (Unit* unit = _GetScriptUnit(source, true, step.script))
                {
                    if (!sWaypointMgr->GetPath(step.script->LoadPath.PathID))
                        ...
                        unit->GetMotionMaster()->MovePath(
                            step.script->LoadPath.PathID, step.script->LoadPath.IsRepeatable);
                }
                break;
            ...
            case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
            {
                ...
                {
                    LOG_ERROR(
                        "maps.script", "{} creature entry is not specified, skipping.", step.script->GetDebugInfo());
                    break;
                }
                ...
                if (creatureBounds.first != creatureBounds.second)
                {
                    ...
                    auto creatureItr = std::find_if(creatureBounds.first,
                        creatureBounds.second,
                        [](Map::CreatureBySpawnIdContainer::value_type const& pair) { return pair.second->IsAlive(); });
