...
SpellEffectInfo::SpellEffectInfo(SpellEntry const* spellEntry, SpellInfo const* spellInfo, uint8 effIndex)
{
    ...
    RadiusEntry = spellEntry->EffectRadiusIndex[effIndex]
                      ? sSpellRadiusStore.LookupEntry(spellEntry->EffectRadiusIndex[effIndex])
                      : nullptr;
...
int32 SpellEffectInfo::CalcValue(Unit const* caster, int32 const* bp, Unit const* /*target*/) const
{
    ...
    if (caster)
    {
            ...
            !basePointsPerLevel && _spellInfo->HasAttribute(SPELL_ATTR0_SCALES_WITH_CREATURE_LEVEL))
        {
            ...
            if ((sSpellMgr->GetSpellInfo(_spellInfo->Effects[_effIndex].TriggerSpell) &&
                    sSpellMgr->GetSpellInfo(_spellInfo->Effects[_effIndex].TriggerSpell)
                        ->HasAttribute(SPELL_ATTR0_SCALES_WITH_CREATURE_LEVEL)) &&
            ...
            if (canEffectScale)
            {
                ...
                CreatureBaseStats const* pCBS =
                    sObjectMgr->GetCreatureBaseStats(caster->GetLevel(), caster->getClass());
                float CBSPowerCreature = pCBS->BaseDamage[cInfo->expansion];
                CreatureBaseStats const* spellCBS =
                    sObjectMgr->GetCreatureBaseStats(_spellInfo->SpellLevel, caster->getClass());
...
uint32 SpellEffectInfo::GetMissingTargetMask(bool srcSet /*= false*/, bool dstSet /*= false*/, uint32 mask /*=0*/) const
{
    ...
    uint32 providedTargetMask =
        GetTargetFlagMask(TargetA.GetObjectType()) | GetTargetFlagMask(TargetB.GetObjectType()) | mask;
...
std::array<SpellEffectInfo::StaticData, TOTAL_SPELL_EFFECTS> SpellEffectInfo::_data = {
    {
     // implicit target type           used target object type
        ...
        {EFFECT_IMPLICIT_TARGET_EXPLICIT,
            TARGET_OBJECT_TYPE_UNIT_AND_DEST},                      // 43 SPELL_EFFECT_TELEPORT_UNITS_FACE_CASTER
...
SpellInfo::SpellInfo(SpellEntry const* spellEntry)
{
    ...
    CastTimeEntry =
        spellEntry->CastingTimeIndex ? sSpellCastTimesStore.LookupEntry(spellEntry->CastingTimeIndex) : nullptr;
...
bool SpellInfo::IsAffectingArea() const
{
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (Effects[i].IsEffect() &&
            (Effects[i].IsTargetingArea() || Effects[i].IsEffect(SPELL_EFFECT_PERSISTENT_AREA_AURA) ||
...
bool SpellInfo::NeedsToBeTriggeredByCaster(SpellInfo const* triggeringSpell, uint8 effIndex) const
{
    ...
    if (effIndex < MAX_SPELL_EFFECTS &&
        (triggeringSpell->Effects[effIndex].TargetA.GetCheckType() == TARGET_CHECK_ENTRY ||
            triggeringSpell->Effects[effIndex].TargetB.GetCheckType() == TARGET_CHECK_ENTRY))
    {
        // xinef:
        if (Id == 60563)
            return true;
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            if (Effects[i].IsEffect() && (Effects[i].TargetA.GetCheckType() == TARGET_CHECK_ENTRY ||
...
bool SpellInfo::IsCooldownStartedOnEvent() const
{
    return HasAttribute(SPELL_ATTR0_COOLDOWN_ON_EVENT) ||
           (CategoryEntry && CategoryEntry->Flags & SPELL_CATEGORY_FLAG_COOLDOWN_STARTS_ON_EVENT);
}
...
bool SpellInfo::IsRangedWeaponSpell() const
{
    return (SpellFamilyName == SPELLFAMILY_HUNTER &&
               !(SpellFamilyFlags[1] & 0x10000000)) // for 53352, cannot find better way
...
SpellCastResult SpellInfo::CheckLocation(
    uint32 map_id, uint32 zone_id, uint32 area_id, Player* player /*= nullptr*/, bool strict /*= true*/) const
{
    // normal case
    if (AreaGroupId > 0)
    {
    ...
    if (HasAttribute(SPELL_ATTR4_ONLY_FLYING_AREAS) && (area_id || zone_id))
    {
        ...
        if (!areaEntry || !areaEntry->IsFlyable() || (strict && (areaEntry->flags & AREA_FLAG_NO_FLY_ZONE) != 0) ||
    ...
    switch (Id)
    {
        ...
        case 44535: // Spirit Heal (mana)
        {
            ...
            return zone_id == 4197 || (mapEntry->IsBattleground() && player && player->InBattleground())
                       ? SPELL_CAST_OK
        ...
        case 35775: // Green Team (Horde)
        {
            ...
            return mapEntry->IsBattleArena() && player && player->InBattleground() ? SPELL_CAST_OK
                                                                                   : SPELL_FAILED_REQUIRES_AREA;
        }
    }
        ...
        for (Unit::AuraEffectList::const_iterator iter = auraList.begin(); iter != auraList.end(); ++iter)
        {
            ...
            if (sFlag >= SPELL_GROUP_SPECIAL_FLAG_PRIORITY1 && sFlag <= SPELL_GROUP_SPECIAL_FLAG_PRIORITY4)
            {
                ...
                if (sFlagCurr >= SPELL_GROUP_SPECIAL_FLAG_PRIORITY1 &&
                    sFlagCurr <= SPELL_GROUP_SPECIAL_FLAG_PRIORITY4 && sFlagCurr < sFlag)
                {
                    return true;
                }
            }
...
bool SpellInfo::IsAuraEffectEqual(SpellInfo const* otherSpellInfo) const
{
    ...
    for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        for (uint8 j = EFFECT_0; j < MAX_SPELL_EFFECTS; ++j)
        {
            ...
            if (Effects[i].ApplyAuraName != otherSpellInfo->Effects[j].ApplyAuraName ||
...
bool SpellInfo::ValidateAttribute6SpellDamageMods(Unit const* caster, AuraEffect const* auraEffect, bool isDot) const
{
    ...
    if (Id == 70890 && auraEffect)
    {
        ...
        return auraInfo->SpellIconID == 3086 || (auraInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT &&
    ...
    return !isDot && auraEffect &&
           (auraEffect->GetAmount() < 0 || (auraEffect->GetCasterGUID() == caster->GetGUID() &&
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, WorldObject const* target, bool implicit) const
{
    ...
    if (unitTarget)
    {
        ...
        if (((IsRequiringDeadTarget() != 0) != unitTarget->HasAuraType(SPELL_AURA_GHOST)) &&
            ...
            if (caster->IsPlayer())
            {
                ...
                if (HasAttribute(SPELL_ATTR0_CU_PICKPOCKET))
                {
                    ...
                    if (!LootTemplates_Pickpocketing.HaveLootFor(
                            targetCreature->GetCreatureTemplate()->pickpocketLootId))
    ...
    if (ExcludeTargetAuraSpell &&
        unitTarget->HasAura(sSpellMgr->GetSpellIdForDifficulty(ExcludeTargetAuraSpell, caster)))
        return SPELL_FAILED_TARGET_AURASTATE;
...
SpellCastResult SpellInfo::CheckExplicitTarget(
    Unit const* caster, WorldObject const* target, Item const* itemTarget) const
{
    ...
    {
        if (neededTargets & (TARGET_FLAG_UNIT_ENEMY | TARGET_FLAG_UNIT_ALLY | TARGET_FLAG_UNIT_RAID |
                                TARGET_FLAG_UNIT_PARTY | TARGET_FLAG_UNIT_MINIPET | TARGET_FLAG_UNIT_PASSENGER))
        {
...
SpellSpecificType SpellInfo::LoadSpellSpecific() const
{
    switch (SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (SpellFamilyFlags[1] & 0x26000C00 || SpellFamilyFlags[0] & 0x0A000000)
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            if (SpellFamilyFlags[1] & 0x420 ||
                (SpellFamilyFlags[0] & 0x00000400 && HasAttribute(SPELL_ATTR1_NO_THREAT)) || Id == 23552)
...
int32 SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask, Spell* spell) const
{
    ...
    powerCost = int32(
        powerCost * (1.0f + caster->GetFloatValue(static_cast<uint16>(UNIT_FIELD_POWER_COST_MULTIPLIER) + school)));
    if (powerCost < 0)
        powerCost = 0;
    return powerCost;
}
...
SpellInfo const* SpellInfo::GetAuraRankForLevel(uint8 level) const
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (IsPositiveEffect(i) &&
            (Effects[i].Effect == SPELL_EFFECT_APPLY_AURA || Effects[i].Effect == SPELL_EFFECT_APPLY_AREA_AURA_PARTY ||
