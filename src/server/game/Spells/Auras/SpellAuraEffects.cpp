...
void AuraEffect::CalculatePeriodicData()
{
    ...
    if (GetBase()->GetType() == UNIT_AURA_TYPE && GetCaster())
    {
        if (m_spellInfo->HasAura(SPELL_AURA_PERIODIC_HEAL))
        {
            ...
            // Keep healing done percent dynamic for HoTs (e.g. Life Burst) instead of snapshotting it.
            float healingDonePctMod = GetCaster()->GetTotalAuraMultiplier(SPELL_AURA_MOD_HEALING_DONE_PERCENT);
            if (healingDonePctMod > 0.0f)
                m_pctMods /= healingDonePctMod;
        }
