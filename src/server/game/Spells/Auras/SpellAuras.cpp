    ...
    _disableMask(0)
{
    ...
    if (GetBase()->CanBeSentToClient())
    {
        ...
        if (AuraApplication* foundAura = GetTarget()->GetAuraApplication(
                GetBase()->GetId(), GetBase()->GetCasterGUID(), GetBase()->GetCastItemGUID()))
        {
...
void AuraApplication::_Remove()
{
    ...
    if (AuraApplication* foundAura =
            _target->GetAuraApplication(GetBase()->GetId(), GetBase()->GetCasterGUID(), GetBase()->GetCastItemGUID()))
    {
    ...
    if (uint32 groupId = aurEff->GetAuraGroup())
    {
        ...
        if (!aurEff->IsPeriodic() && (sFlag & SPELL_GROUP_STACK_FLAG_EFFECT_EXCLUSIVE))
        {
            ...
            for (Unit::AuraEffectList::const_iterator iter = auraList.begin(); iter != auraList.end(); ++iter)
            {
                ...
                if (aurEff->GetAuraType() != SPELL_AURA_230 /*SPELL_AURA_MOD_INCREASE_HEALTH_2*/ &&
                    aurEff->GetAuraType() != SPELL_AURA_MOD_CASTING_SPEED_NOT_STACK &&
        ...
        if (Player const* plr = GetTarget()->ToPlayer())
            if (Aura* aura = GetBase())
                if (plr->NeedSendSpectatorData() &&
                    ArenaSpectator::ShouldSendAura(aura, GetEffectMask(), GetTarget()->GetGUID(), remove))
                    ArenaSpectator::SendCommand_Aura(plr->FindMap(),
                        plr->GetGUID(),
                        "AUR",
                        aura->GetCasterGUID(),
                        ...
                        aura->GetDuration(),
                        aura->GetMaxDuration(),
                        (aura->GetCharges() > 1 ? aura->GetCharges() : aura->GetStackAmount()),
                        remove);
...
Aura* Aura::TryRefreshStackOrCreate(SpellInfo const* spellproto, uint8 tryEffMask, WorldObject* owner, Unit* caster,
    int32* baseAmount /*= nullptr*/, Item* castItem /*= nullptr*/, ObjectGuid casterGUID /*= ObjectGuid::Empty*/,
    bool* refresh /*= nullptr*/, bool periodicReset /*= false*/)
{
    ASSERT_NODEBUGINFO(spellproto);
    ASSERT_NODEBUGINFO(owner);
    ...
    if (Aura* foundAura = owner->ToUnit()->_TryStackingOrRefreshingExistingAura(
            spellproto, effMask, caster, baseAmount, castItem, casterGUID, periodicReset))
    {
...
Aura* Aura::TryCreate(SpellInfo const* spellproto, uint8 tryEffMask, WorldObject* owner, Unit* caster,
    int32* baseAmount /*= nullptr*/, Item* castItem /*= nullptr*/, ObjectGuid casterGUID /*= ObjectGuid::Empty*/,
...
Aura* Aura::Create(SpellInfo const* spellproto, uint8 effMask, WorldObject* owner, Unit* caster, int32* baseAmount,
    Item* castItem, ObjectGuid casterGUID, ObjectGuid itemGUID /*= ObjectGuid::Empty*/)
{
...
Aura::Aura(SpellInfo const* spellproto, WorldObject* owner, Unit* caster, Item* castItem, ObjectGuid casterGUID,
    ...
    m_applyTime(GameTime::GetGameTime().count()),
    m_owner(owner),
    m_timeCla(0),
            ...
            if (IsArea())
                for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
                    if ((existing->second & (1 << effIndex)) &&
                        existing->first->IsImmunedToSpellEffect(GetSpellInfo(), effIndex))
    ...
    for (std::map<Unit*, uint8>::iterator itr = targets.begin(); itr != targets.end();)
    {
        ...
        if (addUnit)
        {
            ...
            else // (GetType() == UNIT_AURA_TYPE)
            {
                ...
                if (itr->first != GetOwner())
                {
                    ...
                    for (Unit::AuraApplicationMap::iterator iter = itr->first->GetAppliedAuras().begin();
        ...
        else
        {
            ...
            if (!GetOwner()->IsSelfOrInSameMap(itr->first))
            {
                ...
                LOG_FATAL("spells.aura",
                    "Aura {}: Owner {} (map {}) is not in the same map as target {} (map {}).",
                    GetSpellInfo()->Id,
                    GetOwner()->GetName(),
                    GetOwner()->IsInWorld() ? GetOwner()->GetMap()->GetId() : uint32(-1),
                    itr->first->GetName(),
                    itr->first->IsInWorld() ? itr->first->GetMap()->GetId() : uint32(-1));
                ABORT();
            }
...
void Aura::Update(uint32 diff, Unit* caster)
{
    if (m_duration > 0)
    {
        ...
        if (m_timeCla)
        {
            ...
            {
                if (int32 ManaPerSecond =
                        m_spellInfo->ManaPerSecond + m_spellInfo->ManaPerSecondPerLevel * caster->GetLevel())
                {
                    m_timeCla += 1000 - diff;
...
void Aura::SetStackAmount(uint8 stackAmount)
{
    ...
    for (std::list<AuraApplication*>::const_iterator apptItr = applications.begin(); apptItr != applications.end();
         ++apptItr)
...
bool Aura::ModStackAmount(int32 num, AuraRemoveMode removeMode, bool periodicReset /*= false*/)
{
    ...
    bool refresh = stackAmount >= GetStackAmount() &&
                   (m_spellInfo->StackAmount || !m_spellInfo->HasAttribute(SPELL_ATTR1_AURA_UNIQUE));
...
{
    for (Aura::ApplicationMap::const_iterator appIter = m_applications.begin(); appIter != m_applications.end();
         ++appIter)
...
{
    return (GetCasterGUID() == target->GetGUID() && m_spellInfo->Stances &&
...
void Aura::SetLoadedState(
    int32 maxduration, int32 duration, int32 charges, uint8 stackamount, uint8 recalculateMask, int32* amount)
{
...
void Aura::HandleAuraSpecificMods(AuraApplication const* aurApp, Unit* caster, bool apply, bool onReapply)
{
    ...
    if (!onReapply)
    {
        ...
        if (apply)
        {
            ...
            {
                for (std::vector<int32>::const_iterator itr = spellTriggered->begin(); itr != spellTriggered->end();
                     ++itr)
                {
                    if (*itr < 0)
    ...
    if (apply)
    {
        switch (GetSpellInfo()->SpellFamilyName)
        {
                ...
                switch (GetId())
                {
                    ...
                    case 12043: // Presence of Mind
                        ...
                        if (AuraEffect const* aurEff =
                                caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_MAGE, 2120, 0))
                        {
                            uint32 spellId = 0;
                            ...
                            switch (aurEff->GetId())
                            {
                                ...
                                default:
                                    LOG_ERROR("spells.aura",
                                        "Aura::HandleAuraSpecificMods: Unknown rank of Arcane Potency ({}) found",
                                        aurEff->GetId());
                            }
                            if (spellId)
                    ...
                    case 12494: // Frostbite, synchronise with Fingers of Frost
                    {
                        ...
                        if (AuraEffect* aurEff =
                                caster->GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL, SPELLFAMILY_MAGE, 2947, EFFECT_0))
                        {
                            if (SpellInfo const* triggeringSpellInfo = GetTriggeredByAuraSpellInfo())
                            {
                                ...
                                {
                                    caster->CastSpell(
                                        caster, aurEff->GetSpellInfo()->Effects[EFFECT_0].TriggerSpell, true);
                                }
                            }
                        }
                        break;
                ...
                if (GetSpellInfo()->SpellFamilyFlags[0] & 0x10 && GetEffect(0))
                {
                    ...
                    if (caster->GetAuraEffectDummy(64760))
                    {
                        ...
                        caster->CastCustomSpell(
                            target, 64801, &basepoints0, nullptr, nullptr, true, nullptr, GetEffect(0));
                    }
                }
                break;
            case SPELLFAMILY_PRIEST:
                ...
                if (GetSpellInfo()->SpellFamilyFlags[0] & 0x02000000 && GetEffect(0))
                {
                    ...
                    if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, 3790, 1))
                    {
                        ...
                        caster->CastCustomSpell(
                            target, 63675, &basepoints0, nullptr, nullptr, true, nullptr, GetEffect(0));
                        caster->CastCustomSpell(caster, 75999, &heal, nullptr, nullptr, true, nullptr, GetEffect(0));
                    }
                }
                // Power Word: Shield
                else if (m_spellInfo->SpellFamilyFlags[0] & 0x1 && m_spellInfo->SpellFamilyFlags[2] & 0x400 &&
                         GetEffect(0))
                {
            ...
            case SPELLFAMILY_SHAMAN:
            {
                ...
                if (GetSpellInfo()->SpellFamilyFlags[0] & 0x00000800 && target->HasAura(22801) &&
                    target->GetLevel() <= 60)
                {
                    int32 bp0 = 15;
            ...
            case SPELLFAMILY_DEATHKNIGHT:
                ...
                if (GetSpellInfo()->SpellFamilyFlags[2] & 0x2)
                {
                    ...
                    for (Unit::AuraEffectList::const_iterator itr = TalentAuras.begin(); itr != TalentAuras.end();
                         ++itr)
                    {
                    ...
                    if (aurEff)
                    {
                        ...
                        switch (aurEff->GetId())
                        {
                            ...
                            default:
                                LOG_ERROR("spells.aura",
                                    "Aura::HandleAuraSpecificMods: Unknown rank of Crypt Fever/Ebon Plague ({}) found",
                                    aurEff->GetId());
                        }
            ...
            case SPELLFAMILY_POTION:
                // Alchemy: Mixology
                if (caster && caster->HasAura(53042) && caster->IsPlayer() &&
                            ...
                            for (int i = 0; i < MAX_SPELL_EFFECTS; ++i)
                                if (GetEffect(i))
                                    GetEffect(i)->SetAmount(CalculatePct(
                                        GetEffect(i)->GetAmount(), 100 + sSpellMgr->GetSpellMixologyBonus(GetId())));
    ...
    else
    {
        switch (GetSpellInfo()->SpellFamilyName)
        {
                    ...
                    case 72369:
                        if (AuraEffect* aurEff = GetEffect(0))
                        {
                            int32 remainingDamage =
                                aurEff->GetAmount() * (aurEff->GetTotalTicks() - aurEff->GetTickNumber());
                ...
                if (GetSpellInfo()->SpellFamilyFlags[1] & 0x00000400)
                {
                    if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_WARLOCK, 98, 0))
                    {
                        ...
                        switch (aurEff->GetId())
                        {
                            ...
                            default:
                                LOG_ERROR("spells.aura",
                                    "Aura::HandleAuraSpecificMods: Unknown rank of Improved Fear ({}) found",
                                    aurEff->GetId());
                        }
                        if (spellId)
            ...
            case SPELLFAMILY_PRIEST:
                ...
                if (removeMode == AURA_REMOVE_BY_ENEMY_SPELL && (GetSpellInfo()->SpellFamilyFlags[0] & 0x00008000 ||
                                                                    GetSpellInfo()->SpellFamilyFlags[1] & 0x00000400))
                {
                    ...
                    if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, 178, 1))
                    {
                        ...
                        caster->CastCustomSpell(
                            caster, 64103, &basepoints0, nullptr, nullptr, true, nullptr, GetEffect(0));
                    }
                }
                // Power word: shield
...
bool Aura::IsAuraStronger(Aura const* newAura) const
{
    for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        for (uint8 j = EFFECT_0; j < MAX_SPELL_EFFECTS; ++j)
        {
            ...
            if (!newEffect || thisEffect->GetAuraType() != newEffect->GetAuraType() ||
...
bool Aura::CanStackWith(Aura const* existingAura, bool remove) const
{
    ...
    if (IsPassive() && sameCaster &&
        (m_spellInfo->IsDifferentRankOf(existingSpellInfo) ||
            (m_spellInfo->Id == existingSpellInfo->Id && m_castItemGuid.IsEmpty())))
        return false;
    ...
    SpellGroupStackFlags stackFlags =
        sSpellMgr->CheckSpellGroupStackRules(m_spellInfo, existingSpellInfo, remove, IsArea());
    if (stackFlags)
    {
    ...
    if (m_spellInfo->IsRankOf(existingSpellInfo) &&
        !(m_spellInfo->SpellFamilyName == SPELLFAMILY_HUNTER && m_spellInfo->SpellFamilyFlags[1] & 0x80000000))
    {
...
void Aura::PrepareProcToTrigger(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    if (IsUsingCharges() &&
        (!eventInfo.GetSpellInfo() || !eventInfo.GetSpellInfo()->HasAttribute(SPELL_ATTR6_DO_NOT_CONSUME_RESOURCES)))
    {
        --m_procCharges;
        SetNeedClientUpdateForTargets();
    }
...
bool Aura::IsProcTriggeredOnEvent(AuraApplication* aurApp, ProcEventInfo& eventInfo) const
{
    ...
    ConditionList conditions =
        sConditionMgr->GetConditionsForNotGroupedEntry(CONDITION_SOURCE_TYPE_SPELL_PROC, GetId());
            ...
            if (GetSpellInfo()->EquippedItemClass == ITEM_CLASS_WEAPON)
            {
                ...
                if (DamageInfo const* damageInfo = eventInfo.GetDamageInfo())
                {
                    switch (damageInfo->GetAttackType())
                    {
                        case BASE_ATTACK:
                            item =
                                target->ToPlayer()->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND);
                            break;
                        case OFF_ATTACK:
                            item =
                                target->ToPlayer()->GetUseableItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND);
                            break;
                        default:
...
bool Aura::CallScriptCheckAreaTargetHandlers(Unit* target)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::CheckAreaTargetHandler>::iterator hookItrEnd = (*scritr)->DoCheckAreaTarget.end(),
...
void Aura::CallScriptDispel(DispelInfo* dispelInfo)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::AuraDispelHandler>::iterator hookItrEnd = (*scritr)->OnDispel.end(),
...
void Aura::CallScriptAfterDispel(DispelInfo* dispelInfo)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::AuraDispelHandler>::iterator hookItrEnd = (*scritr)->AfterDispel.end(),
...
bool Aura::CallScriptEffectApplyHandlers(
    AuraEffect const* aurEff, AuraApplication const* aurApp, AuraEffectHandleModes mode)
{
    bool preventDefault = false;
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_APPLY, aurApp);
        std::list<AuraScript::EffectApplyHandler>::iterator effEndItr = (*scritr)->OnEffectApply.end(),
...
bool Aura::CallScriptEffectRemoveHandlers(
    AuraEffect const* aurEff, AuraApplication const* aurApp, AuraEffectHandleModes mode)
{
    bool preventDefault = false;
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::EffectApplyHandler>::iterator effEndItr = (*scritr)->OnEffectRemove.end(),
...
void Aura::CallScriptAfterEffectApplyHandlers(
    AuraEffect const* aurEff, AuraApplication const* aurApp, AuraEffectHandleModes mode)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_AFTER_APPLY, aurApp);
        std::list<AuraScript::EffectApplyHandler>::iterator effEndItr = (*scritr)->AfterEffectApply.end(),
    ...
    AuraEffect const* aurEff, AuraApplication const* aurApp, AuraEffectHandleModes mode)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::EffectApplyHandler>::iterator effEndItr = (*scritr)->AfterEffectRemove.end(),
...
bool Aura::CallScriptEffectPeriodicHandlers(AuraEffect const* aurEff, AuraApplication const* aurApp)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::EffectPeriodicHandler>::iterator effEndItr = (*scritr)->OnEffectPeriodic.end(),
...
void Aura::CallScriptEffectUpdatePeriodicHandlers(AuraEffect* aurEff)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::EffectUpdatePeriodicHandler>::iterator effEndItr =
                                                                         (*scritr)->OnEffectUpdatePeriodic.end(),
...
void Aura::CallScriptEffectCalcAmountHandlers(AuraEffect const* aurEff, int32& amount, bool& canBeRecalculated)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::EffectCalcAmountHandler>::iterator effEndItr = (*scritr)->DoEffectCalcAmount.end(),
...
void Aura::CallScriptEffectCalcPeriodicHandlers(AuraEffect const* aurEff, bool& isPeriodic, int32& amplitude)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::EffectCalcPeriodicHandler>::iterator effEndItr = (*scritr)->DoEffectCalcPeriodic.end(),
...
void Aura::CallScriptEffectCalcSpellModHandlers(AuraEffect const* aurEff, SpellModifier*& spellMod)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::EffectCalcSpellModHandler>::iterator effEndItr = (*scritr)->DoEffectCalcSpellMod.end(),
...
void Aura::CallScriptEffectAbsorbHandlers(AuraEffect* aurEff, AuraApplication const* aurApp, DamageInfo& dmgInfo,
    uint32& absorbAmount, bool& defaultPrevented)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_ABSORB, aurApp);
        std::list<AuraScript::EffectAbsorbHandler>::iterator effEndItr = (*scritr)->OnEffectAbsorb.end(),
...
void Aura::CallScriptEffectAfterAbsorbHandlers(
    AuraEffect* aurEff, AuraApplication const* aurApp, DamageInfo& dmgInfo, uint32& absorbAmount)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_AFTER_ABSORB, aurApp);
        std::list<AuraScript::EffectAbsorbHandler>::iterator effEndItr = (*scritr)->AfterEffectAbsorb.end(),
...
void Aura::CallScriptEffectManaShieldHandlers(AuraEffect* aurEff, AuraApplication const* aurApp, DamageInfo& dmgInfo,
    uint32& absorbAmount, bool& /*defaultPrevented*/)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_MANASHIELD, aurApp);
        std::list<AuraScript::EffectManaShieldHandler>::iterator effEndItr = (*scritr)->OnEffectManaShield.end(),
    ...
    AuraEffect* aurEff, AuraApplication const* aurApp, DamageInfo& dmgInfo, uint32& absorbAmount)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::EffectManaShieldHandler>::iterator effEndItr = (*scritr)->AfterEffectManaShield.end(),
...
void Aura::CallScriptEffectSplitHandlers(
    AuraEffect* aurEff, AuraApplication const* aurApp, DamageInfo& dmgInfo, uint32& splitAmount)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_SPLIT, aurApp);
        std::list<AuraScript::EffectSplitHandler>::iterator effEndItr = (*scritr)->OnEffectSplit.end(),
...
bool Aura::CallScriptCheckProcHandlers(AuraApplication const* aurApp, ProcEventInfo& eventInfo)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::CheckProcHandler>::iterator hookItrEnd = (*scritr)->DoCheckProc.end(),
...
bool Aura::CallScriptAfterCheckProcHandlers(
    AuraApplication const* aurApp, ProcEventInfo& eventInfo, bool isTriggeredAtSpellProcEvent)
{
    bool result = isTriggeredAtSpellProcEvent;
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_AFTER_CHECK_PROC, aurApp);
        std::list<AuraScript::AfterCheckProcHandler>::iterator hookItrEnd = (*scritr)->DoAfterCheckProc.end(),
...
bool Aura::CallScriptPrepareProcHandlers(AuraApplication const* aurApp, ProcEventInfo& eventInfo)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::AuraProcHandler>::iterator effEndItr = (*scritr)->DoPrepareProc.end(),
...
bool Aura::CallScriptProcHandlers(AuraApplication const* aurApp, ProcEventInfo& eventInfo)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::AuraProcHandler>::iterator hookItrEnd = (*scritr)->OnProc.end(),
...
void Aura::CallScriptAfterProcHandlers(AuraApplication const* aurApp, ProcEventInfo& eventInfo)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        std::list<AuraScript::AuraProcHandler>::iterator hookItrEnd = (*scritr)->AfterProc.end(),
...
bool Aura::CallScriptEffectProcHandlers(
    AuraEffect const* aurEff, AuraApplication const* aurApp, ProcEventInfo& eventInfo)
{
    bool preventDefault = false;
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_PROC, aurApp);
        std::list<AuraScript::EffectProcHandler>::iterator effEndItr = (*scritr)->OnEffectProc.end(),
...
void Aura::CallScriptAfterEffectProcHandlers(
    AuraEffect const* aurEff, AuraApplication const* aurApp, ProcEventInfo& eventInfo)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_AFTER_PROC, aurApp);
        std::list<AuraScript::EffectProcHandler>::iterator effEndItr = (*scritr)->AfterEffectProc.end(),
...
std::string Aura::GetDebugInfo() const
{
    ...
    sstr << std::boolalpha << "Id: " << GetId() << " Caster: " << GetCasterGUID().ToString()
...
UnitAura::UnitAura(SpellInfo const* spellproto, uint8 effMask, WorldObject* owner, Unit* caster, int32* baseAmount,
    Item* castItem, ObjectGuid casterGUID, ObjectGuid itemGUID /*= ObjectGuid::Empty*/) :
        ...
        else
        {
            ...
            if (!GetUnitOwner()->HasUnitState(UNIT_STATE_ISOLATED))
            {
                switch (GetSpellInfo()->Effects[effIndex].Effect)
                {
                    ...
                    {
                        targetList.push_back(GetUnitOwner());
                        Acore::AnyGroupedUnitInObjectRangeCheck u_check(GetUnitOwner(),
                            GetUnitOwner(),
                            radius,
                            GetSpellInfo()->Effects[effIndex].Effect == SPELL_EFFECT_APPLY_AREA_AURA_RAID);
                        Acore::UnitListSearcher<Acore::AnyGroupedUnitInObjectRangeCheck> searcher(
                    ...
                    case SPELL_EFFECT_APPLY_AREA_AURA_ENEMY:
                    {
                        ...
                        Acore::UnitListSearcher<Acore::AnyAoETargetUnitInObjectRangeCheck> searcher(
                            GetUnitOwner(), targetList, u_check);
                        Cell::VisitAllObjects(GetUnitOwner(), searcher, radius);
                        break;
                    }
...
DynObjAura::DynObjAura(SpellInfo const* spellproto, uint8 effMask, WorldObject* owner, Unit* caster, int32* baseAmount,
    Item* castItem, ObjectGuid casterGUID, ObjectGuid itemGUID /*= ObjectGuid::Empty*/) :
    Aura(spellproto, owner, caster, castItem, casterGUID, itemGUID)
{
    LoadScripts();
...
void DynObjAura::FillTargetMap(std::map<Unit*, uint8>& targets, Unit* /*caster*/)
{
    ...
    for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
    {
        ...
        {
            Acore::AnyAttackableUnitExceptForOriginalCasterInObjectRangeCheck u_check(
                GetDynobjOwner(), dynObjOwnerCaster, radius);
            Acore::UnitListSearcher<Acore::AnyAttackableUnitExceptForOriginalCasterInObjectRangeCheck> searcher(
                GetDynobjOwner(), targetList, u_check);
            Cell::VisitAllObjects(GetDynobjOwner(), searcher, radius);
        }
        else
        ...
        for (UnitList::iterator itr = targetList.begin(); itr != targetList.end(); ++itr)
        {
            ...
            if (!spellInfo->HasAttribute(SPELL_ATTR2_IGNORE_LINE_OF_SIGHT) &&
                ...
                !GetDynobjOwner()->IsWithinLOSInMap(
                    target, VMAP::ModelIgnoreFlags::Nothing, LINEOFSIGHT_ALL_CHECKS, collisionHeight, combatReach))
            {
                continue;
            }
