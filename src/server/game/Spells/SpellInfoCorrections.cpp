...
void SpellMgr::LoadSpellInfoCorrections()
{
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_ALWAYS_HIT; });

    // Scarlet Raven Priest Image
    ApplySpellFix({48763, 48761},
        [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags &= ~AURA_INTERRUPT_FLAG_SPELL_ATTACK; });
        ...
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_100_YARDS); // 100yards instead of 50000?!
    });
        ...
        [](SpellInfo* spellInfo)
    {
        spellInfo->AttributesEx3 |=
            SPELL_ATTR3_CAN_PROC_FROM_PROCS; // Entries were not updated after spell effect change, we have to do that manually
    });
    ...
    // Divine Storm (Damage)
    ApplySpellFix({53385}, [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 4; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_CASTER_MODIFIERS; });

    // Spitfire Totem
    ApplySpellFix({38296}, [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 5; });
    ...
    // Tidal Wave
    ApplySpellFix({53390}, [](SpellInfo* spellInfo) { spellInfo->ProcCharges = 2; });

    // Oscillation Field
    ApplySpellFix({37408}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });

    // Ascendance (Talisman of Ascendance trinket)
    ApplySpellFix({28200}, [](SpellInfo* spellInfo) { spellInfo->ProcCharges = 6; });

    // Crafty's Ultra-Advanced Proto-Typical Shortening Blaster
    ApplySpellFix({51912}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].Amplitude = 3000; });
    ...
    ApplySpellFix({48420}, [](SpellInfo* spellInfo) { spellInfo->Stances = 1 << (FORM_CAT - 1); });

    ApplySpellFix({48421}, [](SpellInfo* spellInfo) { spellInfo->Stances = 1 << (FORM_MOONKIN - 1); });

    ApplySpellFix({48422}, [](SpellInfo* spellInfo) { spellInfo->Stances = 1 << (FORM_TREE - 1); });

    // Elemental Oath
    ApplySpellFix({51466, 51470},
        [](SpellInfo* spellInfo)
    {
        ...
        [](SpellInfo* spellInfo)
    {
        spellInfo->Attributes &=
            ~SPELL_ATTR0_NOT_SHAPESHIFTED; // with this spell atrribute aura can be stacked several times
    });

    // Natural shapeshifter
    ApplySpellFix(
        {16834, 16835}, [](SpellInfo* spellInfo) { spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(21); });

    // Ebon Plague
    ApplySpellFix({51735, 51734, 51726},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx5 |= SPELL_ATTR5_EXTRA_INITIAL_PERIOD; });

    // Marked for Death
    ApplySpellFix({53241, 53243, 53244, 53245, 53246},
        [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].SpellClassMask = flag96(399361, 276955137, 1); });
        ...
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].BasePoints =
            0; // force seat 0, vehicle doesn't have the required seat flags for "no seat specified (-1)"
    });
    ...
    {
        spellInfo->Effects[EFFECT_0].BasePoints =
            100; // 100% chance of procc'ing, not -10% (chance calculated in PrepareTriggersExecutedOnHit)
    });
    ...
    // Hand of Reckoning
    ApplySpellFix({62124}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx |= SPELL_ATTR1_NO_REDIRECTION; });

    // Redemption
    ApplySpellFix({7328, 10322, 10324, 20772, 20773, 48949, 48950},
        [](SpellInfo* spellInfo) { spellInfo->SpellFamilyName = SPELLFAMILY_PALADIN; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx7 |= SPELL_ATTR7_DONT_CAUSE_SPELL_PUSHBACK; });

    // Seal of Command trigger
    ApplySpellFix(
        {20424}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 &= ~SPELL_ATTR3_SUPPRESS_CASTER_PROCS; });

    // Arcane Missiles
    ApplySpellFix({5143, 5144, 5145, 8416, 8417, 10211, 10212, 25345, 27075, 38699, 38704, 42843, 42846},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_TARGET_PROCS; });
    ...
    // Silencing Shot
    ApplySpellFix({34490, 41084, 42671}, [](SpellInfo* spellInfo) { spellInfo->Speed = 0.0f; });
    ...
    // Hunter's Mark
    ApplySpellFix({1130, 14323, 14324, 14325, 53338},
        [](SpellInfo* spellInfo)
    {
        spellInfo->DmgClass = SPELL_DAMAGE_CLASS_MAGIC;
    ...
    ApplySpellFix({34027, 37529}, [](SpellInfo* spellInfo) { spellInfo->ProcCharges = 0; });

    // Kindred Spirits, damage aura
    ApplySpellFix({57458}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx4 |= SPELL_ATTR4_ALLOW_ENETRING_ARENA; });

    // Chimera Shot - Serpent trigger
    ApplySpellFix(
        {53353}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_CASTER_MODIFIERS; });

    // Entrapment trigger
    ApplySpellFix({19185, 64803, 64804},
        [](SpellInfo* spellInfo)
    {
    ...
    // Master of Subtlety
    ApplySpellFix({31221, 31222, 31223}, [](SpellInfo* spellInfo) { spellInfo->SpellFamilyName = SPELLFAMILY_ROGUE; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_SCRIPT_EFFECT; });

    // Honor Among Thieves
    ApplySpellFix(
        {51698, 51700, 51701}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TriggerSpell = 51699; });
    ...
    // Envenom
    ApplySpellFix({32645, 32684, 57992, 57993}, [](SpellInfo* spellInfo) { spellInfo->Dispel = DISPEL_NONE; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].TargetB = SpellImplicitTargetInfo(TARGET_DEST_DB); });
    ...
    // Killing Spree
    ApplySpellFix({51690}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx |= SPELL_ATTR1_ALLOW_WHILE_STEALTHED; });
    ...
    // Chains of Ice
    ApplySpellFix({45524}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_2].Effect = 0; });

    // Impurity
    ApplySpellFix({49220, 49633, 49635, 49636, 49638},
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_APPLY_AURA;
        spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_DUMMY;
    ...
    ApplySpellFix({60549}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].Effect = 0; });
    ...
    ApplySpellFix({67118}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].Effect = 0; });

    // Pestilence
    ApplySpellFix(
        {50842}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_2].TargetA = TARGET_DEST_TARGET_ENEMY; });

    // Horn of Winter, stacking issues
    ApplySpellFix({57330, 57623},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({70890}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_CASTER_PROCS; });

    // Blood-caked Blade - Blood-caked Strike trigger
    ApplySpellFix({50463}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_CASTER_PROCS; });
    ...
    ApplySpellFix({57604}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Death Pact
    ApplySpellFix({48743}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx &= ~SPELL_ATTR1_EXCLUDE_CASTER; });

    // Raise Ally (trigger)
    ApplySpellFix({46619}, [](SpellInfo* spellInfo) { spellInfo->Attributes &= ~SPELL_ATTR0_NO_AURA_CANCEL; });

    // Frost Strike
    ApplySpellFix({49143, 51416, 51417, 51418, 51419, 55268},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_COMPLETELY_BLOCKED; });

    // Death Knight T10 Tank 2p Bonus
    ApplySpellFix({70650},
        [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_ADD_PCT_MODIFIER; });

    ApplySpellFix({45297, 45284},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix(
        {51560, 51561}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].MiscValue = SPELLMOD_DAMAGE; });

    // Tidal Force
    ApplySpellFix({55166, 55198},
        [](SpellInfo* spellInfo)
    {
    ...
    // Stormstrike
    ApplySpellFix({17364}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });
    ...
    // Sentry Totem
    ApplySpellFix({6495},
        [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(0); });
    ...
    ApplySpellFix(
        {52752}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_CASTER_MODIFIERS; });
    ...
    // Unstable Affliction
    ApplySpellFix({31117}, [](SpellInfo* spellInfo) { spellInfo->PreventionType = SPELL_PREVENTION_TYPE_NONE; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx |= SPELL_ATTR1_NO_REDIRECTION; });
    ...
    // Death's Embrace
    ApplySpellFix({47198, 47199, 47200},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({54347, 54348, 54349},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({42227}, [](SpellInfo* spellInfo) { spellInfo->Speed = 0.0f; });

    // Ritual Enslavement
    ApplySpellFix(
        {22987}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_MOD_CHARM; });

    // Combustion, make this passive
    ApplySpellFix({11129}, [](SpellInfo* spellInfo) { spellInfo->Dispel = DISPEL_NONE; });

    // Magic Absorption
    ApplySpellFix({29441, 29444}, [](SpellInfo* spellInfo) { spellInfo->SpellLevel = 0; });

    // Living Bomb
    ApplySpellFix({44461, 55361, 55362},
        [](SpellInfo* spellInfo)
    {
        spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_TARGET_PROCS;
    ...
    // Evocation
    ApplySpellFix({12051}, [](SpellInfo* spellInfo) { spellInfo->InterruptFlags |= SPELL_INTERRUPT_FLAG_INTERRUPT; });

    // MI Fireblast, WE Frostbolt, MI Frostbolt
    ApplySpellFix({59637, 31707, 72898}, [](SpellInfo* spellInfo) { spellInfo->DmgClass = SPELL_DAMAGE_CLASS_MAGIC; });

    // Blazing Speed
    ApplySpellFix({31641, 31642}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TriggerSpell = 31643; });
    ...
    // // Burnout, trigger
    ApplySpellFix({44450}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_POWER_BURN; });

    // Mirror Image - Summon Spells
    ApplySpellFix({58831, 58833, 58834, 65047},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({36032}, [](SpellInfo* spellInfo) { spellInfo->Attributes |= SPELL_ATTR0_NO_IMMUNITIES; });

    // Chilled (frost armor, ice armor proc)
    ApplySpellFix({6136, 7321}, [](SpellInfo* spellInfo) { spellInfo->PreventionType = SPELL_PREVENTION_TYPE_NONE; });
    ...
    // Glyph of blocking
    ApplySpellFix({58375}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TriggerSpell = 58374; });

    // Sweeping Strikes stance change
    ApplySpellFix({12328}, [](SpellInfo* spellInfo) { spellInfo->Attributes |= SPELL_ATTR0_NOT_SHAPESHIFTED; });
    ...
    // Owlkin Frenzy
    ApplySpellFix({48391}, [](SpellInfo* spellInfo) { spellInfo->Attributes |= SPELL_ATTR0_NOT_SHAPESHIFTED; });

    // Item T10 Restoration 4P Bonus
    ApplySpellFix(
        {70691}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_CASTER_MODIFIERS; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx &= ~SPELL_ATTR1_IMMUNITY_TO_HOSTILE_AND_FRIENDLY_EFFECTS; });
    ...
    // Feral Charge - Cat
    ApplySpellFix({61138, 61132, 50259},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_TARGET_PROCS; });
    ...
    ApplySpellFix({15007}, [](SpellInfo* spellInfo) { spellInfo->SpellFamilyName = SPELLFAMILY_GENERIC; });

    // Luck of the Draw
    ApplySpellFix(
        {72221}, [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags |= AURA_INTERRUPT_FLAG_CHANGE_MAP; });
    ...
    // Playback Speech
    ApplySpellFix({74209}, [](SpellInfo* spellInfo) { spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(1); });
    ...
    ApplySpellFix({49099}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].Amplitude = 15000; });

    // Frightening Shout
    ApplySpellFix({19134}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_DUMMY; });
        ...
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_25_YARDS); // SPELL_EFFECT_WMO_DAMAGE
    });
    ...
    {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_10_YARDS); // SPELL_EFFECT_SCHOOL_DAMAGE
    });
    ...
    {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_3_YARDS); // SPELL_EFFECT_KNOCK_BACK
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_3_YARDS); // SPELL_EFFECT_SCHOOL_DAMAGE
        spellInfo->Effects[EFFECT_2].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_3_YARDS); // SPELL_EFFECT_WEAPON_DAMAGE
    });
    ...
    {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_10_YARDS); // SPELL_EFFECT_KNOCK_BACK
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_10_YARDS); // SPELL_EFFECT_SCHOOL_DAMAGE
        spellInfo->Effects[EFFECT_2].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_20_YARDS); // SPELL_EFFECT_WEAPON_DAMAGE
    });
    ...
    ApplySpellFix({25139}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_ALLOW_AURA_WHILE_DEAD; });

    // Landen Stilwell Transform
    ApplySpellFix({31310}, [](SpellInfo* spellInfo) { spellInfo->Attributes |= SPELL_ATTR0_PASSIVE; });

    // Shadowstalker Stealth
    ApplySpellFix({5916}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].RealPointsPerLevel = 5.0f; });

    // Sneak
    ApplySpellFix({22766}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].RealPointsPerLevel = 5.0f; });

    // Murmur's Touch
    ApplySpellFix({38794, 33711},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });
    ...
    ApplySpellFix({36449, 30632}, [](SpellInfo* spellInfo) { spellInfo->Attributes |= SPELL_ATTR0_AURA_IS_DEBUFF; });

    // Soul Channel
    ApplySpellFix({30531}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });

    // Activate Sunblade Protecto
    ApplySpellFix({46475, 46476},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({45785}, [](SpellInfo* spellInfo) { spellInfo->Speed = 0.0f; });

    // Armageddon
    ApplySpellFix({45909}, [](SpellInfo* spellInfo) { spellInfo->Speed = 8.0f; });
    ...
    // Shared Bonds
    ApplySpellFix({41363}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx &= ~SPELL_ATTR1_IS_CHANNELED; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx6 |= SPELL_ATTR6_IGNORE_PHASE_SHIFT; });
    ...
    ApplySpellFix({37852}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx5 |= SPELL_ATTR5_ALLOW_WHILE_STUNNED; });

    // Amplify Damage
    ApplySpellFix({39095}, [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 1; });

    // Energy Feedback
    ApplySpellFix(
        {44335}, [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags |= AURA_INTERRUPT_FLAG_CHANGE_MAP; });
    ...
    ApplySpellFix(
        {50989}, [](SpellInfo* spellInfo) { spellInfo->Attributes &= ~SPELL_ATTR0_SCALES_WITH_CREATURE_LEVEL; });
    ...
    // Acid Volley
    ApplySpellFix({54714, 29325}, [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 1; });
    ...
    // Infected Wound
    ApplySpellFix({29306}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });
    ...
    // Jagged Knife
    ApplySpellFix({55550}, [](SpellInfo* spellInfo) { spellInfo->Attributes |= SPELL_ATTR0_USES_RANGED_SLOT; });

    // Moorabi - Transformation
    ApplySpellFix({55098}, [](SpellInfo* spellInfo) { spellInfo->InterruptFlags |= SPELL_INTERRUPT_FLAG_INTERRUPT; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->Attributes |= SPELL_ATTR0_USES_RANGED_SLOT; });
    ...
    ApplySpellFix({49456, 49457, 49458},
        [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TargetA = SpellImplicitTargetInfo(TARGET_DEST_DB); });
    ...
    // Flame Breath
    ApplySpellFix({47592}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].Amplitude = 200; });
    ...
    ApplySpellFix({42730}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].TriggerSpell = 42739; });

    ApplySpellFix({59735}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].TriggerSpell = 59736; });

    // Ingvar the Plunderer, Ingvar transform
    ApplySpellFix({42796}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_ALLOW_AURA_WHILE_DEAD; });
    ...
    // Ichoron, Water Blast
    ApplySpellFix({54237, 59520}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_ALWAYS_HIT; });

    // Krik'thir - Mind Flay
    ApplySpellFix(
        {52586, 59367}, [](SpellInfo* spellInfo) { spellInfo->ChannelInterruptFlags |= AURA_INTERRUPT_FLAG_MOVE; });

    // Glare of the Tribunal
    ApplySpellFix({50988, 59870},
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].TargetA = SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ANY);
        spellInfo->Effects[EFFECT_0].TargetB = SpellImplicitTargetInfo();
    ...
    // Lava Strike damage
    ApplySpellFix({57697},
        [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TargetA = SpellImplicitTargetInfo(TARGET_DEST_DEST); });

    // Lava Strike trigger
    ApplySpellFix({57578}, [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 1; });

    // Gift of Twilight Shadow/Fire
    ApplySpellFix({57835, 58766}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx &= ~SPELL_ATTR1_IS_CHANNELED; });

    // Pyrobuffet
    ApplySpellFix({57557}, [](SpellInfo* spellInfo) { spellInfo->ExcludeTargetAuraSpell = 56911; });
    ...
    ApplySpellFix({56263}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_ALWAYS_HIT; });
    ...
    // Vortex (freeze anim)
    ApplySpellFix(
        {55883}, [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags |= AURA_INTERRUPT_FLAG_CHANGE_MAP; });

    // Hurl Pyrite
    ApplySpellFix({62490}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].Effect = 0; });
    ...
    ApplySpellFix({63801}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].MiscValue = 17286; });
    ...
    ApplySpellFix({63278}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].Effect = 0; });

    // Boom (XT-002)
    ApplySpellFix({62834}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].Effect = 0; });

    // Supercharge
    ApplySpellFix({61920}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });

    // Lightning Whirl
    ApplySpellFix({61916}, [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 3; });

    ApplySpellFix({63482}, [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 8; });

    // Stone Grip, remove absorb aura
    ApplySpellFix({62056, 63985}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].Effect = 0; });

    // Sentinel Blast
    ApplySpellFix({64389, 64678}, [](SpellInfo* spellInfo) { spellInfo->Dispel = DISPEL_MAGIC; });

    // Potent Pheromones
    ApplySpellFix({62619}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx |= SPELL_ATTR1_IMMUNITY_PURGES_EFFECT; });
    ...
    // Protective Gaze
    ApplySpellFix({64175}, [](SpellInfo* spellInfo) { spellInfo->RecoveryTime = 25000; });
        ...
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].TriggerSpell =
            64467; // why do they need two script effects :/ (this one has visual effect)
    });

    // Sanity
    ApplySpellFix({63050}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx6 |= SPELL_ATTR6_IGNORE_PHASE_SHIFT; });

    // Shadow Nova
    ApplySpellFix({62714, 65209}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_ALWAYS_HIT; });
    ...
    ApplySpellFix({65509}, [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 1; });

    // Black Hole
    ApplySpellFix(
        {62168, 65250, 62169}, [](SpellInfo* spellInfo) { spellInfo->Attributes |= SPELL_ATTR0_AURA_IS_DEBUFF; });

    // Ground Slam
    ApplySpellFix({62625}, [](SpellInfo* spellInfo) { spellInfo->InterruptFlags |= SPELL_INTERRUPT_FLAG_INTERRUPT; });

    // Onyxia's Lair, Onyxia, Flame Breath (TriggerSpell = 0 and spamming errors in console)
    ApplySpellFix({18435}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].Effect = 0; });

    // Onyxia's Lair, Onyxia, Create Onyxia Spawner
    ApplySpellFix(
        {17647}, [](SpellInfo* spellInfo) { spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(37); });
    ...
    ApplySpellFix({17731, 69294},
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_1].Effect = SPELL_EFFECT_DUMMY;
        spellInfo->CastTimeEntry = sSpellCastTimesStore.LookupEntry(3);
        spellInfo->Effects[EFFECT_1].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_18_YARDS); // 18yd instead of 13yd to make sure all cracks erupt
    });
        ...
        [](SpellInfo* spellInfo)
    {
        ...
        if (spellInfo->Effects[EFFECT_1].Effect)
        {
            ...
            spellInfo->Effects[EFFECT_1].Amplitude =
                ((spellInfo->CastTimeEntry == sSpellCastTimesStore.LookupEntry(170)) ? 50 : 215);
        }
    });
    ...
    ApplySpellFix({61407, 62136, 56251, 54069},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix(
        {59086}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TargetA = SpellImplicitTargetInfo(1); });

    // Halls of Lightning, Arcing Burn
    ApplySpellFix(
        {52671, 59834}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });
    ...
    ApplySpellFix({66867}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_DUMMY; });

    // Trial of the Champion, Summon Risen Jaeren/Arelas
    ApplySpellFix(
        {67705, 67715}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_ALLOW_DEAD_TARGET; });
    ...
    ApplySpellFix({67778, 67877}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TriggerSpell = 68766; });
    ...
    ApplySpellFix({66320, 67472, 67473, 67475},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({66947}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx5 |= SPELL_ATTR5_ALLOW_WHILE_STUNNED; });
    ...
    ApplySpellFix({66532, 66963, 66964, 66965},
        [](SpellInfo* spellInfo) { spellInfo->InterruptFlags |= SPELL_INTERRUPT_FLAG_INTERRUPT; });

    // tempfix, make Nether Power not stealable
    ApplySpellFix({66228, 67106, 67107, 67108},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx4 |= SPELL_ATTR4_CANNOT_BE_STOLEN; });

    // Trial of the Crusader, Faction Champions, Druid - Tranquality
    ApplySpellFix({66086, 67974, 67975, 67976},
        [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].Effect = SPELL_EFFECT_APPLY_AREA_AURA_FRIEND; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->ExcludeTargetAuraSpell = 65686; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->ExcludeTargetAuraSpell = 67222; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->ExcludeTargetAuraSpell = 67223; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->ExcludeTargetAuraSpell = 67224; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->ExcludeTargetAuraSpell = 65684; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->ExcludeTargetAuraSpell = 67176; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->ExcludeTargetAuraSpell = 67177; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->ExcludeTargetAuraSpell = 67178; });
    ...
    ApplySpellFix({65982}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx5 |= SPELL_ATTR5_ALLOW_WHILE_STUNNED; });

    // Trial of the Crusader, Anub'arak, Penetrating Cold
    ApplySpellFix({66013, 67700, 68509, 68510},
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].RadiusEntry = sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_100_YARDS); // 100yd
    });
    ...
    ApplySpellFix({65920, 65922, 65923},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({66129}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });
    ...
    // Phantom Blast
    ApplySpellFix(
        {68982, 70322}, [](SpellInfo* spellInfo) { spellInfo->InterruptFlags |= SPELL_INTERRUPT_FLAG_INTERRUPT; });
    ...
    ApplySpellFix({69029, 70850}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_2].Effect = 0; });

    // Explosive Barrage
    ApplySpellFix(
        {69263}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_MOD_STUN; });
    ...
    ApplySpellFix(
        {69233, 69646}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    ApplySpellFix({69238, 69628},
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].TargetA = SpellImplicitTargetInfo(TARGET_DEST_DEST);
    ...
    // Hoarfrost
    ApplySpellFix({69246, 69245, 69645},
    ...
    ApplySpellFix(
        {73035, 70719}, [](SpellInfo* spellInfo) { spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(13); });
        ...
        [](SpellInfo* spellInfo)
    {
        ...
        spellInfo->Effects[EFFECT_1].TargetA =
            SpellImplicitTargetInfo(TARGET_DEST_DB); // this target is for SPELL_EFFECT_TELEPORT_UNITS
        spellInfo->Effects[EFFECT_1].TargetB = SpellImplicitTargetInfo();
        spellInfo->Effects[EFFECT_2].TargetA = SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ANY);
        spellInfo->Effects[EFFECT_2].TargetB = SpellImplicitTargetInfo();
    });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx &= ~SPELL_ATTR1_IS_SELF_CHANNELED; });

    // Saber Lash (Lord Marrowgar)
    ApplySpellFix({69055, 70814},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({72701, 72702, 72703, 72704},
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].TargetA = SpellImplicitTargetInfo(TARGET_UNIT_CASTER);
        spellInfo->Effects[EFFECT_1].TargetA = SpellImplicitTargetInfo(TARGET_DEST_DEST);
        ...
        spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(
            9); // 30 secs instead of 12, need him for longer, but will stop his actions after 12 secs
    });
    ...
    ApplySpellFix({69146, 70823, 70824, 70825},
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_3_YARDS); // 3yd instead of 5yd
        spellInfo->AttributesEx4 &= ~SPELL_ATTR4_NO_CAST_LOG;
    });

    // Dark Martyrdom (Lady Deathwhisper)
    ApplySpellFix({70897}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_ALLOW_DEAD_TARGET; });
    ...
    // Shadow's Fate
    ApplySpellFix({71169}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });

    // Lock Players and Tap Chest
    ApplySpellFix(
        {72347}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 &= ~SPELL_ATTR3_SUPPRESS_TARGET_PROCS; });
    ...
    ApplySpellFix({72864}, [](SpellInfo* spellInfo) { spellInfo->ExcludeTargetAuraSpell = 0; });

    // Gunship Battle, spell Below Zero
    ApplySpellFix({69705}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_ALWAYS_HIT; });
    ...
    ApplySpellFix({72255, 72444, 72445, 72446},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(21); });

    // Blood Prince Council, Invocation of Blood
    ApplySpellFix({70981, 70982, 70952},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix(
        {71274}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TargetA = SpellImplicitTargetInfo(6); });

    // Ooze Flood (Rotface)
    ApplySpellFix({69783, 69797, 69799, 69802},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx |= SPELL_ATTR1_EXCLUDE_CASTER; });
        ...
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].Effect =
            SPELL_EFFECT_APPLY_AURA; // blizzard typo, 65 instead of 6, aura itself is defined (dummy)
    });

    // Professor Putricide, Gaseous Bloat (Orange Ooze Channel)
    ApplySpellFix({70672, 72455, 72832, 72833},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix(
        {70402, 72511, 72512, 72513}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_CANT_CRIT; });

    // Empowered Flare (Blood Prince Council)
    ApplySpellFix({71708, 72785, 72786, 72787},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_CASTER_MODIFIERS; });
    ...
    {
        spellInfo->Effects[EFFECT_0].RadiusEntry =
            sSpellRadiusStore.LookupEntry(EFFECT_RADIUS_200_YARDS); // another missing radius
    });
    ...
    // Corruption
    ApplySpellFix({70602}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });
    ...
    ApplySpellFix({70127, 72528, 72529, 72530},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({70084, 71050, 71051, 71052},
        [](SpellInfo* spellInfo) { spellInfo->Attributes &= ~SPELL_ATTR0_NO_IMMUNITIES; });

    // Ice Lock
    ApplySpellFix({71614}, [](SpellInfo* spellInfo) { spellInfo->Mechanic = MECHANIC_STUN; });

    // Lich King, Infest
    ApplySpellFix({70541, 73779, 73780, 73781},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });
    ...
    // Harvest Soul
    ApplySpellFix(
        {73655}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_IGNORE_CASTER_MODIFIERS; });
    ...
    ApplySpellFix(
        {73261}, [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags |= AURA_INTERRUPT_FLAG_CHANGE_MAP; });
    ...
    //Blazing Aura
    ApplySpellFix({75885, 75886}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx4 &= ~SPELL_ATTR4_NO_CAST_LOG; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx4 &= ~SPELL_ATTR4_NO_CAST_LOG; });
    ...
    // Still At It
    ApplySpellFix({51931, 51932, 51933},
        [](SpellInfo* spellInfo)
    {
    ...
    // Rallying the Troops
    ApplySpellFix({47394}, [](SpellInfo* spellInfo) { spellInfo->ExcludeTargetAuraSpell = 47394; });

    // A Tangled Skein
    ApplySpellFix(
        {51165, 51173}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_TARGET_PROCS; });
    ...
    // Control
    ApplySpellFix({30790}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].MiscValue = 0; });
    ...
    ApplySpellFix(
        {46432}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 &= ~SPELL_ATTR3_ALLOW_AURA_WHILE_DEAD; });
    ...
    ApplySpellFix({63163}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].BasePoints = 52390; });
    ...
    ApplySpellFix({46260}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });
    ...
    ApplySpellFix({37390}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].MiscValueB = 181; });

    // Where in the World is Hemet Nesingwary? (12521)
    ApplySpellFix({50860}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].BasePoints = 50860; });

    ApplySpellFix({50861}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].BasePoints = 0; });

    // Riding Jokkum
    ApplySpellFix({56606}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].BasePoints = 1; });

    // Blightbeasts be Damned! (12072)
    ApplySpellFix(
        {47424}, [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags &= ~AURA_INTERRUPT_FLAG_NOT_ABOVEWATER; });

    // Dark Horizon (12664), Reunited (12663)
    ApplySpellFix({52190}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].BasePoints = 52391 - 1; });

    // The Sum is Greater than the Parts (13043) - Chained Grip
    ApplySpellFix({60540}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].MiscValue = 300; });

    // Not a Bug (13342)
    ApplySpellFix({60531}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_ALLOW_DEAD_TARGET; });

    // Frankly,  It Makes No Sense... (10672)
    ApplySpellFix({37851}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_TARGET_PROCS; });

    // Honor Challenge (12939)
    ApplySpellFix({21855}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_TARGET_PROCS; });
    ...
    ApplySpellFix({55637}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].MiscValue = 126; });
    ...
    ApplySpellFix(
        {26740}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].ApplyAuraName = SPELL_AURA_MOD_CHARM; });
    ...
    // Deadly Swiftness
    ApplySpellFix({31255}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TriggerSpell = 22588; });

    // Black Magic enchant
    ApplySpellFix({59630}, [](SpellInfo* spellInfo) { spellInfo->Attributes |= SPELL_ATTR0_PASSIVE; });

    // Precious's Ribbon
    ApplySpellFix({72968}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_ALLOW_AURA_WHILE_DEAD; });
    ...
    // Alchemist's Stone
    ApplySpellFix({17619}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_ALLOW_AURA_WHILE_DEAD; });
    ...
    ApplySpellFix(
        {8219, 8220, 8221, 8222}, [](SpellInfo* spellInfo) { spellInfo->Attributes &= ~SPELL_ATTR0_NO_IMMUNITIES; });
    ...
    // Brewfest quests
    ApplySpellFix({47134, 51798}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].Effect = 0; });
    ...
    // Water splash
    ApplySpellFix({42348}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].Effect = 0; });

    // Summon Lantersn
    ApplySpellFix({44255, 44231},
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].TargetA = SpellImplicitTargetInfo(TARGET_DEST_DEST);
        spellInfo->Effects[EFFECT_0].TargetB = SpellImplicitTargetInfo();
    ...
    // Food
    ApplySpellFix({65418}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_2].TriggerSpell = 65410; });

    ApplySpellFix({65422}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_2].TriggerSpell = 65414; });

    ApplySpellFix({65419}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_2].TriggerSpell = 65416; });

    ApplySpellFix({65420}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_2].TriggerSpell = 65412; });

    ApplySpellFix({65421}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_2].TriggerSpell = 65415; });
    ...
    // Light Bonfire (DND)
    ApplySpellFix({29831}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx7 |= SPELL_ATTR7_CAN_CAUSE_INTERRUPT; });

    // Shadowmeld
    ApplySpellFix({58984}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_ONLY_ON_PLAYER; });

    // Flare activation speed
    ApplySpellFix({1543}, [](SpellInfo* spellInfo) { spellInfo->Speed = 0.0f; });
    ...
    ApplySpellFix(
        {16097}, [](SpellInfo* spellInfo) { spellInfo->CastTimeEntry = sSpellCastTimesStore.LookupEntry(16); });
    ...
    ApplySpellFix({42380, 42587}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_ALWAYS_HIT; });
    ...
    // Mortal Shots
    ApplySpellFix({19485, 19487, 19488, 19489, 19490},
        [](SpellInfo* spellInfo)
    {
    ...
    ApplySpellFix({67118, 67150, 67188}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].Effect = 0; });

    // Green Beam
    ApplySpellFix({31628, 31630, 31631},
        [](SpellInfo* spellInfo)
    {
        spellInfo->Effects[EFFECT_0].TargetA = SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ANY);
        spellInfo->MaxAffectedTargets = 1;
    ...
    // Manastorm
    ApplySpellFix({21097}, [](SpellInfo* spellInfo) { spellInfo->InterruptFlags &= ~SPELL_INTERRUPT_FLAG_INTERRUPT; });
    ...
    // Reflection
    ApplySpellFix({22067}, [](SpellInfo* spellInfo) { spellInfo->Dispel = DISPEL_NONE; });

    // Focused Assault
    // Brutal Assault
    ApplySpellFix(
        {46392, 46393}, [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags |= AURA_INTERRUPT_FLAG_CHANGE_MAP; });
    ...
    // Bestial Wrath
    ApplySpellFix({19574}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx4 |= SPELL_ATTR4_AURA_EXPIRES_OFFLINE; });

    // Shadowflame
    ApplySpellFix({22539}, [](SpellInfo* spellInfo) { spellInfo->InterruptFlags &= ~SPELL_INTERRUPT_FLAG_INTERRUPT; });

    // PX-238 Winter Wondervolt
    ApplySpellFix({26157, 26272, 26273, 26274}, [](SpellInfo* spellInfo) { spellInfo->Mechanic = 0; });

    // Calm Dragonkin
    ApplySpellFix({19872}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx |= SPELL_ATTR1_EXCLUDE_CASTER; });

    // Cosmetic - Lightning Beam Channel
    ApplySpellFix({45537}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });
    ...
    // Brood Power : Bronze
    ApplySpellFix({22311}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_CASTER_PROCS; });

    // Rapture
    ApplySpellFix({63652, 63653, 63654, 63655},
        [](SpellInfo* spellInfo)
    {
        spellInfo->AttributesEx |= SPELL_ATTR1_NO_THREAT;
        spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT;
    ...
    // Everlasting Affliction
    ApplySpellFix({47422}, [](SpellInfo* spellInfo) { spellInfo->SchoolMask = SPELL_SCHOOL_MASK_SHADOW; });
    ...
    ApplySpellFix(
        {21895}, [](SpellInfo* spellInfo) { spellInfo->Effects[0].SpellClassMask = flag96(0x0603E000, 0x00200100); });

    // Jokkum Summon
    ApplySpellFix({56541}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].MiscValueB = 844; });

    // Hakkar Cause Insanity
    ApplySpellFix({24327}, [](SpellInfo* spellInfo) { spellInfo->Dispel = DISPEL_NONE; });

    // Summon Nightmare Illusions
    ApplySpellFix({24681, 24728, 24729}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].MiscValueB = 64; });

    // Blood Siphon
    ApplySpellFix({24322, 24323},
        [](SpellInfo* spellInfo)
    {
    ...
    // Place Fake Fur
    ApplySpellFix({46085}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].MiscValue = 8; });

    // Smash Mammoth Trap
    ApplySpellFix({46201}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].MiscValue = 8; });

    // Elemental Mastery
    ApplySpellFix({16166},
        [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].SpellClassMask = flag96(0x00000003, 0x00001000); });

    // Elemental Vulnerability
    ApplySpellFix({28772}, [](SpellInfo* spellInfo) { spellInfo->Speed = 1; });

    // Find the Ancient Hero: Kill Credit
    ApplySpellFix({25729}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TargetA = TARGET_UNIT_SUMMONER; });
    ...
    // Lash
    ApplySpellFix({25852}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].Effect = 0; });
    ...
    // Death's Respite
    ApplySpellFix(
        {67731, 68305}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_TARGET_PROCS; });
    ...
    // Feed Pet
    ApplySpellFix(
        {1539, 51284}, [](SpellInfo* spellInfo) { spellInfo->Attributes |= SPELL_ATTR0_ALLOW_WHILE_SITTING; });

    // Judgement (Paladin T2 8P Bonus)
    // Battlegear of Eternal Justice
    ApplySpellFix(
        {23591, 26135}, [](SpellInfo* spellInfo) { spellInfo->ProcFlags = PROC_FLAG_DONE_SPELL_MELEE_DMG_CLASS; });

    // Gift of Arthas
    ApplySpellFix({11371}, [](SpellInfo* spellInfo) { spellInfo->SpellFamilyName = SPELLFAMILY_POTION; });
    ...
    // Collect Rookery Egg
    ApplySpellFix({15958}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_1].Effect = 0; });

    // WotLK Prologue Frozen Shade Visual, temp used to restore visual after Dispersion
    ApplySpellFix(
        {53444}, [](SpellInfo* spellInfo) { spellInfo->DurationEntry = sSpellDurationStore.LookupEntry(27); });

    // Rental Racing Ram
    ApplySpellFix(
        {43883}, [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags &= ~AURA_INTERRUPT_FLAG_NOT_ABOVEWATER; });

    // Summon Worm
    ApplySpellFix({518, 25831, 25832}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].MiscValueB = 64; });

    // Uppercut
    ApplySpellFix({26007}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_CASTER_PROCS; });
    ...
    ApplySpellFix(
        {35475, 35476, 35477, 35478}, [](SpellInfo* spellInfo) { spellInfo->ExcludeTargetAuraSpell = 51120; });
    ...
    ApplySpellFix({6606, 14915, 16093},
        [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags &= ~AURA_INTERRUPT_FLAG_NOT_SEATED; });

    // Cleansing Totem, Healing Stream Totem, Mana Tide Totem
    ApplySpellFix({8171, 52025, 52041, 52042, 52046, 52047, 52048, 52049, 52050, 58759, 58760, 58761, 39610, 39609},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });
    // Game In Session
    ApplySpellFix({39331},
        [](SpellInfo* spellInfo)
    ...
    ApplySpellFix({63882, 63886},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx5 |= SPELL_ATTR5_ALLOW_ACTION_DURING_CHANNEL; });
    ...
    // Turn the Tables
    ApplySpellFix(
        {51627, 51628, 51629}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_DOT_STACKING_RULE; });

    // Silence
    ApplySpellFix({18278},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx4 |= SPELL_ATTR4_NOT_IN_ARENA_OR_RATED_BATTLEGROUND; });

    // Absorb Life
    ApplySpellFix({34239}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].ValueMultiplier = 1; });
    ...
    // Target Fissures
    ApplySpellFix({30745}, [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 1; });

    // Acid Spit
    ApplySpellFix({34290}, [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 1; });
    ...
    // Poultryized!
    ApplySpellFix(
        {30504}, [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags |= AURA_INTERRUPT_FLAG_TAKE_DAMAGE; });

    // Torment of the Worgen
    ApplySpellFix({30567}, [](SpellInfo* spellInfo) { spellInfo->ProcChance = 3; });

    // Summon Water Elementals
    ApplySpellFix({29962, 37051, 37052, 37053},
        [](SpellInfo* spellInfo)
    {
        spellInfo->RangeEntry = sSpellRangeStore.LookupEntry(13); // 50000yd
    });
    ...
    ApplySpellFix({27360}, [](SpellInfo* spellInfo) { spellInfo->ChannelInterruptFlags |= AURA_INTERRUPT_FLAG_MOVE; });
        ...
        [](SpellInfo* spellInfo)
    {
        spellInfo->AuraInterruptFlags |=
            (AURA_INTERRUPT_FLAG_MOUNT | AURA_INTERRUPT_FLAG_CAST | AURA_INTERRUPT_FLAG_TALK);
    });

    // Improved Mind Flay and Smite
    ApplySpellFix({37571}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].SpellClassMask[0] = 8388736; });
    ...
    // Chains of Naberius
    ApplySpellFix({36146}, [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 1; });
    ...
    // Spotlight
    ApplySpellFix(
        {29683, 32214}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx5 |= SPELL_ATTR5_DO_NOT_DISPLAY_DURATION; });

    // Haunted
    ApplySpellFix({53768}, [](SpellInfo* spellInfo) { spellInfo->Attributes |= SPELL_ATTR0_NO_AURA_CANCEL; });

    // Tidal Wave
    ApplySpellFix({37730}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Commanding Shout
    ApplySpellFix({469, 47439, 47440},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });
    ...
    // Plague Effect
    ApplySpellFix(
        {19594, 26557}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Prayer of Fortitude
    ApplySpellFix({21562, 21564, 25392, 48162},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Gift of the Wild
    ApplySpellFix({21849, 21850, 26991, 48470, 69381},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Arcane Brilliance
    ApplySpellFix({23028, 27127, 43002},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Prayer of Spirit
    ApplySpellFix({27681, 32999, 48074},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Prayer of Shadow Protection
    ApplySpellFix({27683, 39374, 48170},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Nagrand Fort Buff Reward Raid
    ApplySpellFix({33006}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Ancestral Awakening
    ApplySpellFix({52759}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Turn the Tables
    ApplySpellFix({52910, 52914, 52915},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Judgements of the Wise
    ApplySpellFix({54180}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Replenishment
    ApplySpellFix({57669}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Dalaran Brilliance
    ApplySpellFix({61316}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // [DND] Dalaran Brilliance
    ApplySpellFix({61332}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Infinite Replenishment + Wisdom
    ApplySpellFix({61782}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Renewed Hope
    ApplySpellFix({63944}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Fortitude
    ApplySpellFix({69377}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Blessing of Forgotten Kings
    ApplySpellFix({69378}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Lucky Charm
    ApplySpellFix({69511}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Shiny Shard of the Scale Heal Targeter
    ApplySpellFix({69749}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Purified Shard of the Scale Heal Targeter
    ApplySpellFix({69754}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Brilliance
    ApplySpellFix({69994}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT; });

    // Domination
    ApplySpellFix({37135}, [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 5; });
    ...
    // Greater Fireball
    ApplySpellFix({33051}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx4 |= SPELL_ATTR4_NO_CAST_LOG; });
    ...
    // Shadow Grasp
    ApplySpellFix({30410}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx6 |= SPELL_ATTR6_NO_PUSHBACK; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->AuraInterruptFlags &= ~AURA_INTERRUPT_FLAG_NOT_ABOVEWATER; });
    ...
    ApplySpellFix({41453}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].MiscValue = 124; });

    // Power of the Guardian
    ApplySpellFix({28142, 28143, 28144, 28145},
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_ALLOW_AURA_WHILE_DEAD; });
    ...
    ApplySpellFix(
        ...
        [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_CASTER_PROCS; });

    // Conjure Refreshment Table (Rank 1, Rank 2)
    ApplySpellFix({43985, 58661},
        [](SpellInfo* spellInfo)
    {
    ...
    // Paralyze
    ApplySpellFix(
        {41083}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx5 |= SPELL_ATTR5_ALLOW_ACTION_DURING_CHANNEL; });

    // Fatal Attraction
    ApplySpellFix({40870}, [](SpellInfo* spellInfo) { spellInfo->MaxAffectedTargets = 1; });
    ...
    // Siphon Life (heal)
    ApplySpellFix({63106}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_CASTER_PROCS; });

    // Fury
    ApplySpellFix({40601}, [](SpellInfo* spellInfo) { spellInfo->AttributesEx3 |= SPELL_ATTR3_SUPPRESS_CASTER_PROCS; });

    // Claw Rage
    ApplySpellFix({43149}, [](SpellInfo* spellInfo) { spellInfo->Effects[EFFECT_0].TriggerSpell = 0; });
                                                  ...
                                                  spellEffectInfo.TargetB.GetTarget() == TARGET_DEST_TRAJ))
            {
                ...
                if (SpellInfo* spellInfoTrigger = const_cast<SpellInfo*>(GetSpellInfo(spellEffectInfo.TriggerSpell)))
                {
                    ...
                    float maxRangeTrigger =
                        spellInfoTrigger->RangeEntry ? spellInfoTrigger->RangeEntry->RangeMax[0] : 0.0f;
    ...
    properties->Type = SUMMON_TYPE_TOTEM;
    if ((properties =
                const_cast<SummonPropertiesEntry*>(sSummonPropertiesStore.LookupEntry(628)))) // Hungry Plaguehound
    {
    ...
    CreatureDisplayInfoEntry* displayEntry =
        const_cast<CreatureDisplayInfoEntry*>(sCreatureDisplayInfoStore.LookupEntry(17028)); // Kurken
    ...
    FactionTemplateEntry* factionTemplateEntry =
        const_cast<FactionTemplateEntry*>(sFactionTemplateStore.LookupEntry(1978)); // Warsong Offensive
    ...
    factionTemplateEntry =
        const_cast<FactionTemplateEntry*>(sFactionTemplateStore.LookupEntry(1806)); // Netherstorm Agent
    factionTemplateEntry->friendlyMask &= ~1;
    ...
    VehicleSeatEntry* vse =
        const_cast<VehicleSeatEntry*>(sVehicleSeatStore.LookupEntry(4689)); // Siege Engine, Accessory
    ...
    vse = const_cast<VehicleSeatEntry*>(
        sVehicleSeatStore.LookupEntry(3806)); // Mimiron - seat on VX-001 for ACU during last phase
