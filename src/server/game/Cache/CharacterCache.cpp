...
void CharacterCache::LoadCharacterCacheStorage()
{
    ...
    QueryResult result =
        CharacterDatabase.Query("SELECT guid, name, account, race, gender, class, level FROM characters");
    if (!result)
    {
    ...
    QueryResult mailCountResult =
        CharacterDatabase.Query("SELECT receiver, COUNT(receiver) FROM mail GROUP BY receiver");
    if (mailCountResult)
    {
        do
        {
            Field* fields = mailCountResult->Fetch();
            UpdateCharacterMailCount(ObjectGuid(HighGuid::Player, fields[0].Get<uint32>()),
...
void CharacterCache::RefreshCacheEntry(uint32 lowGuid)
{
    QueryResult result = CharacterDatabase.Query(
        "SELECT guid, name, account, race, gender, class, level FROM characters WHERE guid = {}", lowGuid);
    if (!result)
        return;
    ...
    QueryResult mailCountResult = CharacterDatabase.Query(
        "SELECT receiver, COUNT(receiver) FROM mail WHERE receiver = {} GROUP BY receiver", lowGuid);
    if (mailCountResult)
    {
        do
        {
...
Modifying functions
*/
void CharacterCache::AddCharacterCacheEntry(ObjectGuid const& guid, uint32 accountId, std::string const& name,
    uint8 gender, uint8 race, uint8 playerClass, uint8 level)
{
...
void CharacterCache::UpdateCharacterData(
    ObjectGuid const& guid, std::string const& name, Optional<uint8> gender /*= {}*/, Optional<uint8> race /*= {}*/)
{
