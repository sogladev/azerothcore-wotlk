...
{
    return !(pos->GetPositionX() < _minX || pos->GetPositionX() > _maxX || pos->GetPositionY() < _minY ||
...
TriangleBoundary::TriangleBoundary(
    Position const& pointA, Position const& pointB, Position const& pointC, bool isInverted) :
    AreaBoundary(isInverted),
    _a(pointA),
    _b(pointB),
    _c(pointC),
...
ParallelogramBoundary::ParallelogramBoundary(
    Position const& cornerA, Position const& cornerB, Position const& cornerD, bool isInverted) :
    AreaBoundary(isInverted),
    _a(cornerA),
    _b(cornerB),
    _d(cornerD),
    _c(DoublePosition(_d.GetDoublePositionX() + (_b.GetDoublePositionX() - _a.GetDoublePositionX()),
        _d.GetDoublePositionY() + (_b.GetDoublePositionY() - _a.GetDoublePositionY()))),
    _abx(_b.GetDoublePositionX() - _a.GetDoublePositionX()),
...
bool ParallelogramBoundary::IsWithinBoundaryArea(Position const* pos) const
{
    ...
    bool sign3 = ((-_d.GetDoublePositionY() + pos->GetPositionY()) * _abx -
                     (-_d.GetDoublePositionX() + pos->GetPositionX()) * _aby) < 0; // AB = -CD
    bool sign4 = ((-_c.GetDoublePositionY() + pos->GetPositionY()) * _dax -
                     (-_c.GetDoublePositionX() + pos->GetPositionX()) * _day) < 0; // DA = -BC
