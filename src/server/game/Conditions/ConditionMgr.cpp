...
bool Condition::Meets(ConditionSourceInfo& sourceInfo)
{
    ...
    {
        LOG_ERROR("condition",
            "ConditionTarget {} for for condition (Entry: {} Type: {} Group: {}) is greater or equal than MAX_CONDITION_TARGETS",
            ConditionTarget,
            SourceEntry,
            SourceType,
            SourceGroup);
    ...
    {
        LOG_DEBUG("condition",
            "Condition object not found for condition (Entry: {} Type: {} Group: {})",
            SourceEntry,
            SourceType,
            SourceGroup);
        return false;
    ...
    switch (ConditionType)
    {
            ...
            if (Unit* unit = object->ToUnit())
            {
                if (Player* player = unit->GetCharmerOrOwnerPlayerOrPlayerItself())
                {
                    condMeets = player->HasSkill(ConditionValue1) &&
                                player->GetBaseSkillValue(ConditionValue1) >= ConditionValue2;
                }
            }
            break;
        }
        ...
        case CONDITION_QUEST_SATISFY_EXCLUSIVE:
        {
            if (Unit* unit = object->ToUnit())
            {
                if (Player* player = unit->GetCharmerOrOwnerPlayerOrPlayerItself())
                {
                    ...
                    condMeets =
                        !player->IsQuestRewarded(ConditionValue1) && player->SatisfyQuestExclusiveGroup(quest, false);
                }
            }
            break;
        }
        ...
        {
            if (Unit* unit = object->ToUnit())
                condMeets = CompareValues(static_cast<ComparisionType>(ConditionValue2),
        ...
        {
            condMeets = static_cast<bool>(GetClosestCreatureWithEntry(
                object, ConditionValue1, static_cast<float>(ConditionValue2), !ConditionValue3));
            break;
        }
        case CONDITION_NEAR_GAMEOBJECT:
        {
            if (!ConditionValue3)
            {
                condMeets = static_cast<bool>(
                    GetClosestGameObjectWithEntry(object, ConditionValue1, static_cast<float>(ConditionValue2)));
                break;
            }
            else
            {
                if (GameObject* go =
                        GetClosestGameObjectWithEntry(object, ConditionValue1, static_cast<float>(ConditionValue2)))
                {
                    if ((go->GetGoState() == GO_STATE_READY && ConditionValue3 == 1) ||
                        (go->GetGoState() != GO_STATE_READY && ConditionValue3 == 2))
                        condMeets = true;
                    else
                        condMeets = false;
                }
        ...
        case CONDITION_OBJECT_ENTRY_GUID:
        {
            if (ConditionValue3 == 1 &&
                object->ToUnit()) // pussywizard: if == 1, ignore not attackable/selectable targets
        ...
        {
            condMeets = sWorldState->IsConditionFulfilled(static_cast<WorldStateCondition>(ConditionValue1),
                static_cast<WorldStateConditionState>(ConditionValue2));
            break;
        }
        default:
            condMeets = false;
    ...
    for (ConditionList::const_iterator i = conditions.begin(); i != conditions.end(); ++i)
    {
        ...
        if ((*i)->isLoaded())
        {
            ...
            if ((*i)->ReferenceId) // handle reference
            {
                ...
                {
                    LOG_DEBUG("condition",
                        "IsPlayerMeetToConditionList: Reference template -{} not found",
                        (*i)->ReferenceId);
                }
            }
            else // handle normal condition
...
bool ConditionMgr::CanHaveSourceGroupSet(ConditionSourceType sourceType) const
{
    return (sourceType == CONDITION_SOURCE_TYPE_CREATURE_LOOT_TEMPLATE ||
            ...
            sourceType == CONDITION_SOURCE_TYPE_GOSSIP_MENU || sourceType == CONDITION_SOURCE_TYPE_GOSSIP_MENU_OPTION ||
            sourceType == CONDITION_SOURCE_TYPE_VEHICLE_SPELL ||
            sourceType == CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET ||
            sourceType == CONDITION_SOURCE_TYPE_SPELL_CLICK_EVENT || sourceType == CONDITION_SOURCE_TYPE_SMART_EVENT ||
            sourceType == CONDITION_SOURCE_TYPE_NPC_VENDOR || sourceType == CONDITION_SOURCE_TYPE_PLAYER_LOOT_TEMPLATE);
}
...
ConditionList ConditionMgr::GetConditionsForSpellClickEvent(uint32 creatureId, uint32 spellId)
{
    ...
    if (itr != SpellClickEventConditionStore.end())
    {
        ...
        if (i != (*itr).second.end())
        {
            cond = (*i).second;
            LOG_DEBUG("condition",
                "GetConditionsForSpellClickEvent: found conditions for Vehicle entry {} spell {}",
                creatureId,
                spellId);
        }
    }
...
ConditionList ConditionMgr::GetConditionsForVehicleSpell(uint32 creatureId, uint32 spellId)
{
    ...
    if (itr != VehicleSpellConditionStore.end())
    {
        ...
        if (i != (*itr).second.end())
        {
            cond = (*i).second;
            LOG_DEBUG("condition",
                "GetConditionsForVehicleSpell: found conditions for Vehicle entry {} spell {}",
                creatureId,
                spellId);
        }
    }
...
ConditionList ConditionMgr::GetConditionsForSmartEvent(int32 entryOrGuid, uint32 eventId, uint32 sourceType)
{
    ...
    if (itr != SmartEventConditionStore.end())
    {
        ...
        if (i != (*itr).second.end())
        {
            cond = (*i).second;
            LOG_DEBUG("condition",
                "GetConditionsForSmartEvent: found conditions for Smart Event entry or guid {} event_id {}",
                entryOrGuid,
                eventId);
        }
    }
...
ConditionList ConditionMgr::GetConditionsForNpcVendorEvent(uint32 creatureId, uint32 itemId)
{
    ...
    if (itr != NpcVendorConditionContainerStore.end())
    {
        ...
        if (i != (*itr).second.end())
        {
            ...
            {
                LOG_DEBUG("condition",
                    "GetConditionsForNpcVendorEvent: found conditions for creature entry {} item {}",
                    creatureId,
                    itemId);
            }
            else
            {
                LOG_DEBUG(
                    "condition", "GetConditionsForNpcVendorEvent: found conditions for creature entry {}", creatureId);
            }
        }
    }
    return cond;
    ...
    do
    {
        ...
        if (iConditionTypeOrReference < 0) // it has a reference
        {
            ...
            if (cond->NegativeCondition)
                LOG_ERROR("sql.sql",
                    "Condition {} {} has useless data in NegativeCondition ({})!",
                    rowType,
                    iSourceTypeOrReferenceId,
        ...
        if (iSourceTypeOrReferenceId < 0) // it is a reference template
        {
            ...
            if (ConditionReferenceStore.find(uRefId) ==
                ConditionReferenceStore.end()) // make sure we have a list for our conditions, based on reference id
            {
        ...
        {
            LOG_ERROR("condition",
                "Condition type {} entry {} can't have ErrorType ({}), set to 0!",
                uint32(cond->SourceType),
                cond->SourceEntry,
                cond->ErrorType);
            cond->ErrorType = 0;
        ...
        {
            LOG_ERROR("condition",
                "Condition type {} entry {} has any ErrorType, ErrorTextId ({}) is set, set to 0!",
                uint32(cond->SourceType),
                cond->SourceEntry,
                cond->ErrorTextId);
            cond->ErrorTextId = 0;
        ...
        if (cond->SourceGroup || cond->SourceType == CONDITION_SOURCE_TYPE_PLAYER_LOOT_TEMPLATE)
        {
            ...
            switch (cond->SourceType)
            {
                ...
                case CONDITION_SOURCE_TYPE_DISENCHANT_LOOT_TEMPLATE:
                    valid =
                        addToLootTemplate(cond, LootTemplates_Disenchant.GetLootForConditionFill(cond->SourceGroup));
                    break;
                ...
                case CONDITION_SOURCE_TYPE_GAMEOBJECT_LOOT_TEMPLATE:
                    valid =
                        addToLootTemplate(cond, LootTemplates_Gameobject.GetLootForConditionFill(cond->SourceGroup));
                    break;
                ...
                case CONDITION_SOURCE_TYPE_PICKPOCKETING_LOOT_TEMPLATE:
                    valid =
                        addToLootTemplate(cond, LootTemplates_Pickpocketing.GetLootForConditionFill(cond->SourceGroup));
                    break;
                case CONDITION_SOURCE_TYPE_PROSPECTING_LOOT_TEMPLATE:
                    valid =
                        addToLootTemplate(cond, LootTemplates_Prospecting.GetLootForConditionFill(cond->SourceGroup));
                    break;
...
bool ConditionMgr::addToGossipMenuItems(Condition* cond)
{
    ...
    LOG_ERROR(
        "sql.sql", "addToGossipMenuItems: GossipMenuId {} Item {} not found", cond->SourceGroup, cond->SourceEntry);
    return false;
}
...
bool ConditionMgr::isSourceTypeValid(Condition* cond)
{
    ...
    {
        LOG_ERROR(
            "sql.sql", "Invalid ConditionSourceType {} in `condition` table, ignoring.", uint32(cond->SourceType));
        return false;
    }
    ...
    switch (cond->SourceType)
    {
        ...
        {
            LOG_ERROR("sql.sql",
                "ConditionSourceType {} in `condition` table is not supported on 3.3.5a, ignoring.",
                uint32(cond->SourceType));
            return false;
        }
        case CONDITION_SOURCE_TYPE_CREATURE_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceGroup {} in `condition` table, does not exist in `creature_loot_template`, ignoring.",
                    cond->SourceGroup);
                return false;
            }
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceType {}, SourceEntry {} in `condition` table, does not exist in `item_template`, ignoring.",
                    cond->SourceType,
                    cond->SourceEntry);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_DISENCHANT_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceGroup {} in `condition` table, does not exist in `disenchant_loot_template`, ignoring.",
                    cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_FISHING_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceGroup {} in `condition` table, does not exist in `fishing_loot_template`, ignoring.",
                    cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_GAMEOBJECT_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceGroup {} in `condition` table, does not exist in `gameobject_loot_template`, ignoring.",
                    cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_ITEM_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceGroup {} in `condition` table, does not exist in `item_loot_template`, ignoring.",
                    cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_MAIL_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceGroup {} in `condition` table, does not exist in `mail_loot_template`, ignoring.",
                    cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_MILLING_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceGroup {} in `condition` table, does not exist in `milling_loot_template`, ignoring.",
                    cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_PICKPOCKETING_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceGroup {} in `condition` table, does not exist in `pickpocketing_loot_template`, ignoring.",
                    cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_PROSPECTING_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceGroup {} in `condition` table, does not exist in `prospecting_loot_template`, ignoring.",
                    cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_REFERENCE_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceGroup {} in `condition` table, does not exist in `reference_loot_template`, ignoring.",
                    cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_SKINNING_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceGroup {} in `condition` table, does not exist in `skinning_loot_template`, ignoring.",
                    cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_SPELL_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceGroup {} in `condition` table, does not exist in `spell_loot_template`, ignoring.",
                    cond->SourceGroup);
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceEntry {} in `condition` table, does not exist in `spell.dbc`, ignoring.",
                    cond->SourceEntry);
                return false;
            }
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceEntry {} in `condition` table, has incorrect SourceGroup {} (spell effectMask) set , ignoring.",
                    cond->SourceEntry,
                    cond->SourceGroup);
                return false;
            }
            ...
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                ...
                LOG_ERROR("sql.sql",
                    "SourceEntry {} SourceGroup {} in `condition` table - spell {} does not have implicit targets of types: _AREA_, _CONE_, _NEARBY_ for effect {}, SourceGroup needs correction, ignoring.",
                    cond->SourceEntry,
                    origGroup,
                    cond->SourceEntry,
                    uint32(i));
        ...
        case CONDITION_SOURCE_TYPE_CREATURE_TEMPLATE_VEHICLE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceEntry {} in `condition` table, does not exist in `creature_template`, ignoring.",
                    cond->SourceEntry);
                return false;
            }
            break;
        ...
        case CONDITION_SOURCE_TYPE_QUEST_AVAILABLE:
            ...
            {
                LOG_ERROR("sql.sql",
                    "CONDITION_SOURCE_TYPE_QUEST_AVAILABLE specifies non-existing quest ({}), skipped",
                    cond->SourceEntry);
                return false;
            }
            break;
        case CONDITION_SOURCE_TYPE_UNUSED_20:
            LOG_ERROR("sql.sql",
                "CONDITION_SOURCE_TYPE_UNUSED_20 is not in use. SourceEntry = ({}), skipped",
                cond->SourceEntry);
            break;
        case CONDITION_SOURCE_TYPE_VEHICLE_SPELL:
        case CONDITION_SOURCE_TYPE_SPELL_CLICK_EVENT:
        ...
        case CONDITION_SOURCE_TYPE_NPC_VENDOR:
        {
            ...
            if (cond->SourceEntry)
            {
                ...
                {
                    LOG_ERROR("condition",
                        "SourceEntry {} in `condition` table, does not exist in `item_template`, ignoring.",
                        cond->SourceEntry);
                    return false;
                }
            }
        ...
        case CONDITION_SOURCE_TYPE_PLAYER_LOOT_TEMPLATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SourceGroup {} in `condition` table, does not exist in `player_loot_template`, ignoring.",
                    cond->SourceGroup);
                return false;
            }
...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
    ...
    {
        LOG_ERROR("sql.sql",
            "SourceEntry {} in `condition` table has an invalid ConditionType ({}), ignoring.",
            cond->SourceEntry,
    ...
    switch (cond->ConditionType)
    {
        case CONDITION_TERRAIN_SWAP:
            LOG_ERROR("sql.sql",
                "SourceEntry {} in `condition` table has a ConditionType that is not supported on 3.3.5a ({}), ignoring.",
                cond->SourceEntry,
                uint32(cond->ConditionType));
            return false;
        default:
    ...
    {
        LOG_ERROR("sql.sql",
            "SourceType {}, SourceEntry {} in `condition` table, has incorrect ConditionTarget set, ignoring.",
            cond->SourceType,
            cond->SourceEntry);
        return false;
    }
    ...
    switch (cond->ConditionType)
    {
        case CONDITION_AURA:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "Aura condition has non existing effect index ({}) (must be 0..2), skipped",
                    cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_ITEM:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "Item condition has 0 set for item count in value2 ({}), skipped",
                    cond->ConditionValue2);
                return false;
            }
            break;
        ...
        case CONDITION_ITEM_EQUIPPED:
        {
            ...
            {
                LOG_ERROR(
                    "sql.sql", "ItemEquipped condition has non existing item ({}), skipped", cond->ConditionValue1);
                return false;
            }
        ...
        case CONDITION_ZONEID:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "ZoneID condition requires to be in area ({}) which is a subzone but zone expected, skipped",
                    cond->ConditionValue1);
                return false;
            }
        ...
        case CONDITION_REPUTATION_RANK:
        {
            ...
            {
                LOG_ERROR(
                    "sql.sql", "Reputation condition has non existing faction ({}), skipped", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_SKILL:
        {
            ...
            {
                LOG_ERROR(
                    "sql.sql", "Skill condition specifies non-existing skill ({}), skipped", cond->ConditionValue1);
                return false;
            }
            ...
            {
                LOG_ERROR(
                    "sql.sql", "Skill condition specifies invalid skill value ({}), skipped", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_QUEST_SATISFY_EXCLUSIVE:
        {
            ...
            {
                LOG_ERROR(
                    "sql.sql", "Quest condition specifies non-existing quest ({}), skipped", cond->ConditionValue1);
                return false;
            }
        ...
        case CONDITION_ACTIVE_EVENT:
        {
            ...
            {
                LOG_ERROR(
                    "sql.sql", "ActiveEvent condition has non existing event id ({}), skipped", cond->ConditionValue1);
                return false;
            }
        ...
        case CONDITION_ACHIEVEMENT:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "Achivement condition has non existing achivement id ({}), skipped",
                    cond->ConditionValue1);
                return false;
            }
        ...
        case CONDITION_NEAR_CREATURE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "NearCreature condition has non existing creature template entry ({}), skipped",
                    cond->ConditionValue1);
                return false;
            }
            break;
        ...
        case CONDITION_NEAR_GAMEOBJECT:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "NearGameObject condition has non existing gameobject template entry ({}), skipped",
                    cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue3 > 2)
                LOG_ERROR("sql.sql",
                    "NearGameObject condition for gameobject ID ({}) has data over 2 for value3 ({})!",
                    cond->ConditionValue1,
                    cond->ConditionValue3);
            break;
        }
        case CONDITION_OBJECT_ENTRY_GUID:
        {
            switch (cond->ConditionValue1)
            {
                case TYPEID_UNIT:
                    ...
                    {
                        LOG_ERROR("sql.sql",
                            "ObjectEntryGuid condition has non existing creature template entry ({}), skipped",
                            cond->ConditionValue2);
                        return false;
                    }
                    if (cond->ConditionValue3 > 1)
                    {
                        if (CreatureData const* creatureData = sObjectMgr->GetCreatureData(cond->ConditionValue3))
                        {
                            ...
                            {
                                LOG_ERROR("sql.sql",
                                    "ObjectEntryGuid condition has guid {} set but does not match creature entry ({}), skipped",
                                    cond->ConditionValue3,
                                    cond->ConditionValue2);
                                return false;
                            }
                        ...
                        {
                            LOG_ERROR("sql.sql",
                                "ObjectEntryGuid condition has non existing creature guid ({}), skipped",
                                cond->ConditionValue3);
                            return false;
                        }
                    }
                ...
                case TYPEID_GAMEOBJECT:
                    ...
                    {
                        LOG_ERROR("sql.sql",
                            "ObjectEntryGuid condition has non existing gameobject template entry ({}), skipped",
                            cond->ConditionValue2);
                        return false;
                    }
                    if (cond->ConditionValue3)
                    {
                        if (GameObjectData const* goData = sObjectMgr->GetGameObjectData(cond->ConditionValue3))
                        {
                            ...
                            {
                                LOG_ERROR("sql.sql",
                                    "ObjectEntryGuid condition has guid {} set but does not match gameobject entry ({}), skipped",
                                    cond->ConditionValue3,
                                    cond->ConditionValue2);
                                return false;
                            }
                        ...
                        {
                            LOG_ERROR("sql.sql",
                                "ObjectEntryGuid condition has non existing gameobject guid ({}), skipped",
                                cond->ConditionValue3);
                            return false;
                        }
                    }
                ...
                case TYPEID_CORPSE:
                    if (cond->ConditionValue2)
                        LOG_ERROR("sql.sql",
                            "ObjectEntryGuid condition has useless data in value2 ({})!",
                            cond->ConditionValue2);
                    if (cond->ConditionValue3)
                        LOG_ERROR("sql.sql",
                            "ObjectEntryGuid condition has useless data in value3 ({})!",
                            cond->ConditionValue3);
                    break;
                default:
                    LOG_ERROR("sql.sql",
        ...
        {
            if (!cond->ConditionValue1 ||
                (cond->ConditionValue1 & ~(TYPEMASK_UNIT | TYPEMASK_PLAYER | TYPEMASK_GAMEOBJECT | TYPEMASK_CORPSE)))
            {
                LOG_ERROR(
                    "sql.sql", "TypeMask condition has invalid typemask set ({}), skipped", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue2)
        ...
        case CONDITION_RELATION_TO:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "RelationTo condition has invalid ConditionValue1(ConditionTarget selection) ({}), skipped",
                    cond->ConditionValue1);
                return false;
            }
            ...
            {
                LOG_ERROR("sql.sql",
                    "RelationTo condition has ConditionValue1(ConditionTarget selection) set to self ({}), skipped",
                    cond->ConditionValue1);
                return false;
            }
            ...
            {
                LOG_ERROR("sql.sql",
                    "RelationTo condition has invalid ConditionValue2(RelationType) ({}), skipped",
                    cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_REACTION_TO:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "ReactionTo condition has invalid ConditionValue1(ConditionTarget selection) ({}), skipped",
                    cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue1 == cond->ConditionTarget)
            {
                LOG_ERROR("sql.sql",
                    "ReactionTo condition has ConditionValue1(ConditionTarget selection) set to self ({}), skipped",
                    cond->ConditionValue1);
                return false;
            }
            if (!cond->ConditionValue2)
            {
                LOG_ERROR("sql.sql",
                    "mConditionValue2 condition has invalid ConditionValue2(rankMask) ({}), skipped",
                    cond->ConditionValue2);
                return false;
            }
            break;
        ...
        case CONDITION_DISTANCE_TO:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "DistanceTo condition has invalid ConditionValue1(ConditionTarget selection) ({}), skipped",
                    cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue1 == cond->ConditionTarget)
            {
                LOG_ERROR("sql.sql",
                    "DistanceTo condition has ConditionValue1(ConditionTarget selection) set to self ({}), skipped",
                    cond->ConditionValue1);
                return false;
            }
            ...
            {
                LOG_ERROR(
                    "sql.sql", "DistanceTo condition has invalid ComparisionType ({}), skipped", cond->ConditionValue3);
                return false;
            }
            break;
        }
        ...
        case CONDITION_HP_VAL:
        {
            ...
            {
                LOG_ERROR(
                    "sql.sql", "HpVal condition has invalid ComparisionType ({}), skipped", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_HP_PCT:
        {
            ...
            {
                LOG_ERROR(
                    "sql.sql", "HpPct condition has invalid ComparisionType ({}), skipped", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
        ...
        case CONDITION_TITLE:
        {
            ...
            {
                LOG_ERROR(
                    "sql.sql", "Title condition has non existing title in value1 ({}), skipped", cond->ConditionValue1);
                return false;
            }
            break;
        }
        case CONDITION_SPAWNMASK:
        {
            ...
            {
                LOG_ERROR("condition",
                    "SpawnMask condition has non existing SpawnMask in value1 ({}), skipped",
                    cond->ConditionValue1);
                return false;
            }
            break;
        ...
        case CONDITION_UNIT_STATE:
        {
            ...
            {
                LOG_ERROR("condition",
                    "UnitState condition has non existing UnitState in value1 ({}), skipped",
                    cond->ConditionValue1);
                return false;
            }
            break;
        ...
        case CONDITION_CREATURE_TYPE:
        {
            ...
            {
                LOG_ERROR("condition",
                    "CreatureType condition has non existing CreatureType in value1 ({}), skipped",
                    cond->ConditionValue1);
                return false;
            }
            break;
        ...
        case CONDITION_REALM_ACHIEVEMENT:
        {
            ...
            {
                LOG_ERROR("condition",
                    "CONDITION_REALM_ACHIEVEMENT has non existing realm first achivement id ({}), skipped.",
                    cond->ConditionValue1);
                return false;
            }
            break;
        ...
        case CONDITION_QUEST_OBJECTIVE_PROGRESS:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "CONDITION_QUEST_OBJECTIVE_PROGRESS points to non-existing quest ({}), skipped.",
                    cond->ConditionValue1);
                return false;
            }
            ...
            {
                LOG_ERROR("sql.sql",
                    "CONDITION_QUEST_OBJECTIVE_PROGRESS has out-of-range quest objective index specified ({}), it must be a number between 0 and 3. skipped.",
                    cond->ConditionValue2);
                return false;
            }
            ...
            {
                LOG_ERROR("sql.sql",
                    "CONDITION_QUEST_OBJECTIVE_PROGRESS has quest objective {} for quest {}, but the field RequiredNPCOrGo{} is 0, skipped.",
                    cond->ConditionValue2,
                    cond->ConditionValue1,
                    cond->ConditionValue2);
                return false;
            ...
            {
                LOG_ERROR("sql.sql",
                    "CONDITION_QUEST_OBJECTIVE_PROGRESS has quest objective count {} in value3, but quest {} has a maximum objective count of {} in RequiredNPCOrGOCount{}, skipped.",
                    cond->ConditionValue3,
                    cond->ConditionValue2,
        ...
        case CONDITION_HAS_AURA_TYPE:
        {
            ...
            {
                LOG_ERROR(
                    "sql.sql", "Has Aura Effect condition has non existing aura ({}), skipped", cond->ConditionValue1);
                return false;
            }
            break;
        }
        case CONDITION_STAND_STATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "CONDITION_STAND_STATE has non-existing stand state ({},{}), skipped.",
                    cond->ConditionValue1,
                    cond->ConditionValue2);
                return false;
            }
        ...
        case CONDITION_DIFFICULTY_ID:
            ...
            {
                LOG_ERROR("sql.sql",
                    "CONDITION_DIFFICULTY_ID has non existing difficulty in value1 ({}), skipped.",
                    cond->ConditionValue1);
                return false;
            }
            break;
        case CONDITION_PET_TYPE:
            ...
            {
                LOG_ERROR(
                    "sql.sql", "CONDITION_PET_TYPE has non-existing pet type {}, skipped.", cond->ConditionValue1);
                return false;
            }
            break;
        case CONDITION_TAXI:
...
{
    for (ConditionReferenceContainer::iterator itr = ConditionReferenceStore.begin();
    ...
    for (CreatureSpellConditionContainer::iterator itr = VehicleSpellConditionStore.begin();
    ...
    for (SmartEventConditionContainer::iterator itr = SmartEventConditionStore.begin();
    ...
    for (CreatureSpellConditionContainer::iterator itr = SpellClickEventConditionStore.begin();
    ...
    for (NpcVendorConditionContainer::iterator itr = NpcVendorConditionContainerStore.begin();
    ...
    for (std::list<Condition*>::const_iterator itr = AllocatedMemoryStore.begin(); itr != AllocatedMemoryStore.end();
         ++itr)
        delete *itr;
