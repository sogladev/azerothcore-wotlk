        ...
        case SMART_ACTION_CAST:
        {
            ...
            for (WorldObject* target : targets)
            {
                ...
                else if (me)
                {
                    ...
                    bool canCastRangedWeaponSpell = spellInfo->IsRangedWeaponSpell() && !me->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISARMED) && (spellInfo->CalcPowerCost(me, spellInfo->GetSchoolMask()) == 0);
                    ...
                    if (isWithinLOSInMap && isWithinMeleeRange && isRangedAttack && isTargetRooted && (canCastSpell || canCastRangedWeaponSpell) && (!isInstantCast || isRangedAttack))
                    {
