...
void SmartWaypointMgr::LoadFromDB()
{
    ...
    do
    {
        ...
        if (last_id != id)
            LOG_ERROR("sql.sql",
                "SmartWaypointMgr::LoadFromDB: Path entry {}, unexpected point id {}, expected {}.",
                entry,
                id,
                last_id);
...
void SmartAIMgr::LoadSmartAIFromDB()
{
    ...
    do
    {
        ...
        if (temp.entryOrGuid >= 0)
        {
            switch (source_type)
            {
                case SMART_SCRIPT_TYPE_CREATURE:
                {
                    ...
                    {
                        LOG_ERROR("sql.sql",
                            "SmartAIMgr::LoadSmartAIFromDB: Creature entry ({}) does not exist, skipped loading.",
                ...
                case SMART_SCRIPT_TYPE_GAMEOBJECT:
                {
                    ...
                    {
                        LOG_ERROR("sql.sql",
                            "SmartAIMgr::LoadSmartAIFromDB: GameObject entry ({}) does not exist, skipped loading.",
                            uint32(temp.entryOrGuid));
                        continue;
                    }
                    break;
                ...
                case SMART_SCRIPT_TYPE_AREATRIGGER:
                {
                    ...
                    {
                        LOG_ERROR("sql.sql",
                            "SmartAIMgr::LoadSmartAIFromDB: AreaTrigger entry ({}) does not exist, skipped loading.",
                            uint32(temp.entryOrGuid));
                        continue;
                    }
                    break;
        ...
        else if (!sObjectMgr->GetCreatureData(uint32(std::abs(temp.entryOrGuid))))
        {
            LOG_ERROR("sql.sql",
                "SmartAIMgr::LoadSmartAIFromDB: Creature guid ({}) does not exist, skipped loading.",
        ...
        if (temp.action.type == SMART_ACTION_MOVE_TO_POS)
            if (temp.target.type == SMART_TARGET_SELF &&
                (std::fabs(temp.target.x) > 200.0f || std::fabs(temp.target.y) > 200.0f ||
...
void SmartAIMgr::CheckIfSmartAIInDatabaseExists()
{
    ...
    for (auto const& [entry, creatureTemplate] : *sObjectMgr->GetCreatureTemplates())
    {
        ...
        // check template SAI
        if (mEventMap[uint32(SmartScriptType::SMART_SCRIPT_TYPE_CREATURE)].find(creatureTemplate.Entry) !=
            mEventMap[uint32(SmartScriptType::SMART_SCRIPT_TYPE_CREATURE)].end())
            found = true;
        else
        {
            // check GUID SAI
            for (auto const& pair : sObjectMgr->GetAllCreatureData())
            {
                ...
                if (mEventMap[uint32(SmartScriptType::SMART_SCRIPT_TYPE_CREATURE)].find((-1) * pair.first) !=
                    mEventMap[uint32(SmartScriptType::SMART_SCRIPT_TYPE_CREATURE)].end())
                {
                    found = true;
                    break;
        ...
        if (!found)
            LOG_ERROR("sql.sql",
                "Creature entry ({}) has SmartAI enabled but no SmartAI entries in the database.",
    ...
    for (auto const& [entry, gameobjectTemplate] : *sObjectMgr->GetGameObjectTemplates())
    {
        ...
        // check template SAI
        if (mEventMap[uint32(SmartScriptType::SMART_SCRIPT_TYPE_GAMEOBJECT)].find(gameobjectTemplate.entry) !=
            mEventMap[uint32(SmartScriptType::SMART_SCRIPT_TYPE_GAMEOBJECT)].end())
            found = true;
        else
        {
            // check GUID SAI
            for (auto const& pair : sObjectMgr->GetAllGOData())
            {
                ...
                if (mEventMap[uint32(SmartScriptType::SMART_SCRIPT_TYPE_GAMEOBJECT)].find((-1) * pair.first) !=
                    mEventMap[uint32(SmartScriptType::SMART_SCRIPT_TYPE_GAMEOBJECT)].end())
                {
                    found = true;
                    break;
        ...
        if (!found)
            LOG_ERROR("sql.sql",
                "Gameobject entry ({}) has SmartGameobjectAI enabled but no SmartAI entries in the database.",
    ...
    for (auto const& pair : sObjectMgr->GetAllAreaTriggerScriptData())
    {
        ...
        if (mEventMap[uint32(SmartScriptType::SMART_SCRIPT_TYPE_AREATRIGGER)].find(pair.first) ==
            mEventMap[uint32(SmartScriptType::SMART_SCRIPT_TYPE_AREATRIGGER)].end())
            LOG_ERROR("sql.sql",
                "AreaTrigger entry ({}) has SmartTrigger enabled but no SmartAI entries in the database.",
                pair.first);
    }
}
...
bool SmartAIMgr::IsTargetValid(SmartScriptHolder const& e)
{
    ...
    switch (e.GetTargetType())
    {
        ...
        case SMART_TARGET_CREATURE_RANGE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Creature entry {} as target_param1, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType(),
        ...
        case SMART_TARGET_GAMEOBJECT_RANGE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent GameObject entry {} as target_param1, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType(),
        ...
        case SMART_TARGET_CLOSEST_PLAYER:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has maxDist 0 as target_param1, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType());
        ...
        case SMART_TARGET_INVOKER_PARTY:
            if (e.GetScriptType() != SMART_SCRIPT_TYPE_TIMED_ACTIONLIST && e.GetEventType() != SMART_EVENT_LINK &&
                !EventHasInvoker(e.event.type))
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has invoker target, but event does not provide any invoker!",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.GetEventType(),
                    e.GetActionType());
        ...
        case SMART_TARGET_INSTANCE_STORAGE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has invalid instance storage type as target ({}).",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.GetEventType(),
                    e.GetActionType(),
        ...
        default:
            LOG_ERROR("sql.sql",
                "SmartAIMgr: Not handled target_type({}), Entry {} SourceType {} Event {} Action {}, skipped.",
                e.GetTargetType(),
                e.entryOrGuid,
                e.GetScriptType(),
                e.event_id,
...
bool SmartAIMgr::CheckUnusedEventParams(SmartScriptHolder const& e)
{
    std::size_t paramsStructSize = [&]() -> std::size_t
    {
        ...
        switch (e.event.type)
        {
            ...
            case SMART_EVENT_WAYPOINT_DATA_REACHED:
                return sizeof(SmartEvent::wpData);
            case SMART_EVENT_WAYPOINT_DATA_ENDED:
                return sizeof(SmartEvent::wpData);
            default:
                LOG_WARN("sql.sql",
                    "SmartAIMgr: entryorguid {} source_type {} id {} action_type {} is using an event {} with no unused params specified in SmartAIMgr::CheckUnusedEventParams(), please report this.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType(),
    ...
    for (std::size_t index = paramsCount; index < rawCount; index++)
    {
        ...
        {
            LOG_ERROR("sql.sql",
                "SmartAIMgr: entryorguid {} source_type {} id {} action_type {} has unused event_param{} with value {}, it must be 0, skipped.",
                e.entryOrGuid,
                e.GetScriptType(),
                e.event_id,
                e.GetActionType(),
...
bool SmartAIMgr::CheckUnusedActionParams(SmartScriptHolder const& e)
{
    std::size_t paramsStructSize = [&]() -> std::size_t
    {
        ...
        switch (e.action.type)
        {
            ...
            case SMART_ACTION_SCRIPTED_SPAWN:
                return sizeof(SmartAction::scriptSpawn);
            ...
            case SMART_ACTION_WAYPOINT_DATA_START:
                return sizeof(SmartAction::wpData);
            case SMART_ACTION_WAYPOINT_DATA_RANDOM:
                return sizeof(SmartAction::wpDataRandom);
            case SMART_ACTION_MOVEMENT_STOP:
                return NO_PARAMS;
            case SMART_ACTION_MOVEMENT_PAUSE:
                return sizeof(SmartAction::move);
            case SMART_ACTION_MOVEMENT_RESUME:
                return sizeof(SmartAction::move);
            case SMART_ACTION_WORLD_SCRIPT:
                return sizeof(SmartAction::worldStateScript);
            default:
                LOG_WARN("sql.sql",
                    "SmartAIMgr: entryorguid {} source_type {} id {} action_type {} is using an action with no unused params specified in SmartAIMgr::CheckUnusedActionParams(), please report this.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType());
    ...
    for (std::size_t index = paramsCount; index < rawCount; index++)
    {
        ...
        {
            LOG_ERROR("sql.sql",
                "SmartAIMgr: entryorguid {} source_type {} id {} action_type {} has unused action_param{} with value {}, it must be 0, skipped.",
                e.entryOrGuid,
                e.GetScriptType(),
                e.event_id,
                e.GetActionType(),
...
bool SmartAIMgr::CheckUnusedTargetParams(SmartScriptHolder const& e)
{
    std::size_t paramsStructSize = [&]() -> std::size_t
    {
        ...
        switch (e.target.type)
        {
            ...
            default:
                LOG_WARN("sql.sql",
                    "SmartAIMgr: entryorguid {} source_type {} id {} action_type {} is using a target {} with no unused params specified in SmartAIMgr::CheckUnusedTargetParams(), please report this.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType(),
    ...
    for (std::size_t index = paramsCount; index < rawCount; index++)
    {
        ...
        {
            LOG_ERROR("sql.sql",
                "SmartAIMgr: entryorguid {} source_type {} id {} action_type {} has unused target_param{} with value {}, it must be 0, skipped.",
                e.entryOrGuid,
                e.GetScriptType(),
                e.event_id,
                e.GetActionType(),
...
{
    if ((e.event.type >= SMART_EVENT_TC_END && e.event.type <= SMART_EVENT_AC_START) ||
        e.event.type >= SMART_EVENT_AC_END)
    {
        LOG_ERROR("sql.sql",
            "SmartAIMgr: EntryOrGuid {} using event({}) has invalid event type ({}), skipped.",
            e.entryOrGuid,
            e.event_id,
            e.GetEventType());
        return false;
    ...
    if (e.GetScriptType() != SMART_SCRIPT_TYPE_TIMED_ACTIONLIST &&
        !(SmartAIEventMask[e.event.type][1] & SmartAITypeMask[e.GetScriptType()][1]))
    {
        LOG_ERROR("sql.sql",
            "SmartAIMgr: EntryOrGuid {}, event type {} can not be used for Script type {}",
            e.entryOrGuid,
            e.GetEventType(),
            e.GetScriptType());
        return false;
    }
    if (e.action.type <= 0 || (e.action.type >= SMART_ACTION_TC_END && e.action.type <= SMART_ACTION_AC_START) ||
        e.action.type >= SMART_ACTION_AC_END)
    {
        LOG_ERROR("sql.sql",
            "SmartAIMgr: EntryOrGuid {} using event({}) has an invalid action type ({}), skipped.",
            e.entryOrGuid,
            e.event_id,
            e.GetActionType());
        return false;
    ...
    switch (e.action.type)
    {
        ...
        case SMART_ACTION_SCENE_CANCEL:
            LOG_ERROR("sql.sql",
                "SmartAIMgr: EntryOrGuid {} using event({}) has an action type that is not supported on 3.3.5a ({}), skipped.",
                e.entryOrGuid,
                e.event_id,
                e.GetActionType());
            return false;
        ...
        case SMART_ACTION_RESPAWN_BY_SPAWNID:
            LOG_ERROR("sql.sql",
                "SmartAIMgr: EntryOrGuid {} using event({}) has an action type that is not yet supported on AzerothCore ({}), skipped.",
                e.entryOrGuid,
                e.event_id,
                e.GetActionType());
            return false;
    ...
    if (e.target.type < 0 || (e.target.type >= SMART_TARGET_TC_END && e.target.type < SMART_TARGET_AC_START) ||
        e.target.type >= SMART_TARGET_AC_END)
    {
        LOG_ERROR("sql.sql",
            "SmartAIMgr: EntryOrGuid {} using event({}) has an invalid target type ({}), skipped.",
            e.entryOrGuid,
            e.event_id,
            e.GetTargetType());
        return false;
    ...
    {
        LOG_ERROR("sql.sql",
            "SmartAIMgr: EntryOrGuid {} using event({}) has a target type that is not yet supported on AzerothCore ({}), skipped.",
            e.entryOrGuid,
            e.event_id,
            e.GetTargetType());
        return false;
    ...
    {
        LOG_ERROR("sql.sql",
            "SmartAIMgr: EntryOrGuid {} using event({}) has invalid phase mask ({}), skipped.",
            e.entryOrGuid,
            e.event_id,
    ...
    {
        LOG_ERROR("sql.sql",
            "SmartAIMgr: EntryOrGuid {} using event({}) has invalid event flags ({}), skipped.",
            e.entryOrGuid,
            e.event_id,
            e.event.event_flags);
        return false;
        ...
        switch (type)
        {
            ...
            case SMART_EVENT_SPELLHIT_TARGET:
                if (e.event.spellHit.spell)
                {
                    ...
                    {
                        LOG_ERROR("sql.sql",
                            "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Spell entry {}, skipped.",
                            e.entryOrGuid,
                            e.GetScriptType(),
                            e.event_id,
                            e.GetActionType(),
                    ...
                    if (e.event.spellHit.school &&
                        (e.event.spellHit.school & spellInfo->SchoolMask) != spellInfo->SchoolMask)
                    {
                        LOG_ERROR("sql.sql",
                            "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses Spell entry {} with invalid school mask, skipped.",
                            e.entryOrGuid,
                            e.GetScriptType(),
                            e.event_id,
                            e.GetActionType(),
            ...
            case SMART_EVENT_IC_LOS:
                ...
                if (e.event.los.hostilityMode >= AsUnderlyingType(SmartEvent::LOSHostilityMode::End))
                {
                    LOG_ERROR("sql.sql",
                        ...
                        AsUnderlyingType(SmartEvent::LOSHostilityMode::End) - 1);
                    return false;
                }
            ...
            case SMART_EVENT_RESPAWN:
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Map entry {}, skipped.",
                        e.entryOrGuid,
                        e.GetScriptType(),
                        e.event_id,
                        e.GetActionType(),
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Area entry {}, skipped.",
                        e.entryOrGuid,
                        e.GetScriptType(),
                        e.event_id,
                        e.GetActionType(),
            ...
            case SMART_EVENT_MOVEMENTINFORM:
            {
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses invalid Motion type {}, skipped.",
                        e.entryOrGuid,
                        e.GetScriptType(),
                        e.event_id,
                        e.GetActionType(),
            ...
            case SMART_EVENT_LINK:
            {
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Entry {} SourceType {}, Event {}, Link Event is linking self (infinite loop), skipped.",
                        e.entryOrGuid,
                        e.GetScriptType(),
                        e.event_id);
                    return false;
            ...
            case SMART_EVENT_EVENT_PHASE_CHANGE:
            {
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has no param set, event won't be executed!.",
                        e.entryOrGuid,
                        e.GetScriptType(),
                        e.event_id,
                        e.GetActionType());
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses invalid phasemask {}, skipped.",
                        e.entryOrGuid,
                        e.GetScriptType(),
                        e.event_id,
                        e.GetActionType(),
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses event phasemask {} and incompatible event_param1 {}, skipped.",
                        e.entryOrGuid,
                        e.GetScriptType(),
                        e.event_id,
                        e.GetActionType(),
            ...
            case SMART_EVENT_ACTION_DONE:
            {
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses invalid event id {}, skipped.",
                        e.entryOrGuid,
                        e.GetScriptType(),
                        e.event_id,
                        e.GetActionType(),
            ...
            case SMART_EVENT_FRIENDLY_HEALTH_PCT:
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has pct value above 100, skipped.",
                        e.entryOrGuid,
                        e.GetScriptType(),
                        e.event_id,
                        e.GetActionType());
                ...
                switch (e.GetTargetType())
                {
                    ...
                    default:
                        LOG_ERROR("sql.sql",
                            "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses invalid target_type {}, skipped.",
                            e.entryOrGuid,
                            e.GetScriptType(),
                            e.event_id,
                            e.GetActionType(),
            ...
            case SMART_EVENT_DISTANCE_CREATURE:
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Event SMART_EVENT_DISTANCE_CREATURE using invalid creature guid {}, skipped.",
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Event SMART_EVENT_DISTANCE_CREATURE using invalid creature entry {}, skipped.",
            ...
            case SMART_EVENT_DISTANCE_GAMEOBJECT:
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Event SMART_EVENT_DISTANCE_GAMEOBJECT using invalid gameobject guid {}, skipped.",
                        e.event.distance.guid);
                    return false;
                }
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Event SMART_EVENT_DISTANCE_GAMEOBJECT using invalid gameobject entry {}, skipped.",
                        e.event.distance.entry);
                    return false;
                }
                break;
            case SMART_EVENT_COUNTER_SET:
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Event SMART_EVENT_COUNTER_SET using invalid counter id {}, skipped.",
                        e.event.counter.id);
                    return false;
                }
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Event SMART_EVENT_COUNTER_SET using invalid value {}, skipped.",
            ...
            default:
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Not handled event_type({}), Entry {} SourceType {} Event {} Action {}, skipped.",
                    e.GetEventType(),
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
    ...
    switch (e.GetActionType())
    {
        case SMART_ACTION_SET_FACTION:
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Faction {}, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType(),
        ...
        case SMART_ACTION_MOUNT_TO_ENTRY_OR_MODEL:
            if (e.action.morphOrMount.creature || e.action.morphOrMount.model)
            {
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Creature entry {}, skipped.",
                        e.entryOrGuid,
                        e.GetScriptType(),
                        e.event_id,
                        e.GetActionType(),
                ...
                if (e.action.morphOrMount.model)
                {
                    ...
                    {
                        LOG_ERROR("sql.sql",
                            "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has ModelID set with also set CreatureId, skipped.",
                            e.entryOrGuid,
                            e.GetScriptType(),
                            e.event_id,
                            e.GetActionType());
                    ...
                    {
                        LOG_ERROR("sql.sql",
                            "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Model id {}, skipped.",
                            e.entryOrGuid,
                            e.GetScriptType(),
                            e.event_id,
                            e.GetActionType(),
        ...
        case SMART_ACTION_ACTIVATE_TAXI:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses invalid Taxi path ID {}, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType(),
        ...
        case SMART_ACTION_RANDOM_EMOTE:
        {
            if (std::all_of(e.action.randomEmote.emotes.begin(), e.action.randomEmote.emotes.end(), [](uint32 emote) {
                return emote == 0;
            }))
            {
                LOG_ERROR("sql.sql",
        ...
        case SMART_ACTION_INVOKER_CAST:
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} has invoker cast action, but event does not provide any invoker!",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.GetEventType(),
                    e.GetActionType());
        ...
        case SMART_ACTION_CALL_GROUPEVENTHAPPENS:
            if (Quest const* qid = sObjectMgr->GetQuestTemplate(e.action.quest.quest))
            {
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Entry {} SourceType {} Event {} Action {} SpecialFlags for Quest entry {} does not include FLAGS_EXPLORATION_OR_EVENT(2), skipped.",
                        e.entryOrGuid,
                        e.GetScriptType(),
                        e.event_id,
                        e.GetActionType(),
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Quest entry {}, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType(),
        ...
        case SMART_ACTION_SET_EVENT_PHASE:
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} attempts to set phase {}. Phase mask cannot be used past phase {}, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType(),
        ...
        case SMART_ACTION_INC_EVENT_PHASE:
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} is incrementing phase by 0, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType());
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} attempts to increment phase by too large value, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType());
        ...
        case SMART_ACTION_RANDOM_PHASE:
        {
            if (std::all_of(e.action.randomPhase.phases.begin(), e.action.randomPhase.phases.end(), [](uint32 phase) {
                return phase == 0;
            }))
            {
                LOG_ERROR("sql.sql",
            ...
            if (std::any_of(e.action.randomPhase.phases.begin(), e.action.randomPhase.phases.end(), [](uint32 phase) {
                return phase >= SMART_EVENT_PHASE_MAX;
            }))
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} attempts to set invalid phase, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType());
        ...
        case SMART_ACTION_SUMMON_CREATURE:
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses incorrect TempSummonType {}, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType(),
        ...
        case SMART_ACTION_SET_SHEATH:
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses incorrect Sheath state {}, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType(),
        ...
        case SMART_ACTION_SET_REACT_STATE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Creature {} Event {} Action {} uses invalid React State {}, skipped.",
                    e.entryOrGuid,
                    e.event_id,
                    e.GetActionType(),
        ...
        case SMART_ACTION_INSTALL_AI_TEMPLATE:
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Creature {} Event {} Action {} uses non-existent AI template id {}, skipped.",
                    e.entryOrGuid,
                    e.event_id,
                    e.GetActionType(),
        ...
        case SMART_ACTION_WP_START:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Creature {} Event {} Action {} uses non-existent WaypointPath id {}, skipped.",
                    e.entryOrGuid,
                    e.event_id,
                    e.GetActionType(),
        ...
        {
            if (!IsMinMaxValid(
                    e, e.action.randTimedActionList.actionLists[0], e.action.randTimedActionList.actionLists[1]))
                return false;
            break;
        }
        case SMART_ACTION_SET_POWER:
        ...
        case SMART_ACTION_REMOVE_POWER:
            ...
            {
                LOG_ERROR("scripts.ai.sai",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent Power {}, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType(),
        ...
        case SMART_ACTION_GAME_EVENT_STOP:
        {
            ...
            {
                LOG_ERROR("scripts.ai.sai",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses non-existent event, eventId {}, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType(),
        ...
        case SMART_ACTION_EQUIP:
        {
            if (e.GetScriptType() == SMART_SCRIPT_TYPE_CREATURE)
            {
                if (int8 equipId = static_cast<int8>(e.action.equip.entry))
                {
                    ...
                    {
                        LOG_ERROR("scripts.ai.sai",
                            "SmartScript: SMART_ACTION_EQUIP uses non-existent equipment info id {} for creature {}, skipped.",
                            equipId,
                            e.entryOrGuid);
                        return false;
                    }
        ...
        case SMART_ACTION_LOAD_GRID:
        {
            ...
            {
                LOG_ERROR("scripts.ai.sai",
                    "SmartScript: SMART_ACTION_LOAD_GRID uses invalid map coords: {}, skipped.",
                    e.entryOrGuid);
                return false;
            }
            break;
        ...
        case SMART_ACTION_SET_IN_COMBAT_WITH_ZONE:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartScript: action_type {} is not allowed with source_type {}. Entry {}, skipped.",
                    e.GetActionType(),
                    e.GetScriptType(),
                    e.entryOrGuid);
                return false;
        ...
        case SMART_ACTION_SET_INST_DATA:
        {
            if (e.action.setInstanceData.type == 1)
            {
                ...
                {
                    LOG_ERROR("sql.sql",
                        "SmartScript: SMART_ACTION_SET_INST_DATA with type 1 (bossState) uses invalid encounter state {}. Source entry {}, type {}",
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartScript: SMART_ACTION_SET_INST_DATA uses unsupported data type {}. Source entry {}, type {}",
        ...
        case SMART_ACTION_SET_MOVEMENT_SPEED:
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses invalid movementType {}, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType(),
            ...
            {
                LOG_ERROR("sql.sql",
                    "SmartAIMgr: Entry {} SourceType {} Event {} Action {} uses speed 0, skipped.",
                    e.entryOrGuid,
                    e.GetScriptType(),
                    e.event_id,
                    e.GetActionType());
        ...
        default:
            LOG_ERROR("sql.sql",
                "SmartAIMgr: Not handled action_type({}), event_type({}), Entry {} SourceType {} Event {}, skipped.",
                e.GetActionType(),
                e.GetEventType(),
                e.entryOrGuid,
                e.GetScriptType(),
...
bool SmartAIMgr::IsTextValid(SmartScriptHolder const& e, uint32 id)
{
        ...
        switch (e.GetTargetType())
        {
            ...
            default:
                if (e.entryOrGuid < 0)
                {
                    ...
                    {
                        LOG_ERROR("sql.sql",
                            "SmartAIMgr: Entry {} SourceType {} Event {} Action {} using non-existent Creature guid {}, skipped.",
                            e.entryOrGuid,
                            e.GetScriptType(),
                            e.event_id,
                            e.GetActionType(),
    ...
    {
        LOG_ERROR("sql.sql",
            "SmartAIMgr: Entry {} SourceType {} Event {} Action {} using non-existent Text id {}, skipped.",
            e.entryOrGuid,
            e.GetScriptType(),
            e.event_id,
            e.GetActionType(),
