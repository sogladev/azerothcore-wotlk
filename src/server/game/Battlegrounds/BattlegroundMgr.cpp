...
void BattlegroundMgr::Update(uint32 diff)
{
    ...
    if (!m_QueueUpdateScheduler.empty())
    {
        ...
        for (uint8 i = 0; i < scheduled.size(); i++)
        {
            ...
            m_BattlegroundQueues[bgQueueTypeId].BattlegroundQueueUpdate(
                diff, bgTypeId, bracket_id, arenaType, arenaMMRating > 0, arenaMMRating);
    ...
    if (m_NextPeriodicQueueUpdateTime < diff)
    {
        ...
        for (uint32 qtype = BATTLEGROUND_QUEUE_2v2; qtype < MAX_BATTLEGROUND_QUEUE_TYPES; ++qtype)
        {
            ...
            {
                m_BattlegroundQueues[qtype].BattlegroundQueueUpdate(m_NextPeriodicQueueUpdateTime,
                    BATTLEGROUND_AA,
        ...
        for (uint32 qtype = BATTLEGROUND_QUEUE_AV; qtype < MAX_BATTLEGROUND_QUEUE_TYPES; ++qtype)
        {
            for (uint32 bracket = BG_BRACKET_ID_FIRST; bracket < MAX_BATTLEGROUND_BRACKETS; ++bracket)
            {
                m_BattlegroundQueues[qtype].BattlegroundQueueAnnouncerUpdate(
                    m_NextPeriodicQueueUpdateTime, BattlegroundQueueTypeId(qtype), BattlegroundBracketId(bracket));
            }
        }
    }
    else
        ...
        if (m_AutoDistributionTimeChecker < diff)
        {
            if (GameTime::GetGameTime() > m_NextAutoDistributionTime)
            {
                ...
                m_NextAutoDistributionTime =
                    ...
                    Seconds(DAY * sWorld->getIntConfig(CONFIG_ARENA_AUTO_DISTRIBUTE_INTERVAL_DAYS));
...
void BattlegroundMgr::BuildBattlegroundStatusPacket(WorldPacket* data, Battleground* bg, uint8 QueueSlot,
    uint8 StatusID, uint32 Time1, uint32 Time2, uint8 arenatype, TeamId teamId, bool isRated,
...
Battleground* BattlegroundMgr::CreateNewBattleground(
    BattlegroundTypeId originalBgTypeId, PvPDifficultyEntry const* bracketEntry, uint8 arenaType, bool isRated)
{
    ...
    do
    {
        ...
        if (!bl)
        {
            LOG_ERROR("bg.battleground",
                "Battleground ID {} not found in BattlemasterList.dbc. Battleground not created.",
                bgTypeId);
            continue;
        }
        ...
        {
            LOG_ERROR("sql.sql",
                "Table `battleground_template` for id {} has bad values for LevelMin ({}) and LevelMax({})",
                bgTemplate.Id,
                bgTemplate.MinLevel,
                bgTemplate.MaxLevel);
            continue;
        ...
        if (bgTemplate.Id != BATTLEGROUND_AA && bgTemplate.Id != BATTLEGROUND_RB)
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "Table `battleground_template` for id {} contains a non-existing WorldSafeLocs.dbc id {} in field `AllianceStartLoc`. BG not created.",
                    bgTemplate.Id,
                    startId);
                continue;
            }
            ...
            {
                LOG_ERROR("sql.sql",
                    "Table `battleground_template` for id {} contains a non-existing WorldSafeLocs.dbc id {} in field `HordeStartLoc`. BG not created.",
                    bgTemplate.Id,
                    startId);
                continue;
            }
...
void BattlegroundMgr::BuildBattlegroundListPacket(
    WorldPacket* data, ObjectGuid guid, Player* player, BattlegroundTypeId bgTypeId, uint8 fromWhere)
{
    if (!player)
        return;

    uint32 winner_kills = player->GetRandomWinner() ? sWorld->getIntConfig(CONFIG_BG_REWARD_WINNER_HONOR_LAST)
    ...
    else                             // battleground
    {
        ...
        *data << uint32(0); // number of bg instances
        ...
        if (it != bgDataStore.end())
        {
            ...
            if (PvPDifficultyEntry const* bracketEntry = GetBattlegroundBracketByLevel(
                    it->second._Battlegrounds.begin()->second->GetMapId(), player->GetLevel()))
            {
                uint32 count = 0;
...
void BattlegroundMgr::SendToBattleground(Player* player, uint32 instanceId, BattlegroundTypeId bgTypeId)
{
    if (Battleground* bg = GetBattleground(instanceId, bgTypeId))
    {
        ...
        LOG_DEBUG("bg.battleground",
            "BattlegroundMgr::SendToBattleground: Sending {} to map {}, {} (bgType {})",
            player->GetName(),
            mapid,
            pos->ToString(),
            bgTypeId);
    ...
    {
        LOG_ERROR("bg.battleground",
            "BattlegroundMgr::SendToBattleground: Instance {} (bgType {}) not found while trying to teleport player {}",
            instanceId,
            bgTypeId,
            player->GetName());
    }
...
void BattlegroundMgr::ScheduleQueueUpdate(uint32 arenaMatchmakerRating, uint8 arenaType,
    BattlegroundQueueTypeId bgQueueTypeId, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id)
{
    //This method must be atomic, @todo add mutex
    //we will use only 1 number created of bgTypeId and bracket_id
    uint64 const scheduleId = ((uint64)arenaMatchmakerRating << 32) | ((uint64)arenaType << 24) |
                              ((uint64)bgQueueTypeId << 16) | ((uint64)bgTypeId << 8) | (uint64)bracket_id;
    if (std::find(m_QueueUpdateScheduler.begin(), m_QueueUpdateScheduler.end(), scheduleId) ==
    ...
    do
    {
        ...
        if (CreatureTemplate const* cInfo = sObjectMgr->GetCreatureTemplate(entry))
        {
            if ((cInfo->npcflag & UNIT_NPC_FLAG_BATTLEMASTER) == 0)
                LOG_ERROR(
                    "sql.sql", "Creature (Entry: {}) listed in `battlemaster_entry` is not a battlemaster.", entry);
        }
        else
        {
        ...
        {
            LOG_ERROR("sql.sql",
                "Table `battlemaster_entry` contain entry {} for not existed battleground type {}, ignored.",
                entry,
                bgTypeId);
            continue;
        }
...
void BattlegroundMgr::CheckBattleMasters()
{
    ...
    for (CreatureTemplateContainer::const_iterator itr = ctc->begin(); itr != ctc->end(); ++itr)
    {
        if ((itr->second.npcflag & UNIT_NPC_FLAG_BATTLEMASTER) &&
            mBattleMastersMap.find(itr->second.Entry) == mBattleMastersMap.end())
        {
            LOG_ERROR("sql.sql",
                "CreatureTemplate (Entry: {}) has UNIT_NPC_FLAG_BATTLEMASTER but no data in `battlemaster_entry` table. Removing flag!",
                itr->second.Entry);
