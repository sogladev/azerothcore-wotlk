...
BattlegroundAB::BattlegroundAB()
{
    ...
    BgCreatures.resize(
        BG_AB_ALL_NODES_COUNT + BG_AB_DYNAMIC_NODES_COUNT); // xinef: +BG_AB_DYNAMIC_NODES_COUNT buff triggers
        ...
        while (uint32 eventId = _bgEvents.ExecuteEvent())
            switch (eventId)
            {
                ...
                case BG_AB_EVENT_CAPTURE_GOLDMINE:
                {
                    ...
                    TeamId teamId =
                        _capturePointInfo[node]._state == BG_AB_NODE_STATE_ALLY_CONTESTED ? TEAM_ALLIANCE : TEAM_HORDE;
                    DeleteBanner(node);
                    _capturePointInfo[node]._ownerTeamId = teamId;
                    _capturePointInfo[node]._state =
                        teamId == TEAM_ALLIANCE ? BG_AB_NODE_STATE_ALLY_OCCUPIED : BG_AB_NODE_STATE_HORDE_OCCUPIED;
                ...
                case BG_AB_EVENT_HORDE_TICK:
                {
                    ...
                    UpdateWorldState(teamId == TEAM_ALLIANCE ? BG_AB_OP_RESOURCES_ALLY : BG_AB_OP_RESOURCES_HORDE,
                        m_TeamScores[teamId]);
...
void BattlegroundAB::StartingEventCloseDoors()
{
    for (uint32 obj = BG_AB_OBJECT_BANNER_NEUTRAL;
         obj < static_cast<uint8>(BG_AB_DYNAMIC_NODES_COUNT) * BG_AB_OBJECTS_PER_NODE;
         ++obj)
...
{
    for (uint32 banner = BG_AB_OBJECT_BANNER_NEUTRAL, i = 0; i < BG_AB_DYNAMIC_NODES_COUNT;
...
void BattlegroundAB::CreateBanner(uint8 node, bool delay)
{
    ...
    SpawnBGObject(node * BG_AB_OBJECTS_PER_NODE + BG_AB_OBJECT_AURA_ALLY + _capturePointInfo[node]._ownerTeamId,
        RESPAWN_IMMEDIATELY);
}
...
void BattlegroundAB::DeleteBanner(uint8 node)
{
    ...
    SpawnBGObject(
        node * BG_AB_OBJECTS_PER_NODE + BG_AB_OBJECT_AURA_ALLY + _capturePointInfo[node]._ownerTeamId, RESPAWN_ONE_DAY);
}
...
void BattlegroundAB::NodeOccupied(uint8 node)
{
    ...
    {
        trigger->SetFaction(
            _capturePointInfo[node]._ownerTeamId == TEAM_ALLIANCE ? FACTION_ALLIANCE_GENERIC : FACTION_HORDE_GENERIC);
...
void BattlegroundAB::EventPlayerClickedOnFlag(Player* player, GameObject* gameObject)
{
    ...
    else if (_capturePointInfo[node]._state == BG_AB_NODE_STATE_ALLY_CONTESTED ||
             _capturePointInfo[node]._state == BG_AB_NODE_STATE_HORDE_CONTESTED)
    {
        ...
        sound = player->GetTeamId() == TEAM_ALLIANCE ? BG_AB_SOUND_NODE_ASSAULTED_ALLIANCE
...
TeamId BattlegroundAB::GetPrematureWinner()
{
    ...
    return _controlledPoints[TEAM_HORDE] > _controlledPoints[TEAM_ALLIANCE] ? TEAM_HORDE
...
GraveyardStruct const* BattlegroundAB::GetClosestGraveyard(Player* player)
{
    GraveyardStruct const* entry =
        sGraveyard->GetGraveyard(BG_AB_GraveyardIds[static_cast<uint8>(BG_AB_SPIRIT_ALIANCE) + player->GetTeamId()]);
    ...
    switch (type)
    {
        case SCORE_BASES_ASSAULTED:
            player->UpdateAchievementCriteria(
                ACHIEVEMENT_CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, BG_AB_OBJECTIVE_ASSAULT_BASE);
            break;
        case SCORE_BASES_DEFENDED:
            player->UpdateAchievementCriteria(
                ACHIEVEMENT_CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, BG_AB_OBJECTIVE_DEFEND_BASE);
            break;
        default:
            break;
    }
