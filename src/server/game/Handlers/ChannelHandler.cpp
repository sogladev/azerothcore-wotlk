...
void WorldSession::HandleLeaveChannel(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("chat.system", "CMSG_LEAVE_CHANNEL {} Channel: {}, unk1: {}", GetPlayerInfo(), channelName, unk);
    if (channelName.empty())
        return;
...
void WorldSession::HandleChannelPassword(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG(
        "chat.system", "CMSG_CHANNEL_PASSWORD {} Channel: {}, Password: {}", GetPlayerInfo(), channelName, password);
...
void WorldSession::HandleChannelSetOwner(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG(
        "chat.system", "CMSG_CHANNEL_SET_OWNER {} Channel: {}, Target: {}", GetPlayerInfo(), channelName, targetName);
...
void WorldSession::HandleChannelOwner(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("chat.system", "CMSG_CHANNEL_OWNER {} Channel: {}", GetPlayerInfo(), channelName);
    if (ChannelMgr* cMgr = ChannelMgr::forTeam(GetPlayer()->GetTeamId()))
        if (Channel* channel = cMgr->GetChannel(channelName, GetPlayer()))
...
void WorldSession::HandleChannelModerator(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG(
        "chat.system", "CMSG_CHANNEL_MODERATOR {} Channel: {}, Target: {}", GetPlayerInfo(), channelName, targetName);
    if (!normalizePlayerName(targetName))
        return;
...
void WorldSession::HandleChannelUnmoderator(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG(
        "chat.system", "CMSG_CHANNEL_UNMODERATOR {} Channel: {}, Target: {}", GetPlayerInfo(), channelName, targetName);
    if (!normalizePlayerName(targetName))
        return;
...
void WorldSession::HandleChannelMute(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("chat.system", "CMSG_CHANNEL_MUTE {} Channel: {}, Target: {}", GetPlayerInfo(), channelName, targetName);
    if (!normalizePlayerName(targetName))
        return;
...
void WorldSession::HandleChannelUnmute(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG(
        "chat.system", "CMSG_CHANNEL_UNMUTE {} Channel: {}, Target: {}", GetPlayerInfo(), channelName, targetName);
    if (!normalizePlayerName(targetName))
        return;
...
void WorldSession::HandleChannelInvite(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG(
        "chat.system", "CMSG_CHANNEL_INVITE {} Channel: {}, Target: {}", GetPlayerInfo(), channelName, targetName);
    if (!normalizePlayerName(targetName))
        return;
...
void WorldSession::HandleChannelKick(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("chat.system", "CMSG_CHANNEL_KICK {} Channel: {}, Target: {}", GetPlayerInfo(), channelName, targetName);
    if (!normalizePlayerName(targetName))
        return;
...
void WorldSession::HandleChannelUnban(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("chat.system", "CMSG_CHANNEL_UNBAN {} Channel: {}, Target: {}", GetPlayerInfo(), channelName, targetName);
    if (!normalizePlayerName(targetName))
        return;
...
void WorldSession::HandleChannelAnnouncements(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("chat.system", "CMSG_CHANNEL_ANNOUNCEMENTS {} Channel: {}", GetPlayerInfo(), channelName);
    if (ChannelMgr* cMgr = ChannelMgr::forTeam(GetPlayer()->GetTeamId()))
        if (Channel* channel = cMgr->GetChannel(channelName, GetPlayer()))
...
void WorldSession::HandleChannelModerateOpcode(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("chat.system", "CMSG_CHANNEL_MODERATE {} Channel: {}", GetPlayerInfo(), channelName);
...
void WorldSession::HandleGetChannelMemberCount(WorldPacket& recvPacket)
{
    ...
    LOG_DEBUG("chat.system", "CMSG_GET_CHANNEL_MEMBER_COUNT {} Channel: {}", GetPlayerInfo(), channelName);
    if (ChannelMgr* cMgr = ChannelMgr::forTeam(GetPlayer()->GetTeamId()))
    {
        if (Channel* channel = cMgr->GetChannel(channelName, GetPlayer()))
        {
