...
void Player::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    else if (IsClass(CLASS_PALADIN, CLASS_CONTEXT_STATS) || IsClass(CLASS_DEATH_KNIGHT, CLASS_CONTEXT_STATS) ||
    ...
    else if (IsClass(CLASS_MAGE, CLASS_CONTEXT_STATS) || IsClass(CLASS_PRIEST, CLASS_CONTEXT_STATS) ||
    ...
    else
    {
        ...
        if (CanDualWield() &&
            HasOffhandWeaponForAttack()) //allow update offhand damage only if player knows DualWield Spec and has equipped offhand weapon
...
void Player::CalculateMinMaxDamage(
    WeaponAttackType attType, bool normalized, bool addTotalPct, float& minDamage, float& maxDamage, uint8 damageIndex)
{
    ...
    float baseValue =
        GetModifierValue(unitMod, BASE_VALUE) + GetTotalAttackPowerValue(attType) / 14.0f * attackSpeedMod;
...
void Player::UpdateBlockPercentage()
{
    ...
    if (CanBlock())
    {
        ...
        {
            value = value > sConfigMgr->GetOption<float>("Stats.Limits.Block", 95.0f)
...
float Player::GetMissPercentageFromDefence() const
{
    ...
    return nondiminishing +
           (diminishing * miss_cap[pclass] / (diminishing + miss_cap[pclass] * m_diminishing_k[pclass]));
}
...
void Player::UpdateParryPercentage()
{
    ...
    if (CanParry() && parry_cap[pclass] > 0.0f)
    {
        ...
        m_realParry = nondiminishing +
                      diminishing * parry_cap[pclass] / (diminishing + parry_cap[pclass] * m_diminishing_k[pclass]);
...
void Player::UpdateDodgePercentage()
{
    ...
    m_realDodge = nondiminishing +
                  (diminishing * dodge_cap[pclass] / (diminishing + dodge_cap[pclass] * m_diminishing_k[pclass]));
...
void Player::UpdateManaRegen()
{
    ...
    float power_regen_mp5 =
        (GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_POWER_REGEN, POWER_MANA) + m_baseManaRegen) / 5.0f;
    ...
    SetStatFloatValue(UNIT_FIELD_POWER_REGEN_INTERRUPTED_FLAT_MODIFIER,
        power_regen_mp5 + CalculatePct(power_regen, modManaRegenInterrupt));
...
void Creature::CalculateMinMaxDamage(WeaponAttackType attType, bool normalized, bool addTotalPct, float& minDamage,
    float& maxDamage, uint8 damageIndex /*= 0*/)
{
...
void Guardian::UpdateMaxPower(Powers power)
{
    ...
    float addValue =
        (power == POWER_MANA) ? std::max<float>(GetStat(STAT_INTELLECT) - GetCreateStat(STAT_INTELLECT), 0.0f) : 0.0f;
