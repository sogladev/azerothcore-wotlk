...
{
    LOG_DEBUG(
        "entities.unit", "Deleting member pointer to spawnId: {} from group {}", member->GetSpawnId(), group->GetId());
...
void FormationMgr::LoadCreatureFormations()
{
    ...
    do
    {
        ...
        if (group_member.leaderGUID != memberGUID)
        {
            ...
            {
                LOG_ERROR("sql.sql",
                    "creature_formations table leader guid {} and member guid {} has unsupported GroupAI flag value ({}). Skipped",
            ...
            if (!group_member.HasGroupFlag(
                    std::underlying_type_t<GroupAIFlags>(GroupAIFlags::GROUP_AI_FLAG_FOLLOW_LEADER)) &&
                (follow_dist > 0.0f || follow_angle > 0.0f))
            {
                LOG_ERROR("sql.sql",
                    "creature_formations table member guid {} and leader guid {} cannot have follow distance or follow angle because don't have GROUP_AI_FLAG_FOLLOW_LEADER flag. Values are not gonna be used",
                    memberGUID,
                    group_member.leaderGUID);
        ...
        else
        {
            ...
            if (group_member.groupAI &&
                !group_member.HasGroupFlag(std::underlying_type_t<GroupAIFlags>(GroupAIFlags::GROUP_AI_FLAG_SUPPORTED)))
            {
                LOG_ERROR("sql.sql",
                    "creature_formations table leader guid {} and member guid {} has unsupported GroupAI flag value ({}). Skipped",
                    group_member.leaderGUID,
            ...
            {
                LOG_ERROR("sql.sql",
                    "creature_formations table member guid {} and leader guid {} cannot have follow distance or follow angle. Values are not gonna be used",
                    memberGUID,
                    group_member.leaderGUID);
            }
        }
        ...
        {
            LOG_ERROR("sql.sql",
                "creature_formations table leader guid {} incorrect (does not exist). Skipped",
                group_member.leaderGUID);
            continue;
        }
        ...
        {
            LOG_ERROR(
                "sql.sql", "creature_formations table member guid {} incorrect (does not exist). Skipped", memberGUID);
            continue;
        }
...
void CreatureGroup::MemberEngagingTarget(Creature* member, Unit* target)
{
    ...
    for (auto const& itr : m_members)
    {
        ...
        if (pMember == m_leader &&
            !(groupAI & std::underlying_type_t<GroupAIFlags>(GroupAIFlags::GROUP_AI_FLAG_LEADER_ASSIST_MEMBER)))
        {
            continue;
        }
...
Unit* CreatureGroup::GetNewTargetForMember(Creature* member)
{
    ...
    for (auto const& itr : m_members)
    {
        ...
        if (pMember == m_leader &&
            !(groupAI & std::underlying_type_t<GroupAIFlags>(GroupAIFlags::GROUP_AI_FLAG_MEMBER_ASSIST_LEADER)))
        {
            continue;
        }
...
void CreatureGroup::MemberEvaded(Creature* member)
{
    ...
    for (auto const& itr : m_members)
    {
        ...
        if (pMember == member || pMember->IsInEvadeMode() ||
            !itr.second.HasGroupFlag(std::underlying_type_t<GroupAIFlags>(GroupAIFlags::GROUP_AI_FLAG_EVADE_MASK)))
            continue;
...
{
    if (m_members.size() && !(m_members.begin()->second.HasGroupFlag(
                                std::underlying_type_t<GroupAIFlags>(GroupAIFlags::GROUP_AI_FLAG_FOLLOW_LEADER))))
        return;
...
void CreatureGroup::LeaderMoveTo(float x, float y, float z, uint32 move_type)
{
    ...
    for (auto const& itr : m_members)
    {
        ...
        if (static_cast<float>(M_PI) -
                std::fabs(std::fabs(m_leader->GetOrientation() - pathAngle) - static_cast<float>(M_PI)) >
            static_cast<float>(M_PI) * 0.5f)
        {
            ...
            followAngle = Position::NormalizeOrientation(
                pFormationInfo.follow_angle + static_cast<float>(M_PI)); //(2 * M_PI) - itr->second->follow_angle;
        }
