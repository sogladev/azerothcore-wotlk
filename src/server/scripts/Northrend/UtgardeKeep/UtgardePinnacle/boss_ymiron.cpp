...
static ActiveBoatStruct BoatStructure[4] = {
    {NPC_RANULF,
     NPC_RANULF_VISUAL, SAY_SUMMON_RANULF,
     404.379f, -335.335f,
     104.756f, 413.594f,
     -335.408f,
     107.995f, 3.157f},
    {NPC_TORGYN,
     NPC_TORGYN_VISUAL, SAY_SUMMON_TORGYN,
     380.813f, -335.069f,
     104.756f, 369.994f,
     -334.771f,
     107.995f, 6.232f},
    {NPC_BJORN,
     NPC_BJORN_VISUAL,  SAY_SUMMON_BJORN,
     381.546f, -314.362f,
     104.756f, 370.841f,
     -314.426f,
     107.995f, 6.232f},
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                case EVENT_YMIRON_HEALTH_CHECK:
                {
                    if (me->GetHealth() <
                        std::max(0.0f,
                            float(me->GetMaxHealth() * (1.0f - (IsHeroic() ? 0.2f : 0.334f) * float(BoatNum + 1)))))
                    {
                        ...
                        if (BoatNum) // different than 0
                            if (Creature* cr = me->SummonTrigger(BoatStructure[BoatOrder[BoatNum - 1]].SpawnX,
                ...
                {
                    // Spawn it!
                    if (Creature* king = me->SummonCreature(BoatStructure[BoatOrder[BoatNum - 1]].npc,
                ...
                {
                    if (Creature* sf = me->SummonCreature(NPC_SPIRIT_FOUNT,
                            385 + rand() % 10,
                            -330 + rand() % 10,
                            104.756f,
                            0,
                ...
                case EVENT_YMIRON_TORGYN_ABILITY:
                {
                    for (uint8 i = 0; i < 4; ++i)
                    {
                        if (Creature* as = me->SummonCreature(NPC_AVENGING_SPIRIT,
