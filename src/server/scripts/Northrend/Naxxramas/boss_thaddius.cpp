        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case EVENT_THADDIUS_CHAIN_LIGHTNING:
                    me->CastSpell(
                        me->GetVictim(), RAID_MODE(SPELL_CHAIN_LIGHTNING_10, SPELL_CHAIN_LIGHTNING_25), false);
                    events.Repeat(15s);
                    break;
        ...
        void UpdateAI(uint32 diff) override
        {
            if (visualTimer)
            {
                ...
                if (visualTimer >= 3000)
                {
                    ...
                    {
                        cr->CastSpell(
                            cr, me->GetEntry() == NPC_STALAGG ? SPELL_STALAGG_CHAIN : SPELL_FEUGEN_CHAIN, false);
                    }
                }
            }
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case EVENT_MINION_MAGNETIC_PULL:
                    ...
                    {
                        if (Creature* feugen =
                                ObjectAccessor::GetCreature(*me, pInstance->GetGuidData(DATA_FEUGEN_BOSS)))
                        {
                        ...
                        if (!me->GetHomePosition().IsInDist(me, 28) && me->IsInCombat())
                        {
                            if (!overload)
                            {
                                ...
                                me->RemoveAurasDueToSpell(
                                    me->GetEntry() == NPC_STALAGG ? SPELL_STALAGG_CHAIN : SPELL_FEUGEN_CHAIN);
                                cr->InterruptNonMeleeSpells(true);
                            }
...
class spell_thaddius_pos_neg_charge : public SpellScript
{
    ...
    void HandleTargets(std::list<WorldObject*>& targets)
    {
        ...
        {
            uint32 spellId =
                GetSpellInfo()->Id == SPELL_POSITIVE_CHARGE ? SPELL_POSITIVE_CHARGE_STACK : SPELL_NEGATIVE_CHARGE_STACK;
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_thaddius_pos_neg_charge::HandleDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_thaddius_pos_neg_charge::HandleTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ALLY);
    }
};
    ...
    void HandleDummy(SpellEffIndex /* effIndex */)
    {
        ...
        if (Unit* target = GetHitUnit())
        {
            ...
            target->CastSpell(target,
                roll_chance_i(50) ? SPELL_POSITIVE_POLARITY : SPELL_NEGATIVE_POLARITY,
                true,
                nullptr,
                nullptr,
                caster->GetGUID());
