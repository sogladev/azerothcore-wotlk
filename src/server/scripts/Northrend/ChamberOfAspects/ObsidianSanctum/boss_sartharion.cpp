...
public:
    ...
    struct boss_sartharionAI : public BossAI
    {
        ...
        void SetData(uint32 type, uint32 data) override
        {
            if (type == DATA_VOLCANO_BLOWS && data)
            {
                if (!volcanoBlows.empty() &&
                    std::find(volcanoBlows.begin(), volcanoBlows.end(), data) != volcanoBlows.end())
                {
                    return;
                }
        ...
        uint32 GetData(uint32 dataOrGuid) const override
        {
            ...
            if (!volcanoBlows.empty() &&
                std::find(volcanoBlows.begin(), volcanoBlows.end(), dataOrGuid) != volcanoBlows.end())
            {
                return 1;
            }
    ...
    private:
        ...
        void SendLavaWaves(bool start)
        {
            ...
            for (ObjectGuid const& guid : summons)
            {
                ...
                if (start)
                {
                    tsunami->GetMotionMaster()->MovePoint(0,
                        ((tsunami->GetPositionX() < 3250.0f) ? 3283.44f : 3208.44f),
        ...
        bool IsTargetInBounds(Unit const* victim) const
        {
            if (!victim || victim->GetPositionX() < SartharionBoundary[0] ||
                victim->GetPositionX() > SartharionBoundary[1] || victim->GetPositionY() > SartharionBoundary[3])
            {
                return false;
            }
...
struct boss_sartharion_dragonAI : public BossAI
{
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        if (!isCalledBySartharion)
            ClearInstance();
        else if (Creature* sartharion = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_SARTHARION)))
        ...
        void ExecuteEvent(uint32 eventId) override
        {
            switch (eventId)
            {
                ...
                case EVENT_MINIBOSS_OPEN_PORTAL:
                {
                    ...
                    if (!isCalledBySartharion)
                    {
                        if (GameObject* Portal = me->GetVictim()->SummonGameObject(GO_TWILIGHT_PORTAL,
                ...
                case EVENT_MINIBOSS_SPAWN_HELPERS:
                {
                    ...
                    {
                        if (Creature* egg = me->SummonCreature(NPC_TWILIGHT_EGG,
                                ...
                                60000))
                        {
                            ...
                            if (isCalledBySartharion && instance->GetBossState(DATA_SARTHARION) == IN_PROGRESS)
                            {
                                if (Creature* sartharion =
                                        ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_SARTHARION)))
                                {
                ...
                case EVENT_MINIBOSS_HATCH_EGGS:
                {
                    ...
                    for (ObjectGuid const& summonGuid : summons)
                    {
                        ...
                        if (Creature* whelp = me->SummonCreature(NPC_TWILIGHT_WHELP,
                                summon->GetPosition(),
                                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                60000))
                        {
        ...
        void ExecuteEvent(uint32 eventId) override
        {
            switch (eventId)
            {
                ...
                case EVENT_MINIBOSS_SPAWN_HELPERS:
                {
                    ...
                    DoCastAOE(static_cast<uint32>(
                                  (isCalledBySartharion ? SPELL_GIFT_OF_TWILIGHT_FIRE : SPELL_GIFT_OF_TWILIGHT_SHADOW)),
                        true);

                    if (Creature* acolyte = me->SummonCreature(
                            (isCalledBySartharion ? NPC_ACOLYTE_OF_SHADRON : NPC_DISCIPLE_OF_SHADRON),
                            me->GetPositionX(),
                            me->GetPositionY(),
                            me->GetPositionZ(),
                            me->GetOrientation()))
    ...
    struct boss_sartharion_vesperonAI : public boss_sartharion_dragonAI
    {
        ...
        void ExecuteEvent(uint32 eventId) override
        {
            switch (eventId)
            {
                ...
                {
                    Talk(WHISPER_SUMMON_DICIPLE);
                    DoCastSelf(isCalledBySartharion ? static_cast<uint32>(SPELL_TWILIGHT_TORMENT_SARTHARION)
                                                    : static_cast<uint32>(SPELL_TWILIGHT_TORMENT_VESPERON),
                        true);
                    if (Creature* acolyte = me->SummonCreature(
                            (isCalledBySartharion ? NPC_ACOLYTE_OF_VESPERON : NPC_DISCIPLE_OF_VESPERON),
                            me->GetPositionX(),
                            me->GetPositionY(),
                            me->GetPositionZ(),
                            me->GetOrientation()))
...
public:
    ...
    {
        npc_twilight_summonAI(Creature* pCreature) : ScriptedAI(pCreature), fadeArmorTimer(urand(0, 15000)) { }
...
class spell_sartharion_lava_strike : public SpellScript
{
    ...
    void Register() override
    {
        ...
        {
            OnEffectHitTarget +=
                SpellEffectFn(spell_sartharion_lava_strike::HandleSchoolDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
    }
...
class spell_obsidian_sanctum_flame_tsunami : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_obsidian_sanctum_flame_tsunami::HandleHit, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
    }
};
...
class spell_obsidian_sanctum_flame_tsunami_leap : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_obsidian_sanctum_flame_tsunami_leap::HandleLeapBack, EFFECT_0, SPELL_EFFECT_LEAP_BACK);
    }
};
