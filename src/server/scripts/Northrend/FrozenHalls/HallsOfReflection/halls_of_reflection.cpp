...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        {
            QuestStatus status =
                player->GetQuestStatus(creature->GetEntry() == NPC_SYLVANAS_PART1 ? QUEST_DELIVRANCE_FROM_THE_PIT_H2
                                                                                  : QUEST_DELIVRANCE_FROM_THE_PIT_A2);
            if (status == QUEST_STATUS_COMPLETE || status == QUEST_STATUS_REWARDED)
            {
                AddGossipItemFor(player,
                    creature->GetEntry() == NPC_SYLVANAS_PART1 ? GOSSIP_MENU_SYLVANAS : GOSISP_MENU_JAINA,
                    GOSSIP_OPTION_START,
                    GOSSIP_SENDER_MAIN,
            ...
            status =
                player->GetQuestStatus(creature->GetEntry() == NPC_SYLVANAS_PART1 ? QUEST_WRATH_OF_THE_LICH_KING_H2
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case EVENT_START_INTRO:
                    ...
                    if (Creature* pLoralen =
                            pInstance->instance->GetCreature(pInstance->GetGuidData(NPC_DARK_RANGER_LORALEN)))
                    {
                ...
                case EVENT_SKIP_INTRO:
                    ...
                    me->SetUInt32Value(UNIT_NPC_EMOTESTATE,
                        (me->GetEntry() == NPC_JAINA_PART1 ? EMOTE_STATE_READY2H : EMOTE_STATE_READY1H));
                    me->GetMotionMaster()->MovePoint(0, MoveThronePos);
                    if (Creature* pLoralen =
                            pInstance->instance->GetCreature(pInstance->GetGuidData(NPC_DARK_RANGER_LORALEN)))
                    {
                ...
                case EVENT_INTRO_LK_1:
                    if (Creature* pLichKing =
                            pInstance->instance->GetCreature(pInstance->GetGuidData(NPC_LICH_KING_EVENT)))
                    {
                ...
                case EVENT_INTRO_LK_1_1:
                    if (Creature* pLichKing =
                            ObjectAccessor::GetCreature(*me, pInstance->GetGuidData(NPC_LICH_KING_EVENT)))
                    {
...
public:
    ...
    struct npc_ghostly_priestAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (me->HasUnitState(UNIT_STATE_CASTING) || me->isFeared() || me->IsCharmed() || me->isFrozen() ||
        ...
        void MovementInform(uint32 type, uint32 /*id*/) override
        {
            ...
            if (type == ESCORT_MOTION_TYPE && me->movespline->Finalized())
            {
                if (currentWall == 0)
                {
                    ...
                    if (Creature* icewall =
                            pInstance->instance->GetCreature(pInstance->GetGuidData(NPC_ICE_WALL_TARGET)))
                    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                case EVENT_LK_CHECK_COMBAT:
                    ...
                    {
                        if (Creature* leader =
                                pInstance->instance->GetCreature(pInstance->GetGuidData(NPC_SYLVANAS_PART2)))
                            if (leader->IsAlive() && leader->GetPositionX() < 5575.0f &&
                                me->GetExactDist2d(leader) <= 12.5f && !leader->HasAura(SPELL_HARVEST_SOUL) &&
                                    ...
                                    for (Map::PlayerList::const_iterator itr = pl.begin(); itr != pl.end(); ++itr)
                                        if (Player* p = itr->GetSource())
                                            if (!p->IsGameMaster() && p->IsAlive() &&
                                                (p->GetPositionX() - me->GetPositionX() + p->GetPositionY() -
                ...
                case EVENT_LK_SUMMON_NEXT_ICE_WALL:
                    ...
                    if (Creature* c =
                            pInstance->instance->GetCreature(pInstance->GetGuidData(NPC_ICE_WALL_TARGET + currentWall)))
...
public:
    ...
    struct npc_hor_leader_secondAI : public NullCreatureAI
    {
        ...
        void MoveToNextStopPoint()
        {
            ...
            Movement::PointsArray path;
            path.push_back(G3D::Vector3(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()));
            for (uint8 i = WP_STOP[currentStopPoint - 1] + (currentStopPoint == 1 ? 0 : 1);
                 i <= WP_STOP[currentStopPoint];
                 ++i)
                path.push_back(G3D::Vector3(
                    PathWaypoints[i].GetPositionX(), PathWaypoints[i].GetPositionY(), PathWaypoints[i].GetPositionZ()));
            me->GetMotionMaster()->MoveSplinePath(&path);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                case EVENT_LK_SAY_AGGRO:
                    if (Creature* lkboss = pInstance->instance->GetCreature(pInstance->GetGuidData(NPC_LICH_KING_BOSS)))
                    {
                        me->Attack(lkboss, true),
                            lkboss->AI()->Talk(
                                me->GetEntry() == NPC_JAINA_PART2 ? SAY_LK_AGGRO_ALLY : SAY_LK_AGGRO_HORDE);
                    }
                    break;
                ...
                case EVENT_ADD_GOSSIP:
                    me->RemoveAura(
                        me->GetEntry() == NPC_JAINA_PART2 ? SPELL_JAINA_ICE_BARRIER : SPELL_SYLVANAS_CLOAK_OF_DARKNESS);
                ...
                case EVENT_START_RUN:
                {
                    ...
                    if (Creature* lkboss = pInstance->instance->GetCreature(pInstance->GetGuidData(NPC_LICH_KING_BOSS)))
                    {
                        ...
                        lkboss->RemoveAura(
                            me->GetEntry() == NPC_JAINA_PART2 ? SPELL_JAINA_ICE_PRISON : SPELL_SYLVANAS_DARK_BINDING);
                ...
                case EVENT_SAY_LEADER_STOP_TEXT:
                {
                    ...
                    if (currentStopPoint <= 4)
                        me->CastSpell((Unit*)nullptr,
                            (me->GetEntry() == NPC_JAINA_PART2 ? SPELL_DESTROY_WALL_JAINA
...
class spell_hor_gunship_cannon_fire_aura : public AuraScript
{
    ...
    void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        if (Unit* caster = GetCaster())
            if (Creature* creature = caster->SummonCreature(WORLD_TRIGGER,
                    CannonFirePos[caster->GetEntry() == NPC_JAINA_PART2 ? 0 : 1][urand(0, 2)],
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_hor_gunship_cannon_fire_aura::HandleEffectPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
