...
enum MiscData
{
    ...
    MUSIC_SPECIAL =
        17458, // Summon Shambling Horror, Remorseless Winter, Quake, Summon Val'kyr Periodic, Harvest Soul, Vile Spirits
...
{
    return target->GetExactDist2dSq(&CenterPosition) < 90.0f * 90.0f && target->GetPositionZ() > 840.0f &&
           target->GetPositionZ() < 875.0f;
}
...
{
public:
    NonTankLKTargetSelector(Creature* source, bool playerOnly = true, bool reqLOS = false, float maxDist = 0.0f,
        uint32 exclude1 = 0, uint32 exclude2 = 0) :
        _source(source),
        _playerOnly(playerOnly),
        _reqLOS(reqLOS),
        _maxDist(maxDist),
...
public:
    ...
    bool Execute(uint64 /*time*/, uint32 /*diff*/) override
    {
        ...
        if (!_owner->GetVictim())
            if (Unit* target =
                    _summoner->AI()->SelectTarget(SelectTargetMethod::Random, 0, NonTankLKTargetSelector(_summoner)))
                _owner->AI()->AttackStart(target);
...
public:
    ...
    {
        if (!unit || !unit->IsPlayer() || unit == _sourceObj || _sourceObj->GetVictim() == unit ||
            !unit->isTargetableForAttack())
            return false;
        if (unit->HasAura(SPELL_PLAGUE_AVOIDANCE) || unit->HasAura(SPELL_BOSS_HITTIN_YA_AURA) ||
...
public:
    ...
    struct boss_the_lich_kingAI : public BossAI
    {
        ...
        void DoAction(int32 action) override
        {
            switch (action)
            {
                ...
                case ACTION_BREAK_FROSTMOURNE:
                    ...
                    if (Creature* tirion =
                            ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_HIGHLORD_TIRION_FORDRING)))
        ...
        void DamageTaken(Unit* attacker, uint32& damage, DamageEffectType, SpellSchoolMask) override
        {
            if (!attacker ||
                (_bFrostmournePhase &&
                    attacker->GetExactDistSq(495.708f, -2523.76f, 1049.95f) >
                        40.0f * 40.0f)) // frostmourne room, prevent exploiting (tele hack to get back and damage him)
            {
                damage = 0;
                return;
            }
            ...
            if (_phase == PHASE_OUTRO)
            {
                ...
                else if (damage >= me->GetHealth()) // dying...
                {
                    ...
                    me->m_Events.AddEvent(new LichKingDeathEvent(*me),
                        me->m_Events.CalculateTime(2500)); // die after spinning anim is over, so death anim is visible
                ...
                case NPC_VILE_SPIRIT:
                {
                    ...
                    if (_phase == PHASE_THREE)
                        summon->m_Events.AddEvent(
                            new VileSpiritActivateEvent(summon), summon->m_Events.CalculateTime(15000));
                    break;
                }
        ...
        {
            if (spell->Id == HARVESTED_SOUL_BUFF && me->IsInCombat() && !IsHeroic() && _phase != PHASE_OUTRO &&
                _lastTalkTimeBuff + 5 <= GameTime::GetGameTime().count())
            {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case EVENT_QUAKE_2:
                    ...
                    events.ScheduleEvent(
                        IsHeroic() ? EVENT_HARVEST_SOULS : EVENT_HARVEST_SOUL, 14s, EVENT_GROUP_ABILITIES);
                ...
                case EVENT_NECROTIC_PLAGUE:
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random,
                            0,
                            NecroticPlagueTargetCheck(me, NECROTIC_PLAGUE_LK, NECROTIC_PLAGUE_PLR)))
                    {
                ...
                case EVENT_SHADOW_TRAP:
                    if (Unit* target = SelectTarget(
                            SelectTargetMethod::Random, 0, ShadowTrapLKTargetSelector(me, true, true, 100.0f)))
                ...
                case EVENT_PAIN_AND_SUFFERING:
                    ...
                    events.ScheduleEvent(EVENT_PAIN_AND_SUFFERING,
                ...
                case EVENT_SUMMON_RAGING_SPIRIT:
                    ...
                    events.ScheduleEvent(
                        EVENT_SUMMON_RAGING_SPIRIT, (!HealthAbovePct(40) ? 15s : 20s), EVENT_GROUP_ABILITIES);
                    break;
                case EVENT_DEFILE:
                {
                    ...
                    if (evTime && (events.GetTimer() > evTime || evTime - events.GetTimer() < 5000))
                    {
                        ...
                        if (events.GetTimer() > evTime || evTime - events.GetTimer() < 3500)
                        {
                            ...
                            events.ScheduleEvent(
                                EVENT_DEFILE, t + (Is25ManRaid() ? 5000 : 4000), EVENT_GROUP_ABILITIES);
                            break;
                        }
                ...
                case EVENT_HARVEST_SOUL:
                    if (Unit* target =
                            SelectTarget(SelectTargetMethod::Random, 0, NonTankLKTargetSelector(me, true, true, 55.0f)))
                    {
                    ...
                    for (SummonList::iterator i = summons.begin(); i != summons.end(); ++i)
                        if (Creature* summon = ObjectAccessor::GetCreature(*me, *i))
                        {
                            if (summon->GetEntry() == NPC_VILE_SPIRIT)
                            {
                                ...
                                summon->m_Events.AddEvent(
                                    new VileSpiritActivateEvent(summon), summon->m_Events.CalculateTime(55000));
                ...
                case EVENT_FROSTMOURNE_HEROIC:
                    ...
                    if (TempSummon* terenas = me->GetMap()->SummonCreature(
                            NPC_TERENAS_MENETHIL_FROSTMOURNE_H, TerenasSpawnHeroic, nullptr, 55000))
                    {
                        terenas->AI()->DoAction(ACTION_FROSTMOURNE_INTRO);
                        if (Creature* spawner =
                                terenas->FindNearestCreature(NPC_WORLD_TRIGGER_INFINITE_AOI, 100.0f, true))
                        {
                            spawner->CastSpell(spawner, SPELL_SUMMON_SPIRIT_BOMB_1, true); // summons bombs randomly
                            spawner->CastSpell(spawner, SPELL_SUMMON_SPIRIT_BOMB_2, true); // summons bombs on players
                            spawner->m_Events.AddEvent(
                                new TriggerWickedSpirit(spawner), spawner->m_Events.CalculateTime(3000));
                            terenas->SetImmuneToAll(
                                true); // to avoid being healed by player trinket procs. terenas' health doesn't matter on heroic
                        }
                    }
                    break;
            ...
            if (!me->HasReactState(REACT_PASSIVE) && me->GetVictim() && me->IsWithinMeleeRange(me->GetVictim()) &&
                me->GetMotionMaster()->GetCurrentMovementGeneratorType() != POINT_MOTION_TYPE &&
                !me->IsWithinLOSInMap(me->GetVictim()))
                me->GetMotionMaster()->MoveCharge(me->GetVictim()->GetPositionX(),
...
public:
    ...
    struct npc_tirion_fordringAI : public ScriptedAI
    {
            ...
            switch (id)
            {
                case POINT_TIRION_INTRO:
                    if (Creature* theLichKing =
                            ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_THE_LICH_KING)))
                    {
        ...
        void sGossipSelect(Player* /*player*/, uint32 sender, uint32 action) override
        {
            ...
            if (me->GetCreatureTemplate()->GossipMenuId == sender && !action && me->HasNpcFlag(UNIT_NPC_FLAG_GOSSIP) &&
                theLichKing && !theLichKing->IsInEvadeMode())
            {
            ...
            switch (_events.ExecuteEvent())
            {
                case EVENT_INTRO_LK_MOVE:
                            ...
                            ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_THE_LICH_KING)))
                    {
                        ...
                        path.push_back(G3D::Vector3(
                            theLichKing->GetPositionX(), theLichKing->GetPositionY(), theLichKing->GetPositionZ()));
                        for (uint8 i = 0; i < 3; ++i)
                            path.push_back(G3D::Vector3(LichKingIntro[i].GetPositionX(),
                ...
                case EVENT_OUTRO_LK_MOVE_CENTER:
                            ...
                            ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_THE_LICH_KING)))
                    {
                        ...
                        uint32 travelTime =
                            1000 * theLichKing->GetExactDist(&CenterPosition) / theLichKing->GetSpeed(MOVE_WALK) + 1000;
                ...
                case EVENT_OUTRO_SOUL_BARRAGE:
                            ...
                            ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_THE_LICH_KING)))
                    {
                        ...
                        sCreatureTextMgr->SendSound(
                            theLichKing, SOUND_PAIN, CHAT_MSG_MONSTER_YELL, 0, TEXT_RANGE_NORMAL, TEAM_NEUTRAL, false);
...
class spell_the_lich_king_quake : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        if (GameObject* platform = ObjectAccessor::GetGameObject(
                *GetCaster(), GetCaster()->GetInstanceScript()->GetGuidData(DATA_ARTHAS_PLATFORM)))
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_the_lich_king_quake::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
...
class spell_the_lich_king_jump : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_the_lich_king_jump::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_the_lich_king_jump_remove_aura : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_the_lich_king_jump_remove_aura::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_the_lich_king_play_movie : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_the_lich_king_play_movie::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_the_lich_king_infest_aura : public AuraScript
{
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_the_lich_king_infest_aura::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
        OnEffectUpdatePeriodic +=
            AuraEffectUpdatePeriodicFn(spell_the_lich_king_infest_aura::OnUpdate, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
    }
};
...
class spell_the_lich_king_necrotic_plague_aura : public AuraScript
{
    ...
    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (dispel)
            values.AddSpellMod(
                SPELLVALUE_BASE_POINT1, AURA_REMOVE_BY_ENEMY_SPELL); // add as marker (spell has no effect 1)
        GetTarget()->CastCustomSpell(
            SPELL_NECROTIC_PLAGUE_JUMP, values, nullptr, TRIGGERED_FULL_MASK, nullptr, nullptr, GetCasterGUID());
    ...
    void Register() override
    {
        AfterEffectRemove += AuraEffectRemoveFn(spell_the_lich_king_necrotic_plague_aura::OnRemove,
            EFFECT_0,
...
class spell_the_lich_king_necrotic_plague_jump : public SpellScript
{
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_the_lich_king_necrotic_plague_jump::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
...
class spell_the_lich_king_necrotic_plague_jump_aura : public AuraScript
{
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(spell_the_lich_king_necrotic_plague_jump_aura::OnApply,
            EFFECT_0,
            SPELL_AURA_PERIODIC_DAMAGE,
            AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
        AfterEffectRemove += AuraEffectRemoveFn(spell_the_lich_king_necrotic_plague_jump_aura::OnRemove,
            EFFECT_0,
            SPELL_AURA_PERIODIC_DAMAGE,
            AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove += AuraEffectRemoveFn(spell_the_lich_king_necrotic_plague_jump_aura::OnDispel,
            EFFECT_0,
            SPELL_AURA_PERIODIC_DAMAGE,
            AURA_EFFECT_HANDLE_REAPPLY);
        AfterEffectApply += AuraEffectApplyFn(spell_the_lich_king_necrotic_plague_jump_aura::AfterDispel,
            EFFECT_0,
            SPELL_AURA_PERIODIC_DAMAGE,
            AURA_EFFECT_HANDLE_REAPPLY);
    }
...
class spell_the_lich_king_shadow_trap_visual_aura : public AuraScript
{
    ...
    void Register() override
    {
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_the_lich_king_shadow_trap_visual_aura::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
};
...
class spell_the_lich_king_shadow_trap_periodic : public SpellScript
{
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_the_lich_king_shadow_trap_periodic::CheckTargetCount, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
    }
};
...
class spell_the_lich_king_ice_burst_target_search : public SpellScript
{
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_the_lich_king_ice_burst_target_search::CheckTargetCount, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
    }
};
    ...
    struct npc_icc_ice_sphereAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            else
            {
                ...
                if (me->GetVictim()->GetGUID() != targetGUID || !target || !me->IsValidAttackTarget(target) ||
                    target->HasUnitFlag2(UNIT_FLAG2_FEIGN_DEATH) ||
                    target->GetExactDist2dSq(&CenterPosition) > 75.0f * 75.0f || target->GetPositionZ() < 830.0f ||
...
class spell_the_lich_king_raging_spirit : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_the_lich_king_raging_spirit::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
        ...
        void Reset() override
        {
            ...
            if (TempSummon* summon = me->ToTempSummon())
                if (Unit* summoner = summon->GetSummonerUnit())
                    if (summoner->IsPlayer() && summoner->IsAlive() && !summoner->ToPlayer()->IsBeingTeleported() &&
            ...
            switch (_events.ExecuteEvent())
            {
                case EVENT_RAGING_SPIRIT_UNROOT:
                {
                    ...
                    if (!me->GetVictim())
                    {
                        ...
                        if (TempSummon* summon = me->ToTempSummon())
                            if (Unit* summoner = summon->GetSummonerUnit())
                                if (summoner->IsPlayer() && summoner->IsAlive() &&
                                    !summoner->ToPlayer()->IsBeingTeleported() && summoner->FindMap() == me->GetMap())
                                {
                                    valid = true;
...
class spell_the_lich_king_defile : public SpellScript
{
    ...
    void CorrectRange(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if(Acore::AllWorldObjectsInExactRange(
            GetCaster(), 10.0f * GetCaster()->GetFloatValue(OBJECT_FIELD_SCALE_X), true));
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_the_lich_king_defile::CorrectRange, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_the_lich_king_defile::CorrectRange, EFFECT_1, TARGET_UNIT_SRC_AREA_ENEMY);
...
class spell_the_lich_king_soul_reaper_aura : public AuraScript
{
    ...
    void Register() override
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_the_lich_king_soul_reaper_aura::OnPeriodic, EFFECT_1, SPELL_AURA_PERIODIC_DAMAGE);
    }
};
...
public:
    ...
    struct npc_valkyr_shadowguardAI : public NullCreatureAI
    {
        ...
        void GoSiphon()
        {
            ...
            _destPoint.Relocate(CenterPosition.GetPositionX() + dist * cos(angle),
            ...
            switch (id)
            {
                case EVENT_CHARGE:
                {
                    ...
                    if (Player* target = ObjectAccessor::GetPlayer(*me, _grabbedPlayer))
                        if (target->FindMap() == me->GetMap() && target->GetExactDist(me) < 15.0f &&
                            !target->GetVehicle())
                            if (GameObject* platform =
                                    ObjectAccessor::GetGameObject(*me, _instance->GetGuidData(DATA_ARTHAS_PLATFORM)))
                            {
                ...
                case POINT_DROP_PLAYER:
                {
                    ...
                    if (me->GetExactDist(&_destPoint) >
                        1.5f) // movement was interrupted (probably by a stun, start again)
                    {
                        _events.Reset();
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (_events.ExecuteEvent())
            {
                ...
                case EVENT_MOVE_TO_SIPHON_POS:
                    me->RemoveUnitFlag(
                        UNIT_FLAG_NON_ATTACKABLE); // just in case if passenger disappears so quickly that EVENT_MOVE_TO_DROP_POS is never executed
                    {
                        int32 bp0 = 80;
                ...
                case EVENT_LIFE_SIPHON:
                {
                    ...
                    if (!tauntAuras.empty())
                        for (Unit::AuraEffectList::const_reverse_iterator itr = tauntAuras.rbegin();
                             itr != tauntAuras.rend();
                             ++itr)
                    ...
                    if (!target)
                        if (Creature* lichKing =
                            ...
                            target = lichKing->AI()->SelectTarget(
                                SelectTargetMethod::Random, 0, NonTankLKTargetSelector(lichKing, true, false, 100.0f));
                    if (target)
...
class spell_the_lich_king_teleport_to_frostmourne_hc : public SpellScript
{
    ...
    void Register() override
    {
        OnEffectHit += SpellEffectFn(
            spell_the_lich_king_teleport_to_frostmourne_hc::ModDest, EFFECT_1, SPELL_EFFECT_TELEPORT_UNITS);
    }
};
...
class spell_the_lich_king_valkyr_target_search : public SpellScript
{
    ...
    void HandleScript(SpellEffIndex effIndex)
    {
        ...
        if (Unit* target = GetHitUnit())
        {
            GetCaster()->GetMotionMaster()->MoveCharge(
                target->GetPositionX(), target->GetPositionY(), target->GetPositionZ() + 4.0f, 42.0f, EVENT_CHARGE);
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_the_lich_king_valkyr_target_search::SelectTarget, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_the_lich_king_valkyr_target_search::ReplaceTarget, EFFECT_1, TARGET_UNIT_SRC_AREA_ENEMY);
        OnEffectHitTarget +=
            SpellEffectFn(spell_the_lich_king_valkyr_target_search::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_the_lich_king_cast_back_to_caster : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_the_lich_king_cast_back_to_caster::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_the_lich_king_life_siphon : public SpellScript
{
    ...
    void TriggerHeal()
    {
        GetHitUnit()->CastCustomSpell(
            SPELL_LIFE_SIPHON_HEAL, SPELLVALUE_BASE_POINT0, GetHitDamage() * 10, GetCaster(), true);
    }
...
class spell_the_lich_king_vile_spirits_aura : public AuraScript
{
    ...
    void OnPeriodic(AuraEffect const* aurEff)
    {
        if (_is25Man || ((aurEff->GetTickNumber() - 1) % 5))
            GetTarget()->CastSpell((Unit*)nullptr,
    ...
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_the_lich_king_vile_spirits_aura::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
    }
...
class spell_the_lich_king_vile_spirits_visual : public SpellScript
{
    ...
    {
        OnEffectLaunch +=
            SpellEffectFn(spell_the_lich_king_vile_spirits_visual::ModDestHeight, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
...
class spell_the_lich_king_vile_spirit_move_target_search : public SpellScript
{
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_the_lich_king_vile_spirit_move_target_search::SelectTarget, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
        OnEffectHitTarget += SpellEffectFn(
            spell_the_lich_king_vile_spirit_move_target_search::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
...
class spell_the_lich_king_vile_spirit_damage_target_search : public SpellScript
{
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect +=
            SpellObjectAreaTargetSelectFn(spell_the_lich_king_vile_spirit_damage_target_search::CheckTargetCount,
                EFFECT_0,
    ...
    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_DEATH)
            GetTarget()->CastSpell((Unit*)nullptr,
                ...
                nullptr,
                nullptr,
                GetTarget()->GetInstanceScript()->GetGuidData(DATA_THE_LICH_KING));
    }

    void Register() override
    {
        AfterEffectRemove += AuraEffectRemoveFn(spell_the_lich_king_harvest_soul_aura::OnRemove,
            EFFECT_0,
            SPELL_AURA_PERIODIC_DAMAGE,
            AURA_EFFECT_HANDLE_REAL);
    }
    ...
    struct npc_strangulate_vehicleAI : public NullCreatureAI
    {
        ...
        void DoAction(int32 action) override
        {
            ...
            if (TempSummon* summ = me->ToTempSummon())
            {
                ...
                {
                    bool buff = _instance->GetBossState(DATA_THE_LICH_KING) == IN_PROGRESS && summoner->IsPlayer() &&
                                (!summoner->IsAlive() || summoner->ToPlayer()->IsBeingTeleported() ||
                                    summoner->FindMap() != me->GetMap());
                    if (summoner->IsPlayer() && !summoner->ToPlayer()->IsBeingTeleported() &&
                        summoner->FindMap() == me->GetMap())
                    {
                        if (buff)
                    ...
                    summoner->RemoveAurasDueToSpell(
                        IsHeroic() ? SPELL_HARVEST_SOULS_TELEPORT : SPELL_HARVEST_SOUL_TELEPORT);
                }
                else
                    me->CastSpell((Unit*)nullptr,
                        SPELL_HARVESTED_SOUL_LK_BUFF,
                    ...
                    if (TempSummon* summ = me->ToTempSummon())
                        if (Unit* summoner = summ->GetSummonerUnit())
                        {
                            if (summoner->IsAlive() && summoner->IsPlayer())
                            {
                                ...
                                me->UpdatePosition(
                                    me->GetPositionX(), me->GetPositionY(), 840.87f, me->GetOrientation(), true);
                                me->StopMovingOnCurrentPos();
                                if (!IsHeroic())
                ...
                case EVENT_MOVE_TO_LICH_KING:
                    if (Creature* lichKing =
                        ...
                        if (me->GetExactDist(lichKing) > 8.0f)
                        {
                            ...
                            Movement::PointsArray path;
                            path.push_back(G3D::Vector3(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()));
                            path.push_back(G3D::Vector3(lichKing->GetPositionX() + dist * cos(angle),
...
class spell_the_lich_king_lights_favor_aura : public AuraScript
{
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_the_lich_king_lights_favor_aura::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_HEAL);
        DoEffectCalcAmount += AuraEffectCalcAmountFn(
            spell_the_lich_king_lights_favor_aura::CalculateBonus, EFFECT_1, SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
    }
};
...
class spell_the_lich_king_restore_soul : public SpellScript
{
        ...
        for (std::list<WorldObject*>::const_iterator itr = unitList.begin(); itr != unitList.end(); ++itr)
            if (Unit* target = (*itr)->ToUnit())
                target->RemoveAurasDueToSpell(
                    target->GetMap()->IsHeroic() ? SPELL_HARVEST_SOULS_TELEPORT : SPELL_HARVEST_SOUL_TELEPORT);
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_the_lich_king_restore_soul::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
    }
...
class spell_the_lich_king_dark_hunger_aura : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        GetTarget()->CastCustomSpell(
            SPELL_DARK_HUNGER_HEAL, SPELLVALUE_BASE_POINT0, heal, GetTarget(), true, nullptr, aurEff);
    }
...
class spell_the_lich_king_soul_rip_aura : public AuraScript
{
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_the_lich_king_soul_rip_aura::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
    }
};
...
class spell_the_lich_king_summon_spirit_bomb : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_the_lich_king_summon_spirit_bomb::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
public:
    ...
    struct npc_lk_wicked_spiritAI : public ScriptedAI
    {
        ...
        bool CanAIAttack(Unit const* target) const override
        {
            return me->GetReactState() == REACT_AGGRESSIVE && target->IsPlayer() &&
                   target->GetExactDistSq(495.708f, -2523.76f, 1049.95f) < 40.0f * 40.0f;
        }
    };
...
void AddSC_boss_the_lich_king()
{
    ...
    RegisterSpellScriptWithArgs(
        spell_trigger_spell_from_caster, "spell_the_lich_king_mass_resurrection", SPELL_MASS_RESURRECTION_REAL);
    ...
    RegisterSpellAndAuraScriptPair(
        spell_the_lich_king_necrotic_plague_jump, spell_the_lich_king_necrotic_plague_jump_aura);
