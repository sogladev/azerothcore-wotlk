...
struct boss_terestian_illhoof : public BossAI
{
    ...
    void SpellHit(Unit* /*caster*/, SpellInfo const* spell) override
    {
        if (spell->Id == SPELL_BROKEN_PACT)
            scheduler.Schedule(45s, [this](TaskContext /*context*/) { SummonKilrek(); });
    }
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
                ...
                [this](TaskContext context)
        {
            if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 100.0f, true, false))
            {
                ...
                target->m_Events.AddEventAtOffset(
                    [target] { target->CastSpell(target, SPELL_SUMMON_DEMONCHAINS, true); }, 1s);
        ...
        }).Schedule(10min, [this](TaskContext /*context*/) { DoCastSelf(SPELL_BERSERK); });
    }
