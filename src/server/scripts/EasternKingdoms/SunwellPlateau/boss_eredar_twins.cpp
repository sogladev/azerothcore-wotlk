...
struct boss_sacrolash : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        ScheduleTimedEvent(10s, [&] { DoCastSelf(SPELL_SHADOW_BLADES); }, 10s);

        ScheduleTimedEvent(25s, [&] { DoCastVictim(SPELL_CONFOUNDING_BLOW); }, 20s, 25s);

        ScheduleTimedEvent(
            20s, [&] { me->SummonCreature(NPC_SHADOW_IMAGE, me->GetPosition(), TEMPSUMMON_TIMED_DESPAWN, 12000); }, 6s);

        scheduler.Schedule(36s,
...
struct boss_alythess : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        ScheduleTimedEvent(1s, [&] { DoCastVictim(SPELL_BLAZE); }, 3800ms);

        ScheduleTimedEvent(15s, [&] { DoCastSelf(SPELL_PYROGENICS); }, 15s);

        ScheduleTimedEvent(
            20s, [&] { me->CastCustomSpell(SPELL_FLAME_SEAR, SPELLVALUE_MAX_TARGETS, 5, me, TRIGGERED_NONE); }, 15s);
...
class spell_eredar_twins_blaze : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_eredar_twins_blaze::HandleScript, EFFECT_1, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
void AddSC_boss_eredar_twins()
{
    ...
    RegisterSpellScriptWithArgs(
        spell_eredar_twins_apply_touch, "spell_eredar_twins_apply_dark_touched", SPELL_DARK_TOUCHED);
    RegisterSpellScriptWithArgs(
        spell_eredar_twins_apply_touch, "spell_eredar_twins_apply_flame_touched", SPELL_FLAME_TOUCHED);
