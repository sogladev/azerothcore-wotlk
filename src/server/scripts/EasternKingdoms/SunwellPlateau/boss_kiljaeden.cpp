...
struct npc_kiljaeden_controller : public NullCreatureAI
{
    ...
    void Reset() override
    {
        ...
        me->SummonCreature(
            NPC_HAND_OF_THE_DECEIVER, 1702.62f, 611.19f, 27.66f, 1.81f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000);
        me->SummonCreature(
            NPC_HAND_OF_THE_DECEIVER, 1684.099f, 618.848f, 27.67f, 0.589f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000);
        me->SummonCreature(
            NPC_HAND_OF_THE_DECEIVER, 1688.38f, 641.10f, 27.50f, 5.43f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30000);
...
struct boss_kiljaeden : public BossAI
{
    ...
    void Reset() override
    {
            ...
            [&]
        {
            ...
            scheduler.Schedule(1s + 200ms, [this](TaskContext) { DoCastSelf(SPELL_SHADOW_SPIKE); });

            ScheduleTimedEvent(3s, [&] { DoCastSelf(SPELL_FLAME_DART); }, 10s);
            ...
            [&]
        {
            ...
            ScheduleTimedEvent(10s, [&] { DoCastSelf(SPELL_ARMAGEDDON_PERIODIC, true); }, 40s);
        });
                ...
                if (Creature* anveena = instance->GetCreature(DATA_ANVEENA))
                {
                        ...
                        [&]
                    {
                            ...
                            [&]
                        {
                            Talk(SAY_KJ_REFLECTION);
                            me->CastCustomSpell(
                                SPELL_SINISTER_REFLECTION, SPELLVALUE_MAX_TARGETS, 1, me, TRIGGERED_NONE);
                            me->CastCustomSpell(
                                SPELL_SINISTER_REFLECTION, SPELLVALUE_MAX_TARGETS, 1, me, TRIGGERED_NONE);
                            me->CastCustomSpell(
                                SPELL_SINISTER_REFLECTION, SPELLVALUE_MAX_TARGETS, 1, me, TRIGGERED_NONE);
                        ...
                        ScheduleTimedEvent(1500ms, [&] { DoCastSelf(SPELL_ARMAGEDDON_PERIODIC, true); }, 20s);
                    },
                        7s);
                }
                Talk(SAY_KJ_PHASE5);
    ...
    {
        ScheduleTimedEvent(1s, [&] { DoCastVictim(SPELL_SOUL_FLAY); }, 4s, 5s);

        ScheduleTimedEvent(
            7s, [&] { DoCastRandomTarget(SPELL_LEGION_LIGHTNING, 0, 40.0f); }, _phase == PHASE_SACRIFICE ? 15s : 30s);
                ...
                [&]
            {
                for (uint8 i = 1; i < _phase; ++i)
                {
                    ...
                    if (Creature* orb = me->SummonCreature(NPC_SHIELD_ORB, x, y, 40.0f, 0, TEMPSUMMON_CORPSE_DESPAWN))
                    {
                        ...
                        {
                            x = me->GetPositionX() +
                                18.0f * cos(((i * 2.0f - 1.0f) * M_PI / 3.0f) + (j / 20.0f * 2 * M_PI));
                            y = me->GetPositionY() +
                                18.0f * std::sin(((i * 2.0f - 1.0f) * M_PI / 3.0f) + (j / 20.0f * 2 * M_PI));
                            movementArray.push_back(G3D::Vector3(x, y, 40.0f));
                        }
    ...
    void DamageTaken(Unit* unit, uint32& damage, DamageEffectType damageType, SpellSchoolMask schoolMask) override
    {
        ...
        if (damage >= me->GetHealth())
        {
            ...
            events.Reset();
            damage = 0;
            me->m_Events.AddEventAtOffset([&] { me->KillSelf(); }, 1s);
        }
    }
        ...
        for (uint8 i = DATA_ORB_OF_THE_BLUE_DRAGONFLIGHT_1; i < DATA_ORB_OF_THE_BLUE_DRAGONFLIGHT_4 + 1; ++i)
        {
            if (GameObject* orb = instance->GetGameObject(i))
            {
                if (orb->HasGameObjectFlag(GO_FLAG_NOT_SELECTABLE))
                {
                    ...
                    if (Creature* trigger = me->SummonTrigger(orb->GetPositionX(),
                            orb->GetPositionY(),
                            orb->GetPositionZ(),
                            0,
                    ...
                    {
                        trigger->CastSpell(
                            trigger, SPELL_RING_OF_BLUE_FLAMES, true, nullptr, nullptr, trigger->GetGUID());
                        if (Creature* controller = instance->GetCreature(DATA_KJ_CONTROLLER))
    ...
    void JustSummoned(Creature* summon) override
    {
        ...
        else if (summon->GetEntry() == NPC_SHATTRATH_PORTAL_DUMMY)
        {
            ...
            summon->GetMotionMaster()->MovePoint(
                0, summon->GetPositionX(), summon->GetPositionY(), summon->GetPositionZ() + 30.0f, false, true);
        }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        switch (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_SCENE_05:
                if (Creature* first = me->SummonCreature(NPC_SHATTERED_SUN_SOLDIER, 1729.48f, 640.49f, 28.06f, 3.49f))
                {
                    first->m_Events.AddEvent(
                        new MoveDelayed(first, 1718.70f, 607.78f, 28.06f, 2.323f), first->m_Events.CalculateTime(5000));
                    first->m_Events.AddEvent(new FixOrientation(first), first->m_Events.CalculateTime(12000));
                    for (uint8 i = 0; i < 9; ++i)
                        if (Creature* follower = me->SummonCreature(NPC_SHATTERED_SUN_SOLDIER,
            ...
            case EVENT_SCENE_06:
                ...
                {
                    first->m_Events.AddEvent(
                        new MoveDelayed(first, 1678.69f, 649.27f, 28.06f, 5.46f), first->m_Events.CalculateTime(5000));
...
class spell_kiljaeden_sinister_reflection : public SpellScript
{
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_kiljaeden_sinister_reflection::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
        OnEffectHitTarget += SpellEffectFn(
            spell_kiljaeden_sinister_reflection::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_kiljaeden_sinister_reflection_clone : public SpellScript
{
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_kiljaeden_sinister_reflection_clone::FilterTargets, EFFECT_ALL, TARGET_UNIT_SRC_AREA_ENEMY);
    }
};
...
class spell_kiljaeden_flame_dart : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_kiljaeden_flame_dart::HandleSchoolDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
    }
};
...
class spell_kiljaeden_darkness_aura : public AuraScript
{
    ...
    void Register() override
    {
        OnEffectRemove += AuraEffectRemoveFn(
            spell_kiljaeden_darkness_aura::HandleRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
};
...
class spell_kiljaeden_power_of_the_blue_flight : public SpellScript
{
    ...
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_kiljaeden_power_of_the_blue_flight::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_kiljaeden_vengeance_of_the_blue_flight_aura : public AuraScript
{
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(spell_kiljaeden_vengeance_of_the_blue_flight_aura::HandleApply,
            EFFECT_0,
        ...
        OnEffectApply += AuraEffectApplyFn(spell_kiljaeden_vengeance_of_the_blue_flight_aura::HandleApply,
            ...
            AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectRemoveFn(spell_kiljaeden_vengeance_of_the_blue_flight_aura::HandleRemove,
            EFFECT_0,
            SPELL_AURA_MOD_POSSESS,
            AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectRemoveFn(spell_kiljaeden_vengeance_of_the_blue_flight_aura::HandleRemove,
...
class spell_kiljaeden_armageddon_periodic_aura : public AuraScript
{
    ...
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_kiljaeden_armageddon_periodic_aura::HandlePeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
...
class spell_kiljaeden_armageddon_missile : public SpellScript
{
    ...
    void Register() override
    {
        OnDestinationTargetSelect +=
            SpellDestinationTargetSelectFn(spell_kiljaeden_armageddon_missile::SetDest, EFFECT_0, TARGET_DEST_CASTER);
    }
};
...
class spell_kiljaeden_dragon_breath : public SpellScript
{
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_kiljaeden_dragon_breath::FilterTargets, EFFECT_ALL, TARGET_UNIT_CONE_ALLY);
    }
};
