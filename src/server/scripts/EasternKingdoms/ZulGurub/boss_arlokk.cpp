                ...
                switch (eventId)
                {
                    ...
                    case EVENT_MARK_OF_ARLOKK:
                    {
                        Unit* target = SelectTarget(
                            SelectTargetMethod::MaxThreat, urand(1, 3), 0.0f, false, true, -SPELL_MARK_OF_ARLOKK);
                        if (!target)
                            target = me->GetVictim();
                        if (target)
                        {
                    ...
                    case EVENT_VANISH:
                        ...
                        me->GetMotionMaster()->MovePoint(
                            0, frand(-11551.0f, -11508.0f), frand(-1638.0f, -1617.0f), me->GetPositionZ());
    ...
    struct npc_zulian_prowlerAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            if (Unit* arlokk = ObjectAccessor::GetUnit(*me, _instance->GetGuidData(NPC_ARLOKK)))
                me->GetMotionMaster()->MovePoint(
                    0, arlokk->GetPositionX(), arlokk->GetPositionY(), arlokk->GetPositionZ());
