...
struct boss_janalai : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        Talk(SAY_AGGRO);
        //schedule abilities
        ScheduleTimedEvent(30s, [&] { StartBombing(); }, 20s, 40s);
            ...
            [&]
        {
            if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0))
            {
                ...
                scheduler.Schedule(2s, [this](TaskContext) { _isFlameBreathing = false; });
            }
        },
            8s);
        ...
        me->GetMap()->DoForAllPlayers([&](Player* player)
        {
            if (player->IsAlive())
                DoTeleportPlayer(player,
                    janalainPos.GetPositionX() - 5 + rand() % 10,
                    janalainPos.GetPositionY() - 5 + rand() % 10,
                    janalainPos.GetPositionZ(),
                    0.0f);
        });
