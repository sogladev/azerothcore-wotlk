...
struct boss_zuljin : public BossAI
{
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        ScheduleTimedEvent(7s, [&] { DoCastAOE(SPELL_WHIRLWIND); }, 15s, 20s);

        ScheduleTimedEvent(8s, [&] { DoCastRandomTarget(SPELL_GRIEVOUS_THROW, 0, 100.0f); }, 10s);
            ...
            [&]
        {
            ...
            ScheduleTimedEvent(7s, [&] { DoCastAOE(SPELL_CREEPING_PARALYSIS); }, 20s);
        ...
        // Phase 3: Eagle Form.
        ScheduleHealthCheckEvent({60}, [&] { EnterPhase(PHASE_EAGLE); });
            ...
            [&]
        {
            ...
            ScheduleTimedEvent(5s, [&] { DoCastRandomTarget(SPELL_CLAW_RAGE_CHARGE); }, 15s, 20s);
            ...
            [&]
        {
            ...
            ScheduleTimedEvent(5s, [&] { DoCastSelf(SPELL_FLAME_WHIRL); }, 12s);

            ScheduleTimedEvent(6s, [&] { DoCastRandomTarget(SPELL_SUMMON_PILLAR); }, 10s);

            ScheduleTimedEvent(7s, [&] { DoCastAOE(SPELL_FLAME_BREATH); }, 10s);
        });
    }
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        me->m_Events.AddEventAtOffset([this] { summons.DespawnAll(); }, 3s);
    }
        ...
        for (auto const& spiritInfo : SpiritInfo)
        {
            if (Creature* creature = me->SummonCreature(spiritInfo.entry, spiritInfo.pos, TEMPSUMMON_DEAD_DESPAWN, 0))
            {
                creature->m_Events.AddEventAtOffset(
                    [creature] { creature->CastSpell(creature, SPELL_SPIRIT_AURA); }, 1s);
            }
        }
    }
        ...
        if (type == POINT_MOTION_TYPE && id == POINT_CENTER)
        {
                ...
                [&]
            {
                ...
                if (_nextPhase == PHASE_EAGLE)
                {
                    ...
                    {
                        if (Creature* vortex =
                                DoSpawnCreature(CREATURE_FEATHER_VORTEX, 0, 0, 0, 0, TEMPSUMMON_CORPSE_DESPAWN, 0))
                        {
    ...
    void EnterPhase(uint32 NextPhase)
    {
        ...
        me->m_Events.AddEventAtOffset([&] { me->GetMotionMaster()->MovePoint(POINT_CENTER, CenterPosition); }, 1s);
...
class spell_claw_rage_aura : public AuraScript
{
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_claw_rage_aura::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
