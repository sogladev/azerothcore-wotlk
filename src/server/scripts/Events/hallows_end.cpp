...
public:
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_hallows_end_put_costume::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
...
class spell_hallows_end_trick_or_treat : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_hallows_end_trick_or_treat::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_hallows_end_candy_pirate_costume : public AuraScript
{
    ...
    void HandleEffectApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        if (Unit* target = GetTarget())
        {
            target->CastSpell(target,
                target->getGender() == GENDER_MALE ? SPELL_HALLOWS_END_CANDY_3_MALE : SPELL_HALLOWS_END_CANDY_3_FEMALE,
                true);
        }
    }
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(spell_hallows_end_candy_pirate_costume::HandleEffectApply,
            EFFECT_0,
            SPELL_AURA_MOD_INCREASE_SWIM_SPEED,
            AURA_EFFECT_HANDLE_REAL);
        OnEffectRemove += AuraEffectRemoveFn(spell_hallows_end_candy_pirate_costume::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_MOD_INCREASE_SWIM_SPEED,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_hallows_end_tricky_treat : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_hallows_end_tricky_treat::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_hallows_end_base_fire : public AuraScript
{
    ...
    void Register() override
    {
        DoEffectCalcPeriodic +=
            AuraEffectCalcPeriodicFn(spell_hallows_end_base_fire::CalcPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
        OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(
            spell_hallows_end_base_fire::HandleEffectPeriodicUpdate, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
        OnEffectApply += AuraEffectApplyFn(spell_hallows_end_base_fire::HandleEffectApply,
            EFFECT_0,
...
struct npc_costumed_orphan_matron : public ScriptedAI
{
    ...
    void DoAction(int32 param) override
    {
        if (param == ACTION_START_EVENT)
        {
            ...
            if (Creature* cr =
                    me->SummonCreature(NPC_SHADE_OF_HORSEMAN, x, y, z, o, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10000))
            {
...
struct boss_headless_horseman : public ScriptedAI
{
    ...
    void JustDied(Unit* /*killer*/) override
    {
        ...
        if (!players.IsEmpty() && players.begin()->GetSource() && players.begin()->GetSource()->GetGroup())
            sLFGMgr->FinishDungeon(
                players.begin()->GetSource()->GetGroup()->GetGUID(), lfg::LFG_DUNGEON_HEADLESS_HORSEMAN, me->FindMap());
    ...
    {
        return playerGUID ? ObjectAccessor::GetPlayer(*me, playerGUID) : nullptr;
    }
            ...
            case EVENT_HH_PLAYER_TALK:
            {
                ...
                switch (talkCount)
                {
                    ...
                    case 2:
                        ...
                        if (Creature* trigger =
                                me->SummonTrigger(1765.28f, 1347.46f, 17.5514f, 0.0f, 15 * IN_MILLISECONDS))
...
struct boss_headless_horseman_pumpkin : public ScriptedAI
{
    ...
    void SpellHit(Unit* /*caster*/, SpellInfo const* spellInfo) override
    {
        ...
        {
            if (Creature* cr = me->SummonCreature(NPC_PUMPKIN_FIEND,
                    me->GetPositionX(),
                    me->GetPositionY(),
                    me->GetPositionZ(),
                    0.0f,
...
public:
    ...
    bool OnQuestReward(Player* player, GameObject* go, Quest const* /*quest*/, uint32 /*opt*/) override
    {
        ...
        if (Creature* horseman = go->SummonCreature(
                NPC_HEADLESS_HORSEMAN_MOUNTED, 1754.00f, 1346.00f, 17.50f, 0.0f, TEMPSUMMON_MANUAL_DESPAWN, 0))
