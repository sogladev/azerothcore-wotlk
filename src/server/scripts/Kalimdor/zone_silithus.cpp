            ...
            if (!eventEnd)
            {
                switch (AnimationCount)
                {
                    ...
                    case 51:
                    {
                        ...
                        {
                            for (std::list<Creature*>::const_iterator itr = constructList.begin();
                    ...
                    case 65:
                        ...
                        if (Creature* AnachronosQuestTrigger =
                                (ObjectAccessor::GetCreature(*me, AnachronosQuestTriggerGUID)))
                        {
                            Talk(ARYGOS_YELL_1);
    ...
    struct npc_qiraj_war_spawnAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            if (!Timers)
            {
                ...
                if (me->GetEntry() == 15423 || me->GetEntry() == 15424 || me->GetEntry() == 15422 ||
                    me->GetEntry() == 15414)
    ...
    struct npc_anachronos_quest_triggerAI : public ScriptedAI
    {
        ...
        void SummonNextWave()
        {
            ...
            for (uint8 i = locIndex; i <= count; ++i)
            {
                ...
                if (Creature* spawn = me->SummonCreature(
                        WavesInfo[WaveCount].CreatureId, SpawnLocation[i], TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, desptimer))
                {
                    ...
                    {
                        if (npc_qiraj_war_spawn::npc_qiraj_war_spawnAI* spawnAI =
                                CAST_AI(npc_qiraj_war_spawn::npc_qiraj_war_spawnAI, spawn->AI()))
                        {
...
void npc_qiraj_war_spawn::npc_qiraj_war_spawnAI::JustDied(Unit* /*slayer*/)
{
    ...
    if (Creature* mob = ObjectAccessor::GetCreature(*me, MobGUID))
        if (npc_anachronos_quest_trigger::npc_anachronos_quest_triggerAI* triggerAI =
                CAST_AI(npc_anachronos_quest_trigger::npc_anachronos_quest_triggerAI, mob->AI()))
...
public:
    ...
    bool OnQuestAccept(Player* player, GameObject* go, Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_A_PAWN_ON_THE_ETERNAL_BOARD)
        {
            ...
            {
                Unit* Merithra = trigger->SummonCreature(
                    15378, -8034.535f, 1535.14f, 2.61f, 0, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
                Unit* Caelestrasz = trigger->SummonCreature(
                    15379, -8032.767f, 1533.148f, 2.61f, 1.5f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
                Unit* Arygos = trigger->SummonCreature(
                    15380, -8034.52f, 1537.843f, 2.61f, 5.7f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
                /* Unit* Fandral = */ trigger->SummonCreature(
                    15382, -8028.462f, 1535.843f, 2.61f, 3.141592f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
                Creature* Anachronos = trigger->SummonCreature(
                    15381, -8028.75f, 1538.795f, 2.61f, 4, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 220000);
                ...
                if (Anachronos)
                {
                    if (npc_anachronos_the_ancient::npc_anachronos_the_ancientAI* anachronosAI =
                            CAST_AI(npc_anachronos_the_ancient::npc_anachronos_the_ancientAI, Anachronos->AI()))
                        anachronosAI->PlayerGUID = player->GetGUID();

                    if (npc_anachronos_quest_trigger::npc_anachronos_quest_triggerAI* triggerAI =
                            CAST_AI(npc_anachronos_quest_trigger::npc_anachronos_quest_triggerAI, trigger->AI()))
                    {
...
public:
    ...
    struct go_wind_stoneAI : public GameObjectAI
    {
            ...
            if (sender == GOSSIPID_LESSER_WS)
            {
                ...
                switch (action)
                {
                    case 0:
                        SummonNPC(me,
                            player,
                            RAND(NPC_TEMPLAR_WATER, NPC_TEMPLAR_FIRE, NPC_TEMPLAR_EARTH, NPC_TEMPLAR_AIR),
            ...
            else if (sender == GOSSIPID_WS)
            {
                ...
                switch (action)
                {
                    case 0:
                        SummonNPC(me,
                            player,
                            RAND(NPC_DUKE_WATER, NPC_DUKE_FIRE, NPC_DUKE_EARTH, NPC_DUKE_AIR),
                            SPELL_DUKE_RANDOM);
                        break;
                    case 1:
            ...
            else if (sender == GOSSIPID_GREATER_WS)
            {
                ...
                switch (action)
                {
                    case 0:
                        SummonNPC(me,
                            player,
                            RAND(NPC_ROYAL_WATER, NPC_ROYAL_FIRE, NPC_ROYAL_EARTH, NPC_ROYAL_AIR),
                            SPELL_ROYAL_RANDOM);
                        break;
                    case 1:
        ...
        void SummonNPC(GameObject* go, Player* player, uint32 npc, uint32 spellId)
        {
            ...
            if (TempSummon* summons = go->SummonCreature(npc,
                    go->GetPositionX(),
                    go->GetPositionY(),
                    go->GetPositionZ(),
                    ...
                    6000))
            {
                ...
                summons->m_Events.AddEvent(
                    new DelayedWindstoneSummonEvent(summons, player->GetGUID()), summons->m_Events.CalculateTime(5200));
