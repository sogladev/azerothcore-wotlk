...
protected:
    ...
    virtual Creature* GetSpeakerByEntry(int32 /*entry*/)
    {
        return nullptr;
    }
...
public:
    ...
    struct npc_ranshallaAI : public npc_escortAI, private DialogueHelper
    {
        ...
        void DoSummonPriestess()
        {
            ...
            if (Creature* priestess = me->SummonCreature(NPC_PRIESTESS_ELUNE,
                    ...
                    0))
            {
                priestess->GetMotionMaster()->MovePoint(0,
                    ...
                    0))
            {
                ...
                priestess->GetMotionMaster()->MovePoint(1,
        ...
        void WaypointReached(uint32 pointId) override
        {
            switch (pointId)
            {
                ...
                case 41:
                {
                    ...
                    for (std::list<GameObject*>::const_iterator itr = eluneLights.begin(); itr != eluneLights.end();
                         ++itr)
                    {
                        if ((*itr)->isSpawned())
                            continue;
        ...
        void JustDidDialogueStep(int32 entry) override
        {
            switch (entry)
            {
                ...
                case SAY_PRIESTESS_ALTAR_13:
                    ...
                    if (Creature* guard = me->SummonCreature(NPC_GUARDIAN_ELUNE,
                            ...
                            0))
                    {
                        guard->GetMotionMaster()->MovePoint(0,
                    ...
                    if (GameObject* altar = me->GetMap()->GetGameObject(_altarGUID))
                    {
                        if (Creature* voice = me->SummonCreature(NPC_VOICE_ELUNE,
                                altar->GetPositionX(),
                                altar->GetPositionY(),
                                altar->GetPositionZ(),
                                0,
    ...
    bool OnGossipHello(Player* /*player*/, GameObject* go) override
    {
        ...
        if (Creature* ranshalla = GetClosestCreatureWithEntry(go, NPC_RANSHALLA, 10.0f))
        {
            if (npc_ranshalla::npc_ranshallaAI* escortAI =
                    dynamic_cast<npc_ranshalla::npc_ranshallaAI*>(ranshalla->AI()))
