...
public:
    ...
    struct npc_arthasAI : public npc_escortAI
    {
        ...
        {
            if (pInstance && pInstance->GetData(DATA_ARTHAS_EVENT) > COS_PROGRESS_FINISHED_INTRO &&
                pInstance->GetData(DATA_ARTHAS_EVENT) < COS_PROGRESS_REACHED_TOWN_HALL)
        ...
        void WaypointReached(uint32 uiPointId) override
        {
            switch (uiPointId)
            {
                ...
                case 8:
                    ...
                    if (Creature* stalker = me->SummonCreature(
                            NPC_INVIS_TARGET, 2026.469f, 1287.088f, 143.596f, 1.37f, TEMPSUMMON_TIMED_DESPAWN, 14000))
                    {
                ...
                case 36:
                    ...
                    if (pInstance)
                        if (GameObject* pGate =
                                pInstance->instance->GetGameObject(pInstance->GetGuidData(DATA_SHKAF_GATE)))
            ...
            if (eventInRun)
            {
                ...
                switch (uint32 currentEvent = actionEvents.ExecuteEvent())
                {
                    case EVENT_ACTION_PHASE1:
                        ...
                        if (Creature* uther = me->SummonCreature(
                                NPC_UTHER, EventPos[EVENT_SRC_UTHER], TEMPSUMMON_DEAD_DESPAWN, 180000))
                        {
                        ...
                        for (int i = 0; i < 3; ++i)
                            if (Creature* horse = me->SummonCreature(
                                    NPC_HORSE_ESCORT, EventPos[EVENT_SRC_HORSE1 + i], TEMPSUMMON_DEAD_DESPAWN, 180000))
                    ...
                    case EVENT_ACTION_PHASE2 + 3:
                        if (Creature* stalker = me->SummonCreature(NPC_INVIS_TARGET,
                                2081.447f,
                                1287.770f,
                                141.3241f,
                                1.37f,
                    ...
                    case EVENT_ACTION_PHASE2 + 4:
                        if (Creature* malganis = me->SummonCreature(
                                NPC_MAL_GANIS, EventPos[EVENT_SRC_MALGANIS], TEMPSUMMON_TIMED_DESPAWN, 60000))
                        {
                            ...
                            for (std::list<Creature*>::const_iterator itr = unitList.begin(); itr != unitList.end();
                                 ++itr)
                                if ((*itr)->ToCreature())
                                {
                    ...
                    case EVENT_ACTION_PHASE5 + 2:
                        ...
                        if (pInstance)
                        {
                            ...
                            if (GameObject* go =
                                    pInstance->instance->GetGameObject(pInstance->GetGuidData(DATA_EXIT_GATE)))
                            ...
                            if (!me->GetMap()->GetPlayers().IsEmpty())
                            {
                                if (Player* player = me->GetMap()->GetPlayers().getFirst()->GetSource())
                                {
                                    if (GameObject* chest = player->SummonGameObject(
                                            DUNGEON_MODE(GO_MALGANIS_CHEST_N, GO_MALGANIS_CHEST_H),
                                            2288.35f,
                                            1498.73f,
                                            128.414f,
                                            -0.994837f,
...
void npc_arthas::npc_arthasAI::SpawnTimeRift()
{
    ...
    for (uint8 i = 0;
         i < uint8(timeRiftId == 0 ? ENCOUNTER_CHRONO_MAX_SPAWNS_FIRST : ENCOUNTER_CHRONO_MAX_SPAWNS_SECOND);
         ++i)
    {
        // Spawn everyone at time rift pos
        if (Creature* cr = me->SummonCreature(/*entry*/ (uint32)RiftAndSpawnsLocations[timeRiftId][i][0],
                ...
                RiftAndSpawnsLocations[timeRiftId][0][4]))
        {
            ...
            else // x, y, z (0 is entry)
            {
                ...
                cr->AddThreat(me, 0.0f);
                cr->SetReactState(REACT_AGGRESSIVE);
                cr->GetMotionMaster()->MovePoint(POINT_CHRONOS,
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 /*action*/) override
    {
        if (!creature->GetInstanceScript() ||
            creature->GetInstanceScript()->GetData(DATA_ARTHAS_EVENT) != COS_PROGRESS_CRATES_FOUND)
            return true;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (allowTimer)
            {
                ...
                if (allowTimer >= 8000 && pInstance &&
                    pInstance->GetData(DATA_ARTHAS_EVENT) >= COS_PROGRESS_FINISHED_CITY_INTRO)
                    allowTimer = 0;
            }
