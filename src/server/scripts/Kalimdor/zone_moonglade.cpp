    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        if (player->GetTeamId() != TEAM_HORDE)
        {
            if (player->IsClass(CLASS_DRUID, CLASS_CONTEXT_QUEST) &&
                player->GetQuestStatus(QUEST_SEA_LION_ALLY) == QUEST_STATUS_INCOMPLETE)
            {
        ...
        else if (player->GetTeamId() == TEAM_HORDE)
        {
            ...
            if (player->IsClass(CLASS_DRUID, CLASS_CONTEXT_QUEST) &&
                player->GetQuestStatus(QUEST_SEA_LION_HORDE) == QUEST_STATUS_INCOMPLETE)
            {
...
public:
    ...
    public:
        ...
        void IsSummonedBy(WorldObject* /*summoner*/) override
        {
            ...
            for (std::list<Player*>::const_iterator itr = playerOnQuestList.begin(); itr != playerOnQuestList.end();
                 ++itr)
            {
                        ...
                        switch (Step)
                        {
                            case 0:
                                if (Creature* mob = me->SummonCreature(
                                        ASPECT_RAVEN, AspectRavenSummon, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 2000))
                                {
