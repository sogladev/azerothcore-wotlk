...
public:
    ...
    static bool HandleSkirmishCommand(ChatHandler* handler, std::vector<std::string_view> args)
    {
        ...
        Battleground* bg = sBattlegroundMgr->CreateNewBattleground(randomizedArenaBgTypeId,
        ...
        for (uint8 i = 0; i < cnt; ++i)
        {
            ...
            sBattlegroundMgr->BuildBattlegroundStatusPacket(
                &data, bg, queueSlot, STATUS_IN_PROGRESS, 0, bg->GetStartTime(), bg->GetArenaType(), teamId);
    ...
    static bool HandleGPSCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
    {
        ...
        uint32 haveMMAP =
            MMAP::MMapFactory::createOrGetMMapMgr()->GetNavMesh(handler->GetSession()->GetPlayer()->GetMapId()) ? 1 : 0;
        ...
        handler->PSendSysMessage(LANG_MAP_POSITION,
            object->GetMapId(),
            (mapEntry ? mapEntry->name[handler->GetSessionDbcLocale()] : "<unknown>"),
            zoneId,
            (zoneEntry ? zoneEntry->area_name[handler->GetSessionDbcLocale()] : "<unknown>"),
            areaId,
            (areaEntry ? areaEntry->area_name[handler->GetSessionDbcLocale()] : "<unknown>"),
            object->GetPhaseMask(),
            object->GetPositionX(),
            object->GetPositionY(),
            object->GetPositionZ(),
        ...
        {
            handler->PSendSysMessage("Transport offset: {:0.2f}, {:0.2f}, {:0.2f}, {:0.2f}",
    ...
    static bool HandleAppearCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
    {
        ...
        if (target->IsConnected())
        {
            ...
            if (map->IsBattlegroundOrArena())
            {
                ...
                _player->SetBattlegroundId(targetPlayer->GetBattlegroundId(),
            ...
            else if (map->IsDungeon())
            {
                ...
                InstancePlayerBind* bind = sInstanceSaveMgr->PlayerGetBoundInstance(
                    _player->GetGUID(), targetPlayer->GetMapId(), targetPlayer->GetDifficulty(map->IsRaid()));
                if (!bind)
                {
                    if (InstanceSave* save =
                            sInstanceSaveMgr->GetInstanceSave(target->GetConnectedPlayer()->GetInstanceId()))
                    {
    ...
    static bool HandleSummonCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
    {
        ...
        if (target->IsConnected())
        {
            ...
            else if (map->IsDungeon())
            {
                ...
                {
                    sInstanceSaveMgr->PlayerUnbindInstance(target->GetGUID(),
                        map->GetInstanceId(),
            ...
            if (handler->needReportToTarget(targetPlayer))
            {
                ChatHandler(targetPlayer->GetSession())
                    .PSendSysMessage(LANG_SUMMONED_BY, handler->playerLink(_player->GetName()));
            }
            ...
            targetPlayer->TeleportTo(handler->GetSession()->GetPlayer()->GetMapId(),
                x,
                y,
                z,
    ...
    static bool HandleGroupSummonCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
    {
        ...
        if (toInstance && (!handler->GetSession()->GetPlayer()->GetGroup() ||
                              (group->GetLeaderGUID() != handler->GetSession()->GetPlayer()->GetGUID()) ||
                              (handler->GetSession()->GetPlayer()->GetGroup()->GetLeaderGUID() !=
                                  handler->GetSession()->GetPlayer()->GetGUID())))
        // the last check is a bit excessive, but let it be, just in case
        {
    ...
    static bool HandleCooldownCommand(ChatHandler* handler, Optional<SpellInfo const*> spell)
    {
        ...
        else
        {
            ...
            handler->PSendSysMessage(LANG_REMOVE_COOLDOWN,
                spell.value()->Id,
                target == handler->GetSession()->GetPlayer() ? handler->GetAcoreString(LANG_YOU) : nameLink);
        }
        return true;
    }
    ...
    static bool HandleGetDistanceCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
    {
        ...
        handler->PSendSysMessage(LANG_DISTANCE,
    ...
    static bool HandleSaveCommand(ChatHandler* handler)
    {
        ...
        if (saveInterval == 0 ||
            (saveInterval > 20 * IN_MILLISECONDS && player->GetSaveTimer() <= saveInterval - 20 * IN_MILLISECONDS))
        {
            player->SaveToDB(false, false);
        }
    ...
    static bool HandleKickPlayerCommand(
        ChatHandler* handler, Optional<PlayerIdentifier> target, Optional<std::string_view> reason)
    {
        if (!target)
            target = PlayerIdentifier::FromTargetOrSelf(handler);
        ...
        {
            handler->SendWorldText(LANG_COMMAND_KICKMESSAGE_WORLD,
                (handler->GetSession() ? handler->GetSession()->GetPlayerName() : "Server"),
                target->GetName(),
                kickReasonStr);
        }
        else
    ...
    static bool HandleUnstuckCommand(
        ChatHandler* handler, Optional<PlayerIdentifier> target, Optional<std::string_view> location)
    {
        ...
        if (!target || !target->IsConnected())
        {
            ...
            CharacterDatabasePreparedStatement* stmt =
                CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_HOMEBIND);
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, Optional<PlayerIdentifier> player,
        ItemTemplate const* itemTemplate, Optional<int32> _count)
    {
    ...
    static bool HandleSetSkillCommand(
        ChatHandler* handler, Variant<Hyperlink<skill>, uint32> skillId, int32 level, Optional<uint16> maxPureSkill)
    {
        ...
        handler->PSendSysMessage(LANG_SET_SKILL,
            skillID,
    ...
    static bool HandlePInfoCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
    {
        ...
        handler->PSendSysMessage(LANG_PINFO_PLAYER,
        ...
        if (banTime >= 0)
        {
            handler->PSendSysMessage(LANG_PINFO_BANNED,
                banType,
                banReason,
                banTime > 0 ? secsToTimeString(banTime - GameTime::GetGameTime().count(), true)
        ...
        if (muteTime > 0)
        {
            handler->PSendSysMessage(LANG_PINFO_MUTED,
                muteReason,
                secsToTimeString(muteTime - GameTime::GetGameTime().count(), true),
                muteBy);
        }
        ...
        handler->PSendSysMessage(
            LANG_PINFO_ACC_IP, lastIp, locked ? handler->GetAcoreString(LANG_YES) : handler->GetAcoreString(LANG_NO));
        ...
        handler->PSendSysMessage(LANG_PINFO_CHR_RACE,
            (gender == 0 ? handler->GetAcoreString(LANG_CHARACTER_GENDER_MALE)
    ...
    static bool HandleMuteCommand(
        ChatHandler* handler, Optional<PlayerIdentifier> player, std::string notSpeakTime, Tail muteReason)
    {
        ...
        if (target)
        {
            ...
            {
                handler->SendWorldText(LANG_COMMAND_MUTEMESSAGE_WORLD,
                    muteBy,
                    nameLink,
            ...
            ChatHandler(target->GetSession())
                .PSendSysMessage(LANG_YOUR_CHAT_DISABLED, secsToTimeString(muteDuration, true), muteBy, muteReasonStr);
        }
        else
        {
        ...
        {
            handler->SendWorldText(
                LANG_COMMAND_MUTEMESSAGE_WORLD, muteBy, nameLink, secsToTimeString(muteDuration, true), muteReasonStr);
        }
        else
        {
            ...
            for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
                if (itr->second->GetSession()->GetSecurity())
                    ChatHandler(itr->second->GetSession())
                        .PSendSysMessage(target ? LANG_YOU_DISABLE_CHAT : LANG_COMMAND_DISABLE_CHAT_DELAYED,
    ...
    static bool HandleUnmuteCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
    {
        ...
        Player* playerTarget = target->GetConnectedPlayer();
        uint32 accountId = playerTarget ? playerTarget->GetSession()->GetAccountId()
                                        : sCharacterCache->GetCharacterAccountIdByGuid(target->GetGUID());
    ...
    static bool HandleMuteInfoHelper(ChatHandler* handler, uint32 accountId, char const* accountName)
    {
        ...
        do
        {
            ...
            handler->PSendSysMessage(LANG_COMMAND_MUTEHISTORY_OUTPUT,
                Acore::Time::TimeToHumanReadable(Seconds(fields[0].Get<uint32>())),
                fields[1].Get<uint32>(),
    ...
    static bool HandleMovegensCommand(ChatHandler* handler)
    {
        ...
        handler->PSendSysMessage(
            LANG_MOVEGENS_LIST, (unit->IsPlayer() ? "Player" : "Creature"), unit->GetGUID().ToString());
        ...
        for (uint8 i = 0; i < MAX_MOTION_SLOT; ++i)
        {
            ...
            switch (movementGenerator->GetMovementGeneratorType())
            {
                ...
                case CHASE_MOTION_TYPE:
                {
                    ...
                    {
                        handler->PSendSysMessage(
                            LANG_MOVEGENS_CHASE_PLAYER, target->GetName(), target->GetGUID().ToString());
                    }
                    else
                    {
                        handler->PSendSysMessage(
                            LANG_MOVEGENS_CHASE_CREATURE, target->GetName(), target->GetGUID().ToString());
                    }
                    break;
                }
                case FOLLOW_MOTION_TYPE:
                {
                    ...
                    {
                        handler->PSendSysMessage(
                            LANG_MOVEGENS_FOLLOW_PLAYER, target->GetName(), target->GetGUID().ToString());
                    }
                    else
                    {
                        handler->PSendSysMessage(
                            LANG_MOVEGENS_FOLLOW_CREATURE, target->GetName(), target->GetGUID().ToString());
                    }
                    break;
                }
    ...
    */
    ...
    static bool HandleDamageCommand(ChatHandler* handler, uint32 damage, Optional<std::string> percent)
    {
        ...
        if (target != handler->GetSession()->GetPlayer())
            handler->GetSession()->GetPlayer()->SendAttackStateUpdate(
                HITINFO_AFFECTS_VICTIM, target, 1, SPELL_SCHOOL_MASK_NORMAL, damage, 0, 0, VICTIMSTATE_HIT, 0);
    ...
    static bool HandleFreezeCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
    {
        ...
        {
            handler->PSendSysMessage(
                LANG_COMMAND_FREEZE, GetLocalizeCreatureName(creatureTarget, handler->GetSessionDbcLocale()));
    ...
    static bool HandleUnFreezeCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
    {
        ...
        {
            handler->PSendSysMessage(
                LANG_COMMAND_UNFREEZE, GetLocalizeCreatureName(creatureTarget, handler->GetSessionDbcLocale()));
        ...
        {
            CharacterDatabasePreparedStatement* stmt =
                CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_AURA_FROZEN);
            stmt->SetData(0, target->GetGUID().GetCounter());
    ...
    static bool HandleStringCommand(ChatHandler* handler, uint32 id, Optional<uint8> locale)
    {
        ...
        char const* str =
            sObjectMgr->GetAcoreString(id, locale ? static_cast<LocaleConstant>(*locale) : DEFAULT_LOCALE);
