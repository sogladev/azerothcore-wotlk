...
struct boss_ambassador_hellmaw : public BossAI
{
    ...
    void InitializeAI() override
    {
        ...
        if (instance->GetPersistentData(TYPE_RITUALISTS) != DONE)
        {
            ...
            me->m_Events.AddEventAtOffset([this]() { DoCastSelf(SPELL_BANISH, true); }, 500ms);
        }
        else
        {
    ...
    void JustEngagedWith(Unit*) override
    {
        ...
        if (IsHeroic())
            scheduler.Schedule(3min, [this](TaskContext /*context*/) { DoCastSelf(SPELL_ENRAGE, true); });
        _JustEngagedWith();
    }
    ...
    void PathEndReached(uint32 pathId) override
    {
        if (pathId == PATH_ID_START)
            me->m_Events.AddEventAtOffset([this]() { me->GetMotionMaster()->MovePath(PATH_ID_PATHING, true); }, 20s);
    }
