                            ...
                            [this](TaskContext /*context*/)
                        {
                            if (TempSummon* summon = me->ToTempSummon())
                            {
                                if (Unit* vorpil = summon->GetSummonerUnit())
                                {
                                    ...
                                    me->CastSpell(
                                        vorpil, SPELL_EMPOWERING_SHADOWS, true, nullptr, nullptr, vorpil->GetGUID());
                                    vorpil->ModifyHealth(int32(vorpil->CountPctFromMaxHealth(4)));
                                }
                            }

                            _scheduler.Schedule(100ms, [this](TaskContext /*context*/) { me->KillSelf(); });
                        });
                    }
                    else
                    {
