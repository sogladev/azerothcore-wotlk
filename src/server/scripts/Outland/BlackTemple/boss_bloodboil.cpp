...
struct boss_gurtogg_bloodboil : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        ScheduleTimedEvent(
            14s, [&] { DoCastVictim(me->HasAura(SPELL_FEL_RAGE_SELF) ? SPELL_EJECT2 : SPELL_EJECT1); }, 20s);
            ...
            [&]
        {
            if (Unit* target = SelectTarget(SelectTargetMethod::Random, 1, 40.0f, true))
            {
                ...
                me->m_Events.AddEventAtOffset([&] { DoCastVictim(SPELL_CHARGE); }, 2s);
...
class spell_gurtogg_bloodboil : public SpellScript
{
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_gurtogg_bloodboil::FilterTargets, EFFECT_ALL, TARGET_UNIT_SRC_AREA_ENEMY);
    }
};
...
class spell_gurtogg_eject : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_gurtogg_eject::HandleScriptEffect, EFFECT_2, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
