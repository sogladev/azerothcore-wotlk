...
struct boss_mother_shahraz : public BossAI
{
    ...
    void Reset() override
    {
        ...
        ScheduleHealthCheckEvent(10, [&] { Talk(SAY_EMOTE_FRENZY); });
    }
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        ScheduleTimedEvent(4s, [&] { DoCastVictim(SPELL_SABER_LASH); }, 30s);

        ScheduleTimedEvent(1min, [&] { Talk(SAY_TAUNT); }, 1min, 2min);

        ScheduleTimedEvent(0s,
            [&]
        {
            DoCastSelf(RAND(SPELL_PRISMATIC_AURA_SHADOW,
        ...
        ScheduleTimedEvent(30s, [&] { DoCastAOE(SPELL_SILENCING_SHRIEK); }, 30s);
    ...
    void KilledUnit(Unit* /*victim*/) override
    {
        if (_canTalk)
        {
            ...
            ScheduleUniqueTimedEvent(6s, [&] { _canTalk = true; }, 1);
        }
    }
...
class spell_mother_shahraz_random_periodic_aura : public AuraScript
{
    ...
    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo(
            {SPELL_SINFUL_PERIODIC, SPELL_SINISTER_PERIODIC, SPELL_VILE_PERIODIC, SPELL_WICKED_PERIODIC});
    }
    ...
    void Update(AuraEffect const* effect)
    {
        ...
        if (GetUnitOwner() &&
            (effect->GetTickNumber() % 6 == 1 || effect->GetTickNumber() == 1)) // Reapplies 12-18s after the third beam
            GetUnitOwner()->CastSpell(GetUnitOwner(),
                RAND(SPELL_SINFUL_PERIODIC, SPELL_SINISTER_PERIODIC, SPELL_VILE_PERIODIC, SPELL_WICKED_PERIODIC),
                true);
    }
...
class spell_mother_shahraz_saber_lash_aura : public AuraScript
{
    ...
    void Register() override
    {
        ...
        OnEffectPeriodic += AuraEffectPeriodicFn(
            spell_mother_shahraz_saber_lash_aura::Update, EFFECT_1, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};

Position const validTeleportStairsPos[9] = {
    //Platform teleports
...
class spell_mother_shahraz_fatal_attraction : public SpellScript
{
    ...
    void SetDest(SpellDestination& dest)
    {
        ...
        {
            finalDest = GetCaster()->GetNearPosition(frand(minTeleportDist, maxTeleportDist),
                static_cast<float>(rand_norm()) * static_cast<float>(2 * M_PI),
                true);
            ...
            if (GetCaster()->GetExactDist2d(finalDest) < minTeleportDist)
            {
                if (finalDest.m_positionX == 932.f || finalDest.m_positionX == 960.f)
                    finalDest.m_positionY =
                        finalDest.m_positionY + (minTeleportDist - GetCaster()->GetExactDist2d(finalDest));
                else if (finalDest.m_positionY == 224.f || finalDest.m_positionY == 320.f)
                    finalDest.m_positionX =
                        finalDest.m_positionX + (minTeleportDist - GetCaster()->GetExactDist2d(finalDest));
            }
        }
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_mother_shahraz_fatal_attraction::FilterTargets, EFFECT_ALL, TARGET_UNIT_SRC_AREA_ENEMY);
        OnDestinationTargetSelect += SpellDestinationTargetSelectFn(
            spell_mother_shahraz_fatal_attraction::SetDest, EFFECT_1, TARGET_DEST_CASTER_RANDOM);
        OnEffectHitTarget += SpellEffectFn(
            spell_mother_shahraz_fatal_attraction::HandleTeleportUnits, EFFECT_1, SPELL_EFFECT_TELEPORT_UNITS);
    }
};
...
class spell_mother_shahraz_fatal_attraction_dummy : public SpellScript
{
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_mother_shahraz_fatal_attraction_dummy::FilterTargets, EFFECT_ALL, TARGET_UNIT_SRC_AREA_ALLY);
        OnEffectHitTarget +=
            SpellEffectFn(spell_mother_shahraz_fatal_attraction_dummy::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};
