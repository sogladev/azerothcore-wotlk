...
struct boss_hydross_the_unstable : public BossAI
{
    ...
    void SummonMovementInform(Creature* summon, uint32 movementType, uint32 pathId) override
    {
        if (movementType == WAYPOINT_MOTION_TYPE)
        {
            if (pathId == PATH_CENTER)
            {
                ...
                summon->m_Events.AddEventAtOffset([summon] { summon->UpdateEntry(NPC_PURIFIED_WATER_ELEMENTAL); }, 1s);
            }
    ...
    void SetForm(bool corrupt, bool initial)
    {
        ...
        if (corrupt)
        {
            ...
            scheduler.Schedule(15s, GROUP_ABILITIES, [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_CORRUPTION1); })
                .Schedule(30s, GROUP_ABILITIES, [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_CORRUPTION2); })
                .Schedule(45s, GROUP_ABILITIES, [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_CORRUPTION3); })
                .Schedule(60s, GROUP_ABILITIES, [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_CORRUPTION4); })
                .Schedule(75s, GROUP_ABILITIES, [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_CORRUPTION5); })
                .Schedule(90s,
                    GROUP_ABILITIES,
        ...
        else
        {
            ...
            scheduler.Schedule(15s, GROUP_ABILITIES, [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_HYDROSS1); })
                .Schedule(30s, GROUP_ABILITIES, [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_HYDROSS2); })
                .Schedule(45s, GROUP_ABILITIES, [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_HYDROSS3); })
                .Schedule(60s, GROUP_ABILITIES, [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_HYDROSS4); })
                .Schedule(75s, GROUP_ABILITIES, [this](TaskContext) { DoCastSelf(SPELL_MARK_OF_HYDROSS5); })
                .Schedule(90s,
                    GROUP_ABILITIES,
                    [this](TaskContext context)
            {
...
class spell_hydross_cleansing_field_aura : public AuraScript
{
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(
            spell_hydross_cleansing_field_aura::HandleEffectApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
        AfterEffectRemove += AuraEffectRemoveFn(spell_hydross_cleansing_field_aura::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
...
class spell_hydross_cleansing_field_command : public AuraScript
{
    ...
    void Register() override
    {
        AfterEffectRemove += AuraEffectApplyFn(spell_hydross_cleansing_field_command::HandleEffectRemove,
            EFFECT_0,
            SPELL_AURA_DUMMY,
            AURA_EFFECT_HANDLE_REAL);
    }
...
class spell_hydross_mark_of_hydross : public AuraScript
{
    ...
    void Register() override
    {
        OnEffectApply += AuraEffectApplyFn(spell_hydross_mark_of_hydross::HandleEffectApply,
            EFFECT_0,
