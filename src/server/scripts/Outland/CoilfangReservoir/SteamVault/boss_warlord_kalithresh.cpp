...
struct boss_warlord_kalithresh : public BossAI
{
    ...
    {
        _Reset();
        instance->DoForAllMinions(DATA_WARLORD_KALITHRESH,
            [&](Creature* minion)
        {
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
                ...
                [this](TaskContext context)
        {
            ...
            else if (Creature* distiller = me->FindNearestCreature(NPC_NAGA_DISTILLER, 100.0f))
            {
                me->GetMotionMaster()->MoveFollow(distiller, 8.0f, 0.0f);

                scheduler.Schedule(1s,
                    [this](TaskContext chaseContext)
                {
                    if (Creature* distiller = me->FindNearestCreature(NPC_NAGA_DISTILLER, 8.0f))
                    {
                        distiller->AI()->DoCast(me, SPELL_WARLORDS_RAGE_DISTILLER, true);
    ...
    void JustDied(Unit* /*killer*/) override
    {
        Talk(SAY_DEATH);
        _JustDied();
        instance->DoForAllMinions(DATA_WARLORD_KALITHRESH, [&](Creature* minion) { minion->KillSelf(); });
    }
...
class spell_warlords_rage : public AuraScript
{
    ...
    void Register() override
    {
        AfterEffectRemove += AuraEffectRemoveFn(
            spell_warlords_rage::HandleAfterRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
};
