...
struct boss_alar : public BossAI
{
    ...
    void JustDied(Unit* killer) override
    {
        ...
        me->GetMap()->DoForAllPlayers([&](Player* player)
        {
            if (player->GetQuestStatus(QUEST_RUSE_OF_THE_ASHTONGUE) == QUEST_STATUS_INCOMPLETE &&
                player->HasAura(SPELL_ASHTONGUE_RUSE))
        ...
        SpellSchoolMask /*damageSchoolMask*/) override
    {
        if (damage >= me->GetHealth() && _platform < POINT_MIDDLE)
        {
            ...
            if (!_hasPretendedToDie)
            {
                ...
                PretendToDie(me);
                _transitionScheduler.Schedule(1s, [this](TaskContext) { me->SetVisible(false); })
                    .Schedule(8s, [this](TaskContext) { me->SetPosition(alarPoints[POINT_MIDDLE]); })
                    .Schedule(12s,
                        [this](TaskContext)
                {
    ...
    {
        _transitionScheduler.CancelAll();
        ScheduleTimedEvent(57s, [&] { DoCastVictim(SPELL_MELT_ARMOR); }, 60s);
        ScheduleTimedEvent(10s, [&] { DoCastRandomTarget(SPELL_CHARGE, 0, 50.0f); }, 30s);
        ...
        me->m_Events.AddEventAtOffset([&] { DoCastSelf(SPELL_BERSERK, true); }, 10min);
    ...
    Position DeterminePhoenixPosition(Position playerPosition)
    {
        ...
        if (float playerFraction = deltaX / deltaY)
        {
            ...
            finalPosition = {centreXPosition + signMultiplier[0] * phoenixDeltaXPosition,
...
class spell_alar_flame_quills : public AuraScript
{
    ...
    void HandlePeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        for (uint8 i = 0; i < 21; ++i)
            GetUnitOwner()->m_Events.AddEvent(new CastQuill(GetUnitOwner(), SPELL_QUILL_MISSILE_1 + i),
                GetUnitOwner()->m_Events.CalculateTime(i * 40));
        GetUnitOwner()->m_Events.AddEvent(
            new CastQuill(GetUnitOwner(), SPELL_QUILL_MISSILE_2 + 0), GetUnitOwner()->m_Events.CalculateTime(22 * 40));
        GetUnitOwner()->m_Events.AddEvent(
            new CastQuill(GetUnitOwner(), SPELL_QUILL_MISSILE_2 + 1), GetUnitOwner()->m_Events.CalculateTime(23 * 40));
        GetUnitOwner()->m_Events.AddEvent(
            new CastQuill(GetUnitOwner(), SPELL_QUILL_MISSILE_2 + 2), GetUnitOwner()->m_Events.CalculateTime(24 * 40));
    }

    void Register() override
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_alar_flame_quills::HandlePeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
...
class spell_alar_dive_bomb : public AuraScript
{
    ...
    {
        OnEffectApply +=
            AuraEffectApplyFn(spell_alar_dive_bomb::OnApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
};
