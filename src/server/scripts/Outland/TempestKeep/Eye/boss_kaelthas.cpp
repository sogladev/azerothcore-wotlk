...
struct boss_kaelthas : public BossAI
{
    ...
    void MovementInform(uint32 type, uint32 point) override
    {
        ...
        else if (point == POINT_START_LAST_PHASE)
        {
            ...
            ScheduleTimedEvent(0ms, [&] { DoCastVictim(SPELL_FIREBALL); }, 2400ms, 7500ms);
            ScheduleTimedEvent(10000ms, [&] { DoCastRandomTarget(SPELL_FLAME_STRIKE, 0, 100.0f); }, 30250ms, 50650ms);
                ...
                [&]
            {
                        ...
                        [this](TaskContext context)
                {
                    ...
                    .Schedule(20500ms,
                        GROUP_SHOCK_BARRIER,
                        [this](TaskContext) { scheduler.CancelGroup(GROUP_SHOCK_BARRIER); })
                    .Schedule(32s,
                        [this](TaskContext)
                {
    ...
    void ExecuteMiddleEvent()
    {
            ...
            [&]
        {
            ...
            for (uint8 i = 0; i < 2; ++i)
                if (Creature* trigger =
                        me->SummonCreature(WORLD_TRIGGER, triggersPos[i], TEMPSUMMON_TIMED_DESPAWN, 60000))
                    trigger->CastSpell(me, SPELL_NETHERBEAM1 + i, false);
            me->SetDisableGravity(true);
            me->SendMovementFlagUpdate();
            me->GetMotionMaster()->MoveTakeoff(POINT_AIR,
                me->GetPositionX(),
                me->GetPositionY(),
                75.0f,
                2.99,
                true); // AnimType Movement::ToFly does not exist for Kael
            ...
            [&]
        {
            ...
            for (uint8 i = 0; i < 2; ++i)
                if (Creature* trigger =
                        me->SummonCreature(WORLD_TRIGGER, triggersPos[i + 2], TEMPSUMMON_TIMED_DESPAWN, 60000))
                    trigger->CastSpell(me, SPELL_NETHERBEAM1 + i, false);
        },
            EVENT_SCENE_4);
            ...
            [&]
        {
            ...
            for (uint8 i = 0; i < 2; ++i)
                if (Creature* trigger =
                        me->SummonCreature(WORLD_TRIGGER, triggersPos[i + 4], TEMPSUMMON_TIMED_DESPAWN, 60000))
                    trigger->CastSpell(me, SPELL_NETHERBEAM1 + i, false);
        },
            EVENT_SCENE_5);
            ...
            [&]
        {
            ...
            EVENT_SCENE_6);
        ScheduleUniqueTimedEvent(17500ms, [&] { SetRoomState(GO_STATE_ACTIVE); }, EVENT_SCENE_7);
            ...
            [&]
        {
            ...
            me->SummonCreature(NPC_WORLD_TRIGGER,
                ...
                60000);
            if (Creature* trigger = me->SummonCreature(WORLD_TRIGGER,
                    me->GetPositionX() + 5,
                    me->GetPositionY(),
                    me->GetPositionZ() + 15.0f,
                    0.0f,
    ...
    void IntroduceNewAdvisor(KTYells talkIntroduction, KTActions kaelAction)
    {
        ...
        scheduler.Schedule(2s, [this, talkIntroduction](TaskContext) { Talk(talkIntroduction); });
        ...
        scheduler.Schedule(attackStartTimer,
            [this, advisorNPCId](TaskContext)
        {
    ...
    void PhaseEnchantedWeaponsExecute()
    {
            ...
            [&]
        {
            ...
            scheduler.Schedule(3min, GROUP_PROGRESS_PHASE, [this](TaskContext) { PhaseAllAdvisorsExecute(); });
        },
    ...
    void PhaseAllAdvisorsExecute()
    {
        ...
        scheduler.Schedule(192s, GROUP_PROGRESS_PHASE, [this](TaskContext) { PhaseKaelExecute(); });
    }
    ...
    void PhaseKaelExecute()
    {
        ...
        ScheduleTimedEvent(1000ms, [&] { DoCastVictim(SPELL_FIREBALL); }, 2400ms, 7500ms);
        ScheduleTimedEvent(15000ms, [&] { DoCastRandomTarget(SPELL_FLAME_STRIKE, 0, 100.0f); }, 30250ms, 50650ms);
        ScheduleTimedEvent(50000ms,
            [&]
        {
            Talk(SAY_SUMMON_PHOENIX);
            ...
            [&]
        {
            ...
            scheduler.Schedule(3s, [this](TaskContext) { DoCastSelf(SPELL_ARCANE_DISRUPTION); });
        },
            23s,
            26s);
            ...
            [&]
        {
                    ...
                    [this](TaskContext context)
            {
                ...
                context.Repeat(4s);
            }).Schedule(8500ms, GROUP_PYROBLAST, [this](TaskContext) { scheduler.CancelGroup(GROUP_PYROBLAST); });
        },
            50s);
    }
...

{
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType /*damageEffectType*/,
...
struct npc_lord_sanguinar : public advisor_baseAI
{
    ...
    {
        ScheduleTimedEvent(0s, 2s, [&] { DoCastSelf(SPELL_BELLOWING_ROAR); }, 30s, 40s);
    }
};
...
struct npc_capernian : public advisor_baseAI
{
    ...
    void ScheduleEvents() override
    {
            ...
            [&]
        {
            ...
            2500ms);
        ScheduleTimedEvent(
            7000ms, 10000ms, [&] { DoCastRandomTarget(SPELL_CONFLAGRATION, 0, 30.0f); }, 18500ms, 20500ms);
        ScheduleTimedEvent(3s, [&] { DoCastRandomTarget(SPELL_ARCANE_BURST, 0, 8.0f); }, 6s);
    }
};
...
struct npc_telonicus : public advisor_baseAI
{
    ...
    void ScheduleEvents() override
    {
        ScheduleTimedEvent(0ms, [&] { DoCastVictim(SPELL_BOMB); }, 3600ms, 7100ms);
        ScheduleTimedEvent(13250ms, [&] { DoCastRandomTarget(SPELL_REMOTE_TOY, 0, 100.0f); }, 15750ms);
    }
};
...
struct npc_thaladred : public advisor_baseAI
{
    ...
    void ScheduleEvents() override
    {
            ...
            [&]
        {
            ...
            10000ms);
        ScheduleTimedEvent(4000ms, 19350ms, [&] { DoCastVictim(SPELL_PSYCHIC_BLOW); }, 15700ms, 48900ms);
        ScheduleTimedEvent(3000ms, 6050ms, [&] { DoCastVictim(SPELL_REND); }, 15700ms, 48900ms);
        ScheduleTimedEvent(3000ms,
            6050ms,
            [&]
        {
            if (Unit* victim = me->GetVictim())
...
class spell_kaelthas_summon_weapons : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_kaelthas_summon_weapons::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_kaelthas_mind_control : public SpellScript
{
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(
            spell_kaelthas_mind_control::SelectTarget, EFFECT_ALL, TARGET_UNIT_SRC_AREA_ENEMY);
...
class spell_kaelthas_burn : public AuraScript
{
    ...
    {
        OnEffectPeriodic +=
            AuraEffectPeriodicFn(spell_kaelthas_burn::HandlePeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
    }
};
...
class spell_kaelthas_flame_strike : public AuraScript
{
    ...
    void Register() override
    {
        OnEffectRemove += AuraEffectRemoveFn(
            spell_kaelthas_flame_strike::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
    }
};
...
class spell_kaelthas_gravity_lapse : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_kaelthas_gravity_lapse::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
...
class spell_kaelthas_nether_beam : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_kaelthas_nether_beam::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_kaelthas_summon_nether_vapor : public SpellScript
{
    ...
    void HandleScriptEffect(SpellEffIndex effIndex)
    {
        ...
        for (uint32 i = 0; i < 5; ++i)
            GetCaster()->SummonCreature(NPC_NETHER_VAPOR,
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_kaelthas_summon_nether_vapor::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_kaelthas_remove_enchanted_weapons : public SpellScript
{
    ...
    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(
            spell_kaelthas_remove_enchanted_weapons::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
...
class spell_kaelthas_kael_explodes : public SpellScript
{
    ...
    {
        OnEffectHitTarget +=
            SpellEffectFn(spell_kaelthas_kael_explodes::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
