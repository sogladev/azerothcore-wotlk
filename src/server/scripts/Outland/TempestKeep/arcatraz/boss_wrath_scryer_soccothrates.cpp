...
struct boss_wrath_scryer_soccothrates : public BossAI
{
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
                ...
                [this](TaskContext context)
        {
            ...
            scheduler.Schedule(4600ms,
                [this](TaskContext)
            {
                DoCastAOE(SPELL_CHARGE);
                DoCastSelf(SPELL_FELFIRE, true);

                scheduler.Schedule(300ms,
                    [this](TaskContext context2)
                {
        ...
        switch (events2.ExecuteEvent())
        {
            ...
            case EVENT_PREFIGHT_9:
                if (Creature* dalliah = instance->GetCreature(DATA_DALLIAH))
                {
                    ...
                    dalliah->SetHomePosition(
                        dalliah->GetPositionX(), dalliah->GetPositionY(), dalliah->GetPositionZ(), 1.51737f);
