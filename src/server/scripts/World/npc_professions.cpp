        ...
        for (uint8 j = EQUIPMENT_SLOT_START; j < EQUIPMENT_SLOT_END; ++j)
        {
            ...
            if (item && item->GetTemplate()->RequiredSpell == reqSpell)
            {
                ...
                LOG_DEBUG("scripts.ai",
                    "Player attempt to unlearn spell {}, but item {} is equipped.",
                    reqSpell,
                    item->GetEntry());
                return false;
            }
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->HasSkill(SKILL_ALCHEMY) && player->GetBaseSkillValue(SKILL_ALCHEMY) >= 325 &&
            player->GetLevel() > 67)
        {
            if (player->GetQuestRewardStatus(Q_MASTER_TRANSMUTE) || player->GetQuestRewardStatus(Q_MASTER_ELIXIR) ||
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetBaseSkillValue(SKILL_BLACKSMITHING) >= 225)
        {
            if (player->GetQuestRewardStatus(5284) || player->GetQuestRewardStatus(5302) ||
                player->GetQuestRewardStatus(5283) || player->GetQuestStatus(5301))
            {
                switch (creatureId)
                {
...
public:
    ...
    bool CanLearn(Player* player, uint32 textId, uint32 altTextId, uint32 skillValue, uint32 reqSpellId, uint32 spellId,
        uint32& npcTextId)
    {
        bool res = false;
        npcTextId = textId;
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        switch (creature->GetEntry())
        {
            ...
            case N_TRAINER_DRAGON2: //Thorkaf Dragoneye
                AddGossipItemFor(player,
                    creature->GetEntry() == N_TRAINER_DRAGON1 ? GOSSIP_MENU_PETER_GALEN : GOSSIP_MENU_THORKAF_DRAGONEYE,
            ...
            case N_TRAINER_ELEMENTAL2: //Brumn Winterhoof
                AddGossipItemFor(player,
                    creature->GetEntry() == N_TRAINER_ELEMENTAL1 ? GOSSIP_MENU_SARAH_TANNER
            ...
            case N_TRAINER_TRIBAL2: //Se'Jib
                AddGossipItemFor(player,
                    creature->GetEntry() == N_TRAINER_TRIBAL1 ? GOSSIP_MENU_CARYSSIA_MOONHUNTER : GOSSIP_MENU_SEJIB,
                    GOSSIP_MENU_OPTION_TRAIN,
                    GOSSIP_SENDER_MAIN,
                    GOSSIP_ACTION_TRAIN);
                break;
        ...
        if (player->HasSkill(SKILL_LEATHERWORKING) && player->GetBaseSkillValue(SKILL_LEATHERWORKING) >= 225 &&
            player->GetLevel() > 40)
        {
            switch (creature->GetEntry())
            {
                ...
                case N_TRAINER_DRAGON2: //Thorkaf Dragoneye
                    if (player->HasSpell(S_DRAGON))
                        AddGossipItemFor(player,
                            creature->GetEntry() == N_TRAINER_DRAGON1 ? GOSSIP_MENU_PETER_GALEN
                ...
                case N_TRAINER_TRIBAL2: //Se'Jib
                    if (player->HasSpell(S_TRIBAL))
                        AddGossipItemFor(player,
                            creature->GetEntry() == N_TRAINER_TRIBAL1 ? GOSSIP_MENU_CARYSSIA_MOONHUNTER
                                                                      : GOSSIP_MENU_SEJIB,
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        //TAILORING SPEC
        if (player->HasSkill(SKILL_TAILORING) && player->GetBaseSkillValue(SKILL_TAILORING) >= 350 &&
            player->GetLevel() > 59)
        {
            if (player->GetQuestRewardStatus(10831) || player->GetQuestRewardStatus(10832) ||
...
public:
    ...
    {
        //ENGINEERING SPEC
        if (player->HasSkill(SKILL_ENGINEERING) && player->GetBaseSkillValue(SKILL_ENGINEERING) >= 225 &&
            player->GetLevel() >= 35)
        {
            if (player->GetQuestRewardStatus(3643) || player->GetQuestRewardStatus(3641) ||
            ...
            player->GetLevel() >= 40)
        {
            if (!HasLeatherSpecialty(player) &&
                (player->GetQuestRewardStatus(5141) || player->GetQuestRewardStatus(5143) ||
                    player->GetQuestRewardStatus(5144) || player->GetQuestRewardStatus(5145) ||
                    player->GetQuestRewardStatus(5146) || player->GetQuestRewardStatus(5148)))
            {
                AddGossipItemFor(player,
