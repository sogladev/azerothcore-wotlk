...
std::unordered_map<uint8, AuthHandler> AuthSession::InitHandlers()
{
    ...
    handlers[AUTH_LOGON_CHALLENGE] = {
        STATUS_CHALLENGE, AUTH_LOGON_CHALLENGE_INITIAL_SIZE, &AuthSession::HandleLogonChallenge};
    ...
    handlers[AUTH_RECONNECT_CHALLENGE] = {
        STATUS_CHALLENGE, AUTH_LOGON_CHALLENGE_INITIAL_SIZE, &AuthSession::HandleReconnectChallenge};
    handlers[AUTH_RECONNECT_PROOF] = {
        STATUS_RECONNECT_PROOF, sizeof(AUTH_RECONNECT_PROOF_C), &AuthSession::HandleReconnectProof};
...
void AccountInfo::LoadResult(Field* fields)
{
    ...
    SecurityLevel = static_cast<AccountTypes>(fields[10].Get<uint8>()) > SEC_CONSOLE
                        ? SEC_CONSOLE
...
void AuthSession::Start()
{
    ...
    _queryProcessor.AddCallback(LoginDatabase.AsyncQuery(stmt).WithPreparedCallback(
        std::bind(&AuthSession::CheckIpCallback, this, std::placeholders::_1)));
}
...
void AuthSession::CheckIpCallback(PreparedQueryResult result)
{
    if (result)
    {
        ...
        if (banned)
        {
            ...
            SendPacket(pkt);
            LOG_DEBUG("session",
                "[AuthSession::CheckIpCallback] Banned ip '{}:{}' tries to login!",
...
void AuthSession::LogonChallengeCallback(PreparedQueryResult result)
{
    ...
    if (_accountInfo.IsLockedToIP)
    {
        LOG_DEBUG("server.authserver",
            "[AuthChallenge] Account '{}' is locked to IP - '{}' is logging in from '{}'",
            _accountInfo.Login,
            _accountInfo.LastIP,
            ipAddress);
    ...
    else
    {
        ...
        {
            LOG_DEBUG("server.authserver",
                "[AuthChallenge] Account '{}' is locked to country: '{}' Player country is '{}'",
                _accountInfo.Login,
    ...
    if (_accountInfo.IsBanned)
    {
        ...
        else
        {
            ...
            SendPacket(pkt);
            LOG_INFO("server.authserver.banned",
                "'{}:{}' [AuthChallenge] Temporarily banned account {} tried to login!",
                ipAddress,
                port,
                _accountInfo.Login);
            return;
        ...
        if (auto const& secret = sSecretMgr->GetSecret(SECRET_TOTP_MASTER_KEY))
        {
            ...
            if (!success)
            {
                ...
                LOG_ERROR("server.authserver",
                    "[AuthChallenge] Account '{}' has invalid ciphertext for TOTP token key stored",
                    _accountInfo.Login);
                SendPacket(pkt);
                return;
            }
...
bool AuthSession::HandleLogonProof()
{
    ...
    if (Optional<SessionKey> K = _srp6->VerifyChallengeResponse(logonProof->A, logonProof->clientM))
    {
        ...
        if (sentToken && _totpSecret)
        {
            ...
            std::string token(
                reinterpret_cast<char*>(GetReadBuffer().GetReadPointer() + sizeof(sAuthLogonProof_C) + sizeof(size)),
                size);
        ...
        Acore::Crypto::SHA1::Digest M2 =
            Acore::Crypto::SRP6::GetSessionVerifier(logonProof->A, logonProof->clientM, _sessionKey);
...
void AuthSession::RealmListCallback(PreparedQueryResult result)
{
    ...
    for (auto const& [realmHandle, realm] : sRealmList->GetRealms())
    {
        ...
        bool okBuild = ((_expversion & POST_BC_EXP_FLAG) && realm.Build == _build) ||
                       ((_expversion & PRE_BC_EXP_FLAG) && !AuthHelper::IsPreBCAcceptedClientBuild(realm.Build));
...
bool AuthSession::VerifyVersion(
    uint8 const* a, int32 aLength, Acore::Crypto::SHA1::Digest const& versionProof, bool isReconnect)
{
