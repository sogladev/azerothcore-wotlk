...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(
        CHAR_INS_QUEST_POOL_SAVE, "INSERT INTO pool_quest_save (pool_id, quest_id) VALUES (?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHARACTER_BAN,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHARACTER_BAN,
    ...
    PrepareStatement(CHAR_DEL_CHARACTER_BAN,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_BANINFO,
    ...
    PrepareStatement(
        CHAR_DEL_QUEST_STATUS_SEASONAL, "DELETE FROM character_queststatus_seasonal WHERE event = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_QUEST_STATUS_DAILY_CHAR, "DELETE FROM character_queststatus_daily WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_QUEST_STATUS_WEEKLY_CHAR, "DELETE FROM character_queststatus_weekly WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_QUEST_STATUS_MONTHLY_CHAR,
        "DELETE FROM character_queststatus_monthly WHERE guid = ?",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_QUEST_STATUS_SEASONAL_CHAR,
    ...
    PrepareStatement(
        CHAR_INS_BATTLEGROUND_RANDOM, "INSERT INTO character_battleground_random (guid) VALUES (?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_AURAS,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_SEL_CHARACTER_SPELL, "SELECT spell, specMask FROM character_spell WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHARACTER_QUESTSTATUS,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHARACTER_DAILYQUESTSTATUS,
        "SELECT quest, time FROM character_queststatus_daily WHERE guid = ?",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHARACTER_WEEKLYQUESTSTATUS,
        "SELECT quest FROM character_queststatus_weekly WHERE guid = ?",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHARACTER_MONTHLYQUESTSTATUS,
    ...
    PrepareStatement(CHAR_INS_CHARACTER_DAILYQUESTSTATUS,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_CHARACTER_WEEKLYQUESTSTATUS,
        "INSERT INTO character_queststatus_weekly (guid, quest) VALUES (?, ?)",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_CHARACTER_MONTHLYQUESTSTATUS,
    ...
    PrepareStatement(CHAR_SEL_MAIL_SERVER_CHARACTER,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_MAIL_SERVER_CHARACTER,
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_DECLINEDNAMES,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHARACTER_ACHIEVEMENTS,
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_EQUIPMENTSETS,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHARACTER_ENTRY_POINT,
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_GLYPHS,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_SEL_CHARACTER_TALENTS, "SELECT spell, specMask FROM character_talent WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_SEL_CHARACTER_SKILLS, "SELECT skill, value, max FROM character_skills WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_SEL_CHARACTER_RANDOMBG, "SELECT guid FROM character_battleground_random WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_SEL_CHARACTER_BANNED, "SELECT guid FROM character_banned WHERE guid = ? AND active = 1", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_ACCOUNT_INSTANCELOCKTIMES,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_BREW_OF_THE_MONTH,
    ...
    PrepareStatement(
        CHAR_UPD_AUCTION_BID, "UPDATE auctionhouse SET buyguid = ?, lastbid = ? WHERE id = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(
        CHAR_INS_MAIL_ITEM, "INSERT INTO mail_items(mail_id, item_guid, receiver) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_EXPIRED_MAIL,
    ...
    PrepareStatement(CHAR_UPD_MAIL_RETURNED,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_UPD_MAIL_ITEM_RECEIVER, "UPDATE mail_items SET receiver = ? WHERE item_guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ITEM_OWNER, "UPDATE item_instance SET owner_guid = ? WHERE guid = ?", CONNECTION_ASYNC);

    PrepareStatement(CHAR_SEL_ITEM_REFUNDS,
    ...
    PrepareStatement(
        CHAR_DEL_ITEM_INSTANCE_BY_OWNER, "DELETE FROM item_instance WHERE owner_guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_GIFT_BY_ITEM,
    ...
    PrepareStatement(CHAR_INS_GUILD_BANK_TAB,
        "INSERT INTO guild_bank_tab (guildid, TabId) VALUES (?, ?)",
        CONNECTION_ASYNC); // 0: uint32, 1: uint8
    PrepareStatement(CHAR_DEL_GUILD_BANK_TAB,
    ...
    PrepareStatement(CHAR_DEL_GUILD_BANK_RIGHTS_FOR_RANK,
    ...
    PrepareStatement(
        CHAR_UPD_GUILD_MOTD, "UPDATE guild SET motd = ? WHERE guildid = ?", CONNECTION_ASYNC); // 0: string, 1: uint32
    PrepareStatement(
        CHAR_UPD_GUILD_INFO, "UPDATE guild SET info = ? WHERE guildid = ?", CONNECTION_ASYNC); // 0: string, 1: uint32
    ...
    // 0-5: uint32
    PrepareStatement(CHAR_UPD_GUILD_EMBLEM_INFO,
    ...
    PrepareStatement(CHAR_UPD_GUILD_RANK_BANK_MONEY,
        ...
        CONNECTION_ASYNC); // 0: uint32, 1: uint8, 2: uint32
    PrepareStatement(CHAR_UPD_GUILD_BANK_TAB_TEXT,
    ...
    PrepareStatement(CHAR_UPD_CHANNEL_USAGE,
        "UPDATE channels SET lastUsed = UNIX_TIMESTAMP() WHERE channelId = ?",
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_OLD_CHANNELS, "DELETE FROM channels WHERE lastUsed + ? < UNIX_TIMESTAMP()", CONNECTION_ASYNC);
    ...
    PrepareStatement(
        CHAR_DEL_CHANNEL_BAN, "DELETE FROM channels_bans WHERE channelId = ? AND playerGUID = ?", CONNECTION_ASYNC);
    ...
    // Account data
    PrepareStatement(
        CHAR_SEL_ACCOUNT_DATA, "SELECT type, time, data FROM account_data WHERE accountId = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_PLAYER_ACCOUNT_DATA,
    ...
    PrepareStatement(CHAR_REP_PLAYER_ACCOUNT_DATA,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_PLAYER_ACCOUNT_DATA, "DELETE FROM character_account_data WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(
        CHAR_UPD_INSTANCE_SAVE_ENCOUNTERMASK, "UPDATE instance SET completedEncounters=? WHERE id=?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_ALL_GAME_EVENT_CONDITION_SAVE,
    ...
    // Petitions
    PrepareStatement(
        CHAR_DEL_ALL_PETITION_SIGNATURES, "DELETE FROM petition_sign WHERE playerguid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_PETITION_SIGNATURE, "DELETE FROM petition_sign WHERE playerguid = ? AND type = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_ARENA_TEAM,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_ARENA_TEAM_MEMBER,
    ...
    PrepareStatement(
        CHAR_DEL_ARENA_TEAM_MEMBERS, "DELETE FROM arena_team_member WHERE arenaTeamId = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_UPD_ARENA_TEAM_CAPTAIN, "UPDATE arena_team SET captainGuid = ? WHERE arenaTeamId = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_ARENA_TEAM_MEMBER,
        "DELETE FROM arena_team_member WHERE arenaTeamId = ? AND guid = ?",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ARENA_TEAM_STATS,
    ...
    PrepareStatement(CHAR_SEL_PLAYER_ARENA_TEAMS,
        ...
        CONNECTION_SYNCH);
    PrepareStatement(
        CHAR_UPD_ARENA_TEAM_NAME, "UPDATE arena_team SET name = ? WHERE arenaTeamId = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(
        CHAR_DEL_CORPSES_FROM_MAP, "DELETE FROM corpse WHERE mapId = ? AND instanceId = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_CREATURE_RESPAWN,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CREATURE_RESPAWN_BY_INSTANCE,
    ...
    PrepareStatement(CHAR_DEL_GO_RESPAWN,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_GO_RESPAWN_BY_INSTANCE,
    ...
    PrepareStatement(CHAR_INS_GM_SURVEY,
    ...
    PrepareStatement(CHAR_INS_GM_SUBSURVEY,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_LAG_REPORT,
        "INSERT INTO lag_reports (guid, lagType, mapId, posX, posY, posZ, latency, createTime) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        CONNECTION_ASYNC);

    // LFG Data
    PrepareStatement(
        CHAR_REP_LFG_DATA, "REPLACE INTO lfg_data (guid, dungeon, state) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHARACTER,
        ...
        "equipmentCache=?,ammoId=?,knownTitles=?,actionBars=?,grantableLevels=?,innTriggerId=?,extraBonusTalentCount=?,online=? WHERE guid=?",
        CONNECTION_ASYNC);

    PrepareStatement(
        CHAR_UPD_ADD_AT_LOGIN_FLAG, "UPDATE characters SET at_login = at_login | ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_UPD_REM_AT_LOGIN_FLAG, "UPDATE characters set at_login = at_login & ~ ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_GROUP_MEMBER,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_GROUP_MEMBER, "DELETE FROM group_member WHERE memberGuid = ? AND guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(
        CHAR_UPD_GROUP_MEMBER_SUBGROUP, "UPDATE group_member SET subgroup = ? WHERE memberGuid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_UPD_GROUP_MEMBER_FLAG, "UPDATE group_member SET memberFlags = ? WHERE memberGuid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_GROUP_DIFFICULTY, "UPDATE `groups` SET difficulty = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_UPD_GROUP_RAID_DIFFICULTY, "UPDATE `groups` SET raidDifficulty = ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_INVALID_ACHIEV_PROGRESS_CRITERIA,
        "DELETE FROM character_achievement_progress WHERE criteria = ?",
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_INVALID_ACHIEVMENT, "DELETE FROM character_achievement WHERE achievement = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(
        CHAR_UPD_CHAR_NAME_AT_LOGIN, "UPDATE characters set name = ?, at_login = ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(
        CHAR_DEL_CHAR_INSTANCE_BY_INSTANCE, "DELETE FROM character_instance WHERE instance = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHAR_INSTANCE_BY_INSTANCE_NOT_EXTENDED,
        "DELETE FROM character_instance WHERE instance = ? AND extended = 0",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHAR_INSTANCE_SET_NOT_EXTENDED,
        "UPDATE character_instance SET extended = 0 WHERE instance = ?",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHAR_INSTANCE_BY_INSTANCE_GUID,
    ...
    PrepareStatement(CHAR_INS_ARENA_LOG_MEMBERSTATS,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_GENDER_AND_APPEARANCE,
        "UPDATE characters SET gender = ?, skin = ?, face = ?, hairStyle = ?, hairColor = ?, facialStyle = ? WHERE guid = ?",
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_CHARACTER_SKILL, "DELETE FROM character_skills WHERE guid = ? AND skill = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_REM_CHARACTER_SOCIAL_FLAGS,
        "UPDATE character_social SET flags = flags & ~ ? WHERE guid = ? AND friend = ?",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_CHARACTER_SOCIAL,
        "REPLACE INTO character_social (guid, friend, flags) VALUES (?, ?, ?)",
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_CHARACTER_SOCIAL, "DELETE FROM character_social WHERE guid = ? AND friend = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHARACTER_SOCIAL_NOTE,
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_AURA_FROZEN,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHARACTER_ONLINE,
        "SELECT name, account, map, zone FROM characters WHERE online > 0",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_CHAR_DEL_INFO_BY_GUID,
    ...
    PrepareStatement(CHAR_SEL_CHAR_PINFO,
    ...
    PrepareStatement(CHAR_SEL_PINFO_BANS,
        ...
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_PINFO_MAILS,
    ...
    PrepareStatement(CHAR_SEL_CHAR_HOMEBIND,
        ...
        CONNECTION_SYNCH);
    PrepareStatement(
        CHAR_SEL_CHAR_GUID_NAME_BY_ACC, "SELECT guid, name FROM characters WHERE account = ?", CONNECTION_SYNCH);
    PrepareStatement(
        CHAR_SEL_POOL_QUEST_SAVE, "SELECT quest_id FROM pool_quest_save WHERE pool_id = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_CHAR_RACE_OR_FACTION_CHANGE_INFOS,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHAR_AT_LOGIN_TITLES_MONEY,
        "SELECT at_login, knownTitles, money FROM characters WHERE guid = ?",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_CHAR_COD_ITEM_MAIL,
        ...
        CONNECTION_SYNCH);
    PrepareStatement(
        CHAR_SEL_CHAR_SOCIAL, "SELECT DISTINCT guid FROM character_social WHERE friend = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_NEXT_MAIL_DELIVERYTIME,
        ...
        CONNECTION_SYNCH);
    PrepareStatement(
        CHAR_DEL_CHAR_AURA_FROZEN, "DELETE FROM character_aura WHERE spell = 9454 AND guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_MAIL_COUNT_ITEM,
        ...
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_AUCTIONHOUSE_COUNT_ITEM,
    ...
    PrepareStatement(CHAR_SEL_CHAR_INVENTORY_ITEM_BY_ENTRY,
        ...
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_CHAR_INVENTORY_ITEM_BY_ENTRY_AND_OWNER,
    ...
    PrepareStatement(CHAR_DEL_CHAR_ACHIEVEMENT_PROGRESS,
    ...
    PrepareStatement(CHAR_INS_CHAR_ACHIEVEMENT_PROGRESS,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_CHAR_ACHIEVEMENT_OFFLINE_UPDATES,
        "INSERT INTO character_achievement_offline_updates (guid, update_type, arg1, arg2, arg3) VALUES (?, ?, ?, ?, ?)",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHAR_ACHIEVEMENT_OFFLINE_UPDATES,
        "SELECT update_type, arg1, arg2, arg3 FROM character_achievement_offline_updates WHERE guid = ?",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHAR_ACHIEVEMENT_OFFLINE_UPDATES,
        "DELETE FROM character_achievement_offline_updates WHERE guid = ?",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHAR_REPUTATION_BY_FACTION,
    ...
    PrepareStatement(CHAR_INS_CHAR_REPUTATION_BY_FACTION,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHAR_ARENA_POINTS,
        "UPDATE characters SET arenaPoints = (arenaPoints + ?) WHERE guid = ?",
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_ITEM_REFUND_INSTANCE, "DELETE FROM item_refund_instance WHERE item_guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_ITEM_REFUND_INSTANCE,
    ...
    PrepareStatement(
        CHAR_DEL_PETITION_SIGNATURE_BY_GUID, "DELETE FROM petition_sign WHERE petitionguid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_CHAR_DECLINED_NAME, "DELETE FROM character_declinedname WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_CHAR_DECLINED_NAME,
    ...
    PrepareStatement(CHAR_DEL_CHAR_ACHIEVEMENT_BY_ACHIEVEMENT,
    ...
    PrepareStatement(CHAR_UPD_CHAR_ACHIEVEMENT,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHAR_INVENTORY_FACTION_CHANGE,
        "UPDATE item_instance ii, character_inventory ci SET ii.itemEntry = ? WHERE ii.itemEntry = ? AND ci.guid = ? AND ci.item = ii.guid",
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_CHAR_SPELL_BY_SPELL, "DELETE FROM character_spell WHERE guid = ? AND spell = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHAR_SPELL_FACTION_CHANGE,
    ...
    PrepareStatement(CHAR_SEL_CHAR_REP_BY_FACTION,
        ...
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_DEL_CHAR_REP_BY_FACTION,
    ...
    PrepareStatement(CHAR_UPD_CHAR_REP_FACTION_CHANGE,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_UPD_CHAR_TITLES_FACTION_CHANGE, "UPDATE characters SET knownTitles = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_RES_CHAR_TITLES_FACTION_CHANGE, "UPDATE characters SET chosenTitle = 0 WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_CHAR_SPELL_COOLDOWN, "DELETE FROM character_spell_cooldown WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_CHAR_QUESTSTATUS_REWARDED,
    ...
    PrepareStatement(CHAR_DEL_CHAR_ACHIEVEMENTS,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_CHAR_EQUIPMENTSETS, "DELETE FROM character_equipmentsets WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_GUILD_EVENTLOG_BY_PLAYER,
    ...
    PrepareStatement(
        CHAR_UDP_CHAR_HONOR_POINTS, "UPDATE characters SET totalHonorPoints = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UDP_CHAR_HONOR_POINTS_ACCUMULATIVE,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_UDP_CHAR_ARENA_POINTS, "UPDATE characters SET arenaPoints = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UDP_CHAR_ARENA_POINTS_ACCUMULATIVE,
        "UPDATE characters SET arenaPoints = arenaPoints + ? WHERE guid = ?",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_UDP_CHAR_MONEY, "UPDATE characters SET money = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_UDP_CHAR_MONEY_ACCUMULATIVE, "UPDATE characters SET money = money + ? WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_CHAR_ACTION_BY_BUTTON_SPEC,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_CHAR_INVENTORY_BY_ITEM, "DELETE FROM character_inventory WHERE item = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHAR_INVENTORY_BY_BAG_SLOT,
        "DELETE FROM character_inventory WHERE bag = ? AND slot = ? AND guid = ?",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_MAIL,
    ...
    PrepareStatement(CHAR_REP_CHAR_QUESTSTATUS,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHAR_QUESTSTATUS_BY_QUEST,
    ...
    PrepareStatement(CHAR_UPD_CHAR_QUESTSTATUS_REWARDED_FACTION_CHANGE,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHAR_QUESTSTATUS_REWARDED_ACTIVE,
    ...
    PrepareStatement(CHAR_UPD_CHAR_QUESTSTATUS_REWARDED_ACTIVE_BY_QUEST,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_CHAR_SKILL_BY_SKILL, "DELETE FROM character_skills WHERE guid = ? AND skill = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UDP_CHAR_SKILLS,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_INS_CHAR_SPELL, "INSERT INTO character_spell (guid, spell, specMask) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(
        CHAR_DEL_PETITION_SIGNATURE_BY_OWNER, "DELETE FROM petition_sign WHERE ownerguid = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_PETITION_BY_OWNER_AND_TYPE, "DELETE FROM petition WHERE ownerguid = ? AND type = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_PETITION_SIGNATURE_BY_OWNER_AND_TYPE,
        "DELETE FROM petition_sign WHERE ownerguid = ? AND type = ?",
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_INS_CHAR_GLYPHS, "INSERT INTO character_glyphs VALUES(?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_CHAR_TALENT_BY_SPELL, "DELETE FROM character_talent WHERE guid = ? AND spell = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_CHAR_TALENT,
        "INSERT INTO character_talent (guid, spell, specMask) VALUES (?, ?, ?)",
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_CHAR_ACTION_EXCEPT_SPEC, "DELETE FROM character_action WHERE spec<>? AND guid = ?", CONNECTION_ASYNC);

    // Items that hold loot or money
    PrepareStatement(CHAR_SEL_ITEMCONTAINER_ITEMS,
        "SELECT containerGUID, itemid, item_index, count, randomPropertyId, randomSuffix, follow_loot_rules, freeforall, is_blocked, is_counted, is_underthreshold, needs_quest, conditionLootId FROM item_loot_storage",
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_DEL_ITEMCONTAINER_SINGLE_ITEM,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_ITEMCONTAINER_SINGLE_ITEM,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_ITEMCONTAINER_CONTAINER, "DELETE FROM item_loot_storage WHERE containerGUID = ?", CONNECTION_ASYNC);

    // Calendar
    PrepareStatement(CHAR_REP_CALENDAR_EVENT,
    ...
    PrepareStatement(CHAR_REP_CALENDAR_INVITE,
    ...
    PrepareStatement(CHAR_DEL_CHAR_PET_DECLINEDNAME_BY_OWNER,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_CHAR_PET_DECLINEDNAME, "DELETE FROM character_pet_declinedname WHERE id = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_ADD_CHAR_PET_DECLINEDNAME,
    ...
    PrepareStatement(CHAR_SEL_PET_SPELL_COOLDOWN,
    ...
    PrepareStatement(CHAR_INS_PET_SPELL_COOLDOWN,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DEL_PET_SPELL_BY_SPELL, "DELETE FROM pet_spell WHERE guid = ? AND spell = ?", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_INS_PET_SPELL, "INSERT INTO pet_spell (guid, spell, active) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHAR_PET_NAME,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_UPD_CHAR_PET_SLOT_BY_ID, "UPDATE character_pet SET slot = ? WHERE owner = ? AND id = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHAR_PET_BY_ID, "DELETE FROM character_pet WHERE id = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CHAR_PET_BY_SLOT,
        "DELETE FROM character_pet WHERE owner = ? AND (slot = ? OR slot > ?)",
        CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_CHAR_PET,
    ...
    PrepareStatement(CHAR_SEL_PVPSTATS_FACTIONS_OVERALL,
        ...
        CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_PVPSTATS_BRACKET_MONTH,
    ...
    PrepareStatement(CHAR_SEL_RECOVERY_ITEM_OLD_ITEMS,
        "SELECT Guid, ItemEntry FROM recovery_item WHERE DeleteDate IS NOT NULL AND DeleteDate < ?",
        CONNECTION_SYNCH);
    ...
    // Character names
    PrepareStatement(
        CHAR_INS_RESERVED_PLAYER_NAME, "INSERT IGNORE INTO reserved_name (name) VALUES (?)", CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_INS_PROFANITY_PLAYER_NAME, "INSERT IGNORE INTO profanity_name (name) VALUES (?)", CONNECTION_ASYNC);

    // Character settings
    PrepareStatement(
        CHAR_SEL_CHAR_SETTINGS, "SELECT source, data FROM character_settings WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_CHAR_SETTINGS,
    ...
    PrepareStatement(CHAR_SELECT_INSTANCE_SAVED_DATA,
    ...
    PrepareStatement(CHAR_INSERT_INSTANCE_SAVED_DATA,
        ...
        CONNECTION_ASYNC);
    PrepareStatement(
        CHAR_DELETE_INSTANCE_SAVED_DATA, "DELETE FROM instance_saved_go_state_data WHERE id = ?", CONNECTION_ASYNC);
